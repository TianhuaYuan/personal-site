...
> **一句话**：用户说话像打哑谜——"它""那个""这个"满天飞，Query 改写就是那个帮你把哑谜翻译成标准题目的"翻译官"。
> 
...
> Query 改写解决的是'用户怎么问'和'知识库怎么写'之间的语义鸿沟。不改写，检索就是在猜。
> 
...
```mermaid
flowchart TD
    Q[用户 Query] --> R1{有对话历史<br/>且话题延续?}
    R1 -->|是| RESOLVE[指代消解<br/>LLM 把'它'还原为实体]
    R1 -->|否| R2{Query 类型?}
    RESOLVE --> R2
    R2 -->|精确标识符<br/>错误码/编号/型号| BM25[走 BM25 关键词检索]
    R2 -->|短模糊查询<br/>≤8字的口语化| HYDE[HyDE 生成假想答案<br/>+ 原始 query 双路检索]
    R2 -->|复合意图<br/>对比/分析/汇总| DECOMP[问题分解<br/>拆为子问题分别检索]
    R2 -->|普通语义查询| VEC[直接向量检索]
    BM25 --> RRF[RRF 合并]
    HYDE --> RRF
    VEC --> RRF
    DECOMP --> RRF
...
## 
> ▶ 对应原理：[[19-Query改写与多轮对话指代消解|19-Query改写与多轮对话指代消解]]

相关链接

- 目录：[[00-AI]]
- 上一篇：[[08-混合检索：向量 + BM25 + RRF 融合]]
- 下一篇：[[10-RAG 评估：检索指标 + 生成指标]]
- 项目实践：[[笔记/技术学习/项目开发笔记/ai-resume笔记/01_技术研读/02_RAG流水线.md#第 7 层：Prompt 组装|ai-resume: RAG Prompt组装]]
