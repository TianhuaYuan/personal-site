...
A：TS 内置了一批"基于已有类型，批量造出变体"的工具类型。它们本质都是**泛型 + 映射类型（mapped types）**的语法糖，记住高频的几个即可。
英文全称：**Utility Types = 工具类型**；**Mapped Types = 映射类型**；**modifier = 修饰符**（ 可选、 只读）。
...
A：| 工具类型 | 作用 | 类比 |
| --- | --- | --- |
|  | 把 T 的所有属性变成**可选**  | 把"必填表"改成"选填表" |
|  | 从 T 里**挑出**部分键 K | 从整套乐高里只挑几块 |
|  | 从 T 里**去掉**部分键 K | 从整套乐高里拿走几块 |
|  | 构造"键为 K、值为 V"的对象类型 | 批量生成统一格式的字典 |
...
**Q5：三、怎么答 —— 怎么理解？**
A：先说"它们是基于映射类型造变体的语法糖"，然后举例  四个最常用的，能现场写出  的简化实现加分：

**Q6：核心速记主线有哪些？**
A：抓住这几根：零、一句话定位、一、高频四个（最爱问）、二、进阶常用、三、怎么答、四、要点图解。
