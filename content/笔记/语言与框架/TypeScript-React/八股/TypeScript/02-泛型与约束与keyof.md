...
A：**泛型（Generics）**：让函数 / 类 / 接口"参数化类型"，做到"类型也能当参数传"。
**约束（constraint，）**：给泛型加"门槛"，限制它能接受的类型范围。
****：取一个对象类型的**所有键名组成的联合类型（union）**。
...
**Q3：一、生活化类比 —— 怎么理解？**
A：泛型像**模具**：你塞进什么材料（类型 ），产出的产品就是那种材料做的——不用为每种材料开一条生产线。
约束  像**机器只收合格原材料**：不是啥都让进，必须符合某个规格。
像**拿出对象的钥匙串**：得到"所有钥匙名（键）"的清单，用来保证"按钥匙取东西"不会拿错。

**Q4：二、为什么需要约束 —— 怎么理解？**
A：没有约束时， 是"任意类型"，你不能在它身上调用任何具体方法（TS 不知道它有没有）。 告诉 TS："放心， 至少有这些东西"，于是你能安全访问 。

**Q5：三、keyof + 泛型 = 类型安全的取值 —— 怎么理解？**
A：保证  一定是  真实存在的键；返回值类型  自动跟随"这个键对应的值类型"。这是 TS 里做"类型安全 getter"的标准写法。

**Q6：核心速记主线有哪些？**
A：抓住这几根：零、一句话定位、一、生活化类比、二、为什么需要约束、三、keyof + 泛型 = 类型安全的取值、四、要点图解、为什么需要泛型？。
