...
A：四个都是"特殊类型"，但职责完全不同：
****：彻底关闭类型检查——"啥都能进，啥都能出"。
****：类型安全的 ——"啥都能进，但用之前必须先验明正身（narrowing）"。
****："永远不可能有的值"——用于穷尽检查和抛异常的函数返回。
****：函数"没有有意义的返回值"（本质就是返回 ）。
...
**Q3：一、生活化类比 —— 怎么理解？**
A：= **不验票的闸机**：人随便进，你也随便被当成任意身份，安全全无。
= **验票闸机**：人能进候车区，但进站台前必须查票（先收窄类型）才放。
= **黑洞**：你永远走不到那儿（函数抛错中断 / switch 的兜底分支）。
= **空手而归**：函数跑完了，但啥也没带回来。

**Q4：二、关键区别 —— 怎么理解？**
A：| 类型 | 能赋给谁 | 能赋给它啥 | 典型用途 |
| --- | :---: | :---: | --- |
|  | 任意 | 任意 | 迁移旧 JS、临时逃生（尽量少用） |
|  | 仅 / | 任意 | 接收未知外部输入（API/JSON） |
|  | 任意（它是底部） | 无（没有值属于 never） | 穷尽检查兜底、抛错函数 |
|  | 仅 // | / | 无返回值函数 |

**Q5：三、怎么答 —— 怎么理解？**
A：放弃安全； 是"受控的 any"，必须先 / 收窄。
表示"不可达"，常用于  穷尽检查的  里用  在编译期报错。
≠ ：函数标注  只是"不在乎返回值"，调用方不会拿到有意义的值。

**Q6：核心速记主线有哪些？**
A：抓住这几根：零、一句话定位、一、生活化类比、二、关键区别、三、怎么答、四、要点图解、为什么需要类型标注？。


## 相关链接

- 目录：[[00-TypeScript]]
- 下一篇：[[02-泛型与约束与keyof|泛型与约束]]
