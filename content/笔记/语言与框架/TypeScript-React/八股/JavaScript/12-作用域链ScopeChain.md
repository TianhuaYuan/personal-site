...
**Q1：一句话讲清「作用域链ScopeChain」到底是什么？**

A：作用域链是 JS 找变量时从当前作用域一层层向外查、找到就停的链路；JS 是词法作用域，函数作用域在定义时就定，与在哪调用无关。

**Q2：三种作用域 —— 怎么理解？**

A：作用域=变量能活动的范围。全局作用域像小区公共区域谁都能用；函数作用域像自家冰箱，函数外进不去；块级作用域是 let/const 的 {} 内部。记忆：var 函数级能穿透 if 的 {}，let/const 块级不能——var 无视花括号。

**Q3：作用域链的查找规则 —— 怎么理解？**

A：找变量从内往外一层层找：inner→outer→全局，找到就停，最外层还找不到就 ReferenceError。方向永远由内向外，不会反过来。像你先翻自己口袋，没有再问爸妈，再没有问小区，全程单向。

**Q4：词法作用域（静态作用域）—— 必问 —— 怎么理解？**

A：JS 函数作用域在"定义时"就定，和"在哪调用"无关。fn 定义在全局，哪怕在 run 内部调用，它记住的还是出生地（全局）的 name，输出"全局"不是"run内部"。一句话：函数记住自己出生的地方，闭包正是利用这点。

**Q5：为什么这个常被考 —— 怎么理解？**

A：这是闭包、this、模块化所有后续概念的地基。弄混词法作用域就会误以为"在哪调就在哪找"，实际是"定义处决定"。面试常拿嵌套函数+外部变量改值来考，认准"定义时定死"四字即可。

**Q6：核心速记主线有哪些？**

- 三种作用域：全局 / 函数（var）/ 块级（let、const）

- 查找规则：由内向外，找到即停，全局外报 ReferenceError

- 词法作用域：定义时确定，与调用位置无关

- 闭包基础：函数记住出生地，正赖此特性

- 坑：var 穿透 {}，let/const 被 {} 限住
