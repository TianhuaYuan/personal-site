...
A：**不是。你写的是 JSX，不是 HTML。** JSX 长得像 HTML，但它会被编译成 JS 函数调用。
所以这篇笔记不是教你手写静态网页——是教你**读懂 JSX 里那些标签和样式**，让你不被卡住。
...
A：一个标签长这样：
**和 TS 对照**：标签 ≈ 函数调用。 ≈  —— 把文字包进段落组件里。
 = division，就是一个**看不见的盒子**，用来把东西归拢到一起。它是**块级元素**——自己独占一行，宽度撑满父容器。React 里  是无处不在的外壳。
 也是看不见的盒子，但它是**行内元素**——不会独占一行，只占内容宽度。用来圈住一段文字加样式。

**Q4：二、CSS：样式怎么长成这样的 —— 怎么理解？**
A：**和 TS 对照**：CSS 规则 ≈ 给元素声明默认属性。 相当于 "所有  的 color 属性默认值设为 red"。
React 里这三种对应： 内联、CSS-in-JS（styled-components）、CSS Modules / Tailwind。先知道原生怎么写，后面学 React 样式方案才不会懵。
**和 React 对照**：
**后面写的覆盖前面写的；越具体的覆盖越笼统的。

**Q5：三、盒模型——CSS 最重要的概念，没有之一 —— 怎么理解？**
A：**每个元素在页面上都是一个矩形盒子。** CSS 做的就是三件事：
盒子多大（width / height）
盒子放哪（margin / flex / position）
盒子里面长什么样（padding / border / background / color）
**生活比喻**：快递盒。

**Q6：核心速记主线有哪些？**
A：抓住这几根：零、先回答一个根本问题：React 里写的 `<div>` 到底是不是 HTML？、一、HTML：页面骨架的 10 个标签、二、CSS：样式怎么长成这样的、三、盒模型——CSS 最重要的概念，没有之一、四、Flexbox —— React 布局 90% 靠这个、五、常用样式属性速查。
