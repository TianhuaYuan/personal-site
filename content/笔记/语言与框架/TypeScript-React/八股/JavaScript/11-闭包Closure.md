...
**Q1：一句话讲清「闭包Closure核心考点」到底是什么？**
A：**闭包 = 函数 + 函数定义时所在的外部作用域。** 当一个函数"记住"了它出生时的外部变量，即使外部函数已经执行完了，这些变量仍然可以被内部函数访问。
**通俗比喻**：你毕业离校了（外部函数执行完了），但你手里有张校友卡（闭包），凭这张卡你仍然能进校图书馆（访问外部变量）。
**关键理解**： 是  的局部变量，正常应该随着函数执行完毕被垃圾回收。

**Q2：二、闭包（Closure）— 核心考点 —— 怎么理解？**
A：**闭包 = 函数 + 函数定义时所在的外部作用域。** 当一个函数"记住"了它出生时的外部变量，即使外部函数已经执行完了，这些变量仍然可以被内部函数访问。
**通俗比喻**：你毕业离校了（外部函数执行完了），但你手里有张校友卡（闭包），凭这张卡你仍然能进校图书馆（访问外部变量）。
**关键理解**： 是  的局部变量，正常应该随着函数执行完毕被垃圾回收。

**Q3：核心速记主线有哪些？**
A：抓住这几根：二、闭包（Closure）— 核心考点。
