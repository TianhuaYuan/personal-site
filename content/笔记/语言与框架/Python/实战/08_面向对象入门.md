...
A：面向对象编程（OOP）把数据和操作绑进"对象"，用类做模板、对象做实例，主打封装、复用与扩展。

**Q2：1. 图片拷贝案例（带优化） —— 怎么理解？**

A：拷贝图片时，面向过程要把路径传来传去、加进度条就越来越乱；面向对象把 buffer_size、进度封装进 ImageCopier 类，copy/batch_copy 成方法。像请了个专职文员（对象，Object），资料和数据都归他管，比你自己满地找文件清爽（封装，Encapsulation）。

**Q3：2. 面向过程 vs 面向函数式 vs 面向对象 —— 怎么理解？**

A：三范式对比——面向过程：函数+全局数据，小脚本够用；面向函数式：纯函数无副作用，易测试并发安全；面向对象：对象=数据+方法，封装复用扩展强。像一个人干活（过程）、流水线标准件（函数式）、成立部门各管一摊（OOP，Object-Oriented Programming）。

**Q4：3. 类和对象的概念 —— 怎么理解？**

A：类（Class）是模板/蓝图，比如"狗类"定义有名字、会叫；对象（Object）是按模板实例化的具体狗（旺财、大黄）。类属性 species 所有实例共享，实例属性 name 每只不同。像建房图纸 vs 按图盖出的每栋楼。

**Q5：4. `__init__` 方法与 self —— 怎么理解？**

A：__init__ 创建对象时自动调用，self 指向新对象本身，self.name = name 等价于 s.name = name。self 不是关键字，只是约定俗成的名字。像员工入职登记：__init__ 是入职流程，self 就是"这位新员工"（绑定，Binding）。

**Q6：核心速记主线有哪些？**

- 类 Class 是模板，对象 Object 是按模板实例化的具体实例

- 三范式：面向过程 / 函数式 / 面向对象，OOP 主打封装复用

- __init__ 自动初始化，self 指向对象自身（非关键字）

- 类属性共享、实例属性独有；实例/类/静态方法三种
