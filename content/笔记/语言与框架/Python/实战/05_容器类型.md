...
**Q1：一句话讲清「Day 05：字符串/元组/集合/字典」到底是什么？**

A：Python 四容器各有取舍：字符串不可变、元组可哈希、集合去重、字典 O(1) 映射。

**Q2：一、字符串 String：不可变的文本 —— 怎么理解？**

A：字符串是不可变类型——任何"修改"其实都创建新字符串（s[0]="J" 直接 TypeError）。比喻：字符串像刻在石头上的字，改不了，只能另刻一块。切片 s[::-1] 反转、join 拼接、split 分割是高频操作。英文：immutable（不可变）、str。

**Q3：二、元组 Tuple：可哈希的不可变序列 —— 怎么理解？**

A：元组不可变，但内部可变对象（如列表）能改。它可哈希，能当字典 key、作集合元素；常用于函数返回多个值（本质元组解包）。比喻：元组像封装好的礼盒，盒子不能拆但盒里东西能换。⚠️ 单元素元组必须加逗号 (1,)。英文：tuple、hashable（可哈希）、unpacking（解包）。

**Q4：三、集合 Set：去重与集合运算 —— 怎么理解？**

A：集合无序、不重复、只能存可哈希对象，自动去重。支持交集 &、并集 |、差集 -、对称差 ^。比喻：集合像投票箱，同一张票投多次也只算一票；找"两组都有的"用交集，找"独有的"用差集。英文：set、intersection（交集）、deduplication（去重）。

**Q5：四、字典 Dict：O(1) 的键值映射 —— 怎么理解？**

A：字典存键值对，key 唯一且必须可哈希，Python 3.7+ 保持插入顺序，查找 O(1) 哈希表。比喻：字典像快递柜，凭取件码（key）秒取包裹（value）。get() 安全访问不存在返回 None，比 [] 不报错更稳。英文：dict、hash table（哈希表）、O(1)。

**Q6：核心速记主线有哪些？**

- 字符串 str：不可变，改=建新的

- 元组 tuple：可哈希，能当 key、函数多返回值

- 集合 set：去重+集合运算，只存可哈希

- 字典 dict：键值映射，O(1)，3.7+ 保序

- 选型：要键值→dict，要去重→set，不可变→tuple
