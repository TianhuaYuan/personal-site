...
# set_age(-5)     # ❌ ValueError: 年龄不能为负数：-5
...
# ❌ AssertionError: 总数不能为零！

# ⚠️ assert 可以被 -O 参数禁用（python -O script.py）
...
A：完善 Day 09 的愤怒的小鸟游戏，加入异常处理：

**Q2：1. 愤怒的小鸟综合案例（续 Day 09） —— 怎么理解？**
A：完善 Day 09 的愤怒的小鸟游戏，加入异常处理：

**Q3：2. 异常处理基本语法 —— 怎么理解？**
A：[!warning] 避免裸 except
 会捕获包括 （Ctrl+C）和  在内的所有异常，导致程序无法正常退出。
始终用  或更具体的异常类型。

**Q4：4. assert 断言 —— 怎么理解？**
A：[!important] raise vs assert
用于**正式异常处理**，不会被禁用
用于**开发阶段调试**，可能被禁用

**Q5：核心速记主线有哪些？**
A：抓住这几根：1. 愤怒的小鸟综合案例（续 Day 09）、2. 异常处理基本语法、3. raise 抛出异常、4. assert 断言、5. 自定义异常、6. 异常的传递。
