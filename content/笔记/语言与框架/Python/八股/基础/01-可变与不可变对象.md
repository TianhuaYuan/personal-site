...
# s += " world" 等价于 s = s + " world" → 创建新字符串，s 指向新字符串
...
# def 语句执行时：Python 创建了一个空列表，存为 add_item 的默认参数

# 每次调用 add_item 时，bucket 都指向那个唯一的列表
...
# t += (3,)                        # 虽然元组是"不可变"的，但这行会执行！

# 步骤：① 计算 t + (3,) → 创建新元组 (1, 2, 3)
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：二、可变 vs 不可变——你到底能不能改、1. 常见进制介绍、2. 进制之间的相互转换、3. 原码、反码、补码、4. 数据类型分类、5. 自动类型转换与强制类型转换。
