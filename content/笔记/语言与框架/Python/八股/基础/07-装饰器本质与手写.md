...
A：**装饰器（Decorator）**：接收一个函数，返回一个新函数，在不改原函数代码的前提下给它加功能。换句话说——**装饰器就是一个返回函数的闭包**。
**生活类比**：你有一部手机（），你不想拆手机就能给它加壳、贴膜、装防摔套——这些"外套"就是装饰器。手机还是那部手机，但功能（防摔、美观）增强了。

**Q2：一、本质：装饰器就是闭包 —— 怎么理解？**
A：**装饰器（Decorator）**：接收一个函数，返回一个新函数，在不改原函数代码的前提下给它加功能。换句话说——**装饰器就是一个返回函数的闭包**。
**生活类比**：你有一部手机（），你不想拆手机就能给它加壳、贴膜、装防摔套——这些"外套"就是装饰器。手机还是那部手机，但功能（防摔、美观）增强了。

**Q3：二、手写一个最朴素的装饰器 —— 怎么理解？**
A：就是装饰器：它**没有碰  的内部代码**，只是把它"裹"进 ，顺手加了日志。

**Q4：三、语法糖 `@` —— 只是语法简化 —— 怎么理解？**
A：写在函数上面，就是告诉 Python："定义完  后，立刻执行 "。语法糖而已，本质不变。
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一、本质：装饰器就是闭包、二、手写一个最朴素的装饰器、三、语法糖 `@` —— 只是语法简化、四、常见实用装饰器、五、手写考点、六、一句话讲清。
