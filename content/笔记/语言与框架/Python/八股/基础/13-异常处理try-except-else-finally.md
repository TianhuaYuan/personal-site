...
A：****：监控“可能出错”的代码块。
****：捕获并处理指定异常（可多个、可捕获基类）。
****：**仅当 try 没有抛异常时**才执行（正常的后续分支）。
****：**无论是否异常、是否 return**，都一定会执行（常用于释放资源）。
...
****：**仅当 try 没有抛异常时**才执行（正常的后续分支）。
****：**无论是否异常、是否 return**，都一定会执行（常用于释放资源）。
**生活化比喻**：去 ATM 取钱——=插卡取钱；=钱没吐出来卡住了，处理一下（退卡/报警）；=顺利取到钱，开开心心去逛街；=不管成功失败，最后把门带上、衣衫整理好再走。

**Q3：四、高频考点（全是坑） —— 怎么理解？**
A：** 里的  会“覆盖” try/except 的返回值**：
** 不要和  混写**： 里的代码如果也可能抛异常，就不该被前面的  捕获，应放到  内或单独处理。
**别裸 **： 会连 、 一起吞掉，导致  都停不了程序；应捕获具体异常或至少 。
**自定义异常继承 **（不要直接继承 ）。
**资源用  更优雅**： 底层就是  的语法糖。
---

**Q4：核心速记主线有哪些？**
A：抓住这几根：一、四个子句各管什么、二、执行顺序、三、可运行代码示例、四、高频考点（全是坑）、。
