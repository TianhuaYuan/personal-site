...
A：实例方法操作具体对象用 self，类方法用 @classmethod 和 cls 造实例，静态方法无 self/cls 只是归类函数。



**Q2：一、三种方法一眼分清 —— 怎么理解？**

A：咖啡机比喻最清：实例方法用"这台"机器做咖啡（self）、类方法按配方造机器（cls）、静态方法是贴墙清洗说明（Instance/Class/Static Method）。



**Q3：三、可运行代码示例 —— 怎么理解？**

A：代码三件套：def make(self) 实例方法、@classmethod def from_config(cls) 工厂、@staticmethod def clean() 工具函数，调用各不同（factory method 工厂方法）。



**Q4：四、对比速查表 —— 怎么理解？**

A：速查表记首参：实例 self、类 cls、静态无；实例/类方法能访状态，静态谁都不认；类方法做工厂、静态放工具（method resolution 方法解析）。



**Q5：五、核心考点 —— 怎么理解？**

A：核心考点四句：staticmethod 与类方法差在 cls；实例方法也能类调用但要手传 self；类方法做工厂支持多态；staticmethod 不提速（polymorphism 多态）。



**Q6：核心速记主线有哪些？**

- 首参不同：实例 self、类 cls、静态方法无参数

- 静态方法不认类、类方法认类；实例方法认具体对象

- 类方法 @classmethod 常做工厂/备选构造器（多态）

- @staticmethod 只是命名空间归属，无性能优势



**口诀**

A：实例方法 self 认这台，类方法 cls 认这模；

静态方法谁都不认，借命名空间放工具；

类方法做工厂，子类调用返子类；

staticmethod 不提速，区别就在 cls 有无。
...
- [[笔记/语言与框架/Python/八股/基础/13-异常处理try-except-else-finally|异常处理 try-except-else-finally]]
