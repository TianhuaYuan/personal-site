...
**Q1：一句话讲清「你以为的"拷贝"——其实只是多贴了一张标签」到底是什么？**
A：**一句话**： 不是把  的值复制一份给 ——只是让  也指向  指向的那个对象。想真正"复制"一份独立的数据——用拷贝。
**一句话**：浅拷贝 = 建一个新容器，把原容器的每个元素**引用**复制过去。容器是新的，里面的东西还是原来那些。

**Q2：一、深浅拷贝——赋值不是拷贝 —— 怎么理解？**
A：**一句话**： 不是把  的值复制一份给 ——只是让  也指向  指向的那个对象。想真正"复制"一份独立的数据——用拷贝。
**一句话**：浅拷贝 = 建一个新容器，把原容器的每个元素**引用**复制过去。容器是新的，里面的东西还是原来那些。

**Q3：核心速记主线有哪些？**
A：抓住这几根：一、深浅拷贝——赋值不是拷贝。
