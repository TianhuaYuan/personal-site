...
A：把你的 Python 文件想象成一栋楼。变量就是住在楼里的住户。你去喊一个人（），得先确定去哪一层找——**作用域**就是"楼层规则"。
没有作用域会怎样？像早期 BASIC 那样所有变量都是全局的，谁都能改，程序一大就乱成一锅粥。Python 用 **LEGB** 四层"楼层"来约束变量能住哪、能被谁看见。

**Q2：一、为什么会有"作用域"（Scope）？ —— 怎么理解？**
A：把你的 Python 文件想象成一栋楼。变量就是住在楼里的住户。你去喊一个人（），得先确定去哪一层找——**作用域**就是"楼层规则"。
没有作用域会怎样？像早期 BASIC 那样所有变量都是全局的，谁都能改，程序一大就乱成一锅粥。Python 用 **LEGB** 四层"楼层"来约束变量能住哪、能被谁看见。
**LEGB** 是四层作用域的首字母缩写（全称 + 解释）：
**L — Local（局部作用域）**：当前函数内部。

**Q3：二、代码说话：LEGB 实战 —— 怎么理解？**
A：注意一个反直觉的点：** /  /  这些代码块不创建新作用域！** 只有函数、类、模块才创建。所以在  里定义的 ，出了循环还能用（在 Python 里这常被吐槽，但确实是这样）。

**Q4：三、`UnboundLocalError`：最常见的坑 —— 怎么理解？**
A：为什么会炸？Python 看到函数里写了 （赋值），就认定  是**局部变量**。但等号右边又去读 ——此时局部  还没赋值，于是报 。
**修法一：**——明确说"我要用的是大堂那个"
**修法二：**——说"我要用的是外层函数那个"
| 关键字 | 作用范围 | 典型场景 |
| :--- | :--- | :--- |
...
**Q5：四、一句话讲清 —— 怎么理解？**
A："LEGB 是 Python 名字解析规则：Local → Enclosing → Global → Built-in，就近命中。函数内赋值会被当成局部变量，所以改全局要用 ，改闭包外层要用 。只有函数、类、模块才引入新作用域，/ 不引入。"

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、为什么会有"作用域"（Scope）？、二、代码说话：LEGB 实战、三、`UnboundLocalError`：最常见的坑、四、一句话讲清。
