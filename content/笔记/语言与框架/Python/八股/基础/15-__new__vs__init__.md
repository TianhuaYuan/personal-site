...
A：****：负责**创建**实例——分配内存、真正“造出”一个对象。它是个**静态方法**，必须  一个对象。
****：负责**初始化**实例——给刚造好的对象设置属性。它**不返回任何值**。
**生活化比喻**： 是“盖房子”——打地基、搭框架，返回一个空房子； 是“装修”——给房子配家具家电。**先有房子（__new__ 返回）才能装修（__init__）**，顺序不能反。
...
**Q3：二、执行顺序（关键） —— 怎么理解？**
A：要点：** 先执行并返回实例后，Python 才会调用 **。如果  返回的不是一个  的实例， 压根不会被调用。

**Q4：四、什么时候必须碰 `__new__` —— 怎么理解？**
A：**继承不可变类型**（int / str / tuple）：它们的值在创建时就定死了，想“改值”只能在  里动手。
**单例模式 Singleton**：控制全局只创建一个实例。
**对象池 / 缓存**：在  里拦截，复用已有实例。

**Q5：核心速记主线有哪些？**
A：抓住这几根：一、分工：先建房，再装修、二、执行顺序（关键）、三、可运行代码示例、四、什么时候必须碰 `__new__`、五、核心考点。
