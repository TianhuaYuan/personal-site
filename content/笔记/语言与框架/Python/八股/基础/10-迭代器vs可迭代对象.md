...
A：可迭代对象（Iterable）有 __iter__，迭代器（Iterator）额外有 __next__，是能逐个吐数据的状态机。



**Q2：一、一句话区分 —— 怎么理解？**

A：Iterable 像"一叠牌"，自己不会发牌，只能给你一个发牌员；Iterator 像"发牌员"（Dealer），手上记着发到哪张，你每次喊 next() 它就发下一张，发完抛 StopIteration。Iterable 实现了 __iter__，Iterator 额外实现 __next__（迭代器协议，Iterator Protocol）。



**Q3：二、Duck Typing 判断标准 —— 怎么理解？**

A：判断只看"会不会这两招"：Iterable 必须有 __iter__()（返回迭代器），"我能给你一个发牌员"；Iterator 必须有 __iter__()（返回 self）+ __next__()，"我就是发牌员"。所以迭代器一定是可迭代对象（鸭子类型，Duck Typing），但 list 有 __iter__ 没 __next__，得用 iter(list) 才拿到迭代器。



**Q4：三、关系与执行流程 —— 怎么理解？**

A：for x in obj 底层先 it = iter(obj) 拿到迭代器，再不断 next(it)，捕获 StopIteration 后悄悄退出——像自动按铃的发牌机。任何能被 for 遍历的东西（生成器、range、文件）最终都靠 __next__ 一个一个给值，理解这点就通了。



**Q5：四、可运行代码示例 —— 怎么理解？**

A：CountDown 类 __iter__ 返回 self、__next__ 返回下一项并在到 0 时抛 StopIteration，所以它既是 Iterable 也是 Iterator。list 是 Iterable 但不是 Iterator——list 要保持可重复遍历，故意不做成"一次性"状态机，需要时 iter(list) 才生成新迭代器（惰性求值，Lazy Evaluation）。



**Q6：核心速记主线有哪些？**

- 可迭代对象 Iterable 有 __iter__，迭代器 Iterator 有 __iter__ + __next__

- 迭代器一定是 Iterable，但 list 不是迭代器

- for 底层 = iter() + next() + 捕获 StopIteration

- 迭代器一次性消费、list 可重复；生成器必是迭代器



**口诀**

A：Iterable 一叠牌，Iterator 发牌员；

前者 __iter__ 给，后者 __next__ 添；

迭代器一次性，StopIteration 断；
...
- 🔗 [[语言与框架/Python/八股/并发/12-yieldfrom委托生成器|yield from委托生成器]] — 委托给另一个迭代器
