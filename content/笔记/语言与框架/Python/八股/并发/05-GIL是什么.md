...
**Q1：一句话讲清「机制 ①：时间片到期——被动释放」到底是什么？**
A：**GIL（Global Interpreter Lock，全局解释器锁）** 是 CPython 解释器中的一把**全局互斥锁**，它保证**同一时刻只有一个线程在执行 Python 字节码**。
想象一间只有一个灶台的厨房（CPU），来了4个厨师（4个线程）。
GIL 就是厨房规定：**同一时刻只允许一个厨师用灶台**。其他厨师可以在旁边洗菜、等食材（IO等待），但不能同时炒菜。

**Q2：一、GIL 是什么？（必答基础题） —— 怎么理解？**
A：**GIL（Global Interpreter Lock，全局解释器锁）** 是 CPython 解释器中的一把**全局互斥锁**，它保证**同一时刻只有一个线程在执行 Python 字节码**。
想象一间只有一个灶台的厨房（CPU），来了4个厨师（4个线程）。
GIL 就是厨房规定：**同一时刻只允许一个厨师用灶台**。其他厨师可以在旁边洗菜、等食材（IO等待），但不能同时炒菜。

**Q3：核心速记主线有哪些？**
A：抓住这几根：一、GIL 是什么？（必答基础题）。
