...
A：PEP 703 让 CPython 可编译成无 GIL 的自由线程版，Python 3.13 起多线程能真并行跑多核。



**Q2：一、这是什么、怎么开启 —— 怎么理解？ —— 怎么理解？**

A：像厨房打通墙多灶台：GIL 是全局大锁，同一时刻只一个线程跑字节码；PEP 703 把它变可选项，3.13 起官方提供 free-threaded build，多线程真并行。开启：启动 python3.13 -X gil=0 或 PYTHON_GIL=0，或构建时 --disable-gil。英文：GIL / free-threaded build。



**Q3：二、底层原理：偏向引用计数 —— 怎么理解？ —— 怎么理解？**

A：像给每桌配私账：GIL 原是为保护全局引用计数；去掉后改"偏向引用计数"——计数拆本地+全局，单线程只改本地免锁，跨线程才合并。再加每对象锁护 dict/list/set、immortal 常量永生、QSBR 安全延迟释放。英文：biased reference counting / per-object lock / QSBR。



**Q4：三、代价与生态现状 —— 怎么理解？ —— 怎么理解？**

A：像拆墙有代价：单线程约 1%~8% 开销（pyperformance 实测），但 CPU 密集任务可线性加速。坑在生态：C 扩展需标记支持自由线程，未适配的导入时会自动重新启用 GIL 并警告。路线三阶段：实验→非默认受支持→未来默认。英文：single-thread overhead / linear speedup。



**Q5：四、一句话讲清与面试口径 —— 怎么理解？ —— 怎么理解？**

A：像别把实验当量产：核心是偏向引用计数+给内置类型加锁，用少量单线程开销换真并行。但 C 扩展生态还在适配，所以面试别张口"GIL 没了"，要说"3.13 起可选、生产仍需观望，短期用多进程或 asyncio"。英文：experimental / asyncio。



**Q6：核心速记主线有哪些？**

- 本质：PEP 703 让 CPython 可选去 GIL，3.13 起官方实验性 free-threaded build

- 开启：-X gil=0 / PYTHON_GIL=0 / --disable-gil 三选法

- 原理：偏向引用计数 + 每对象锁 + immortal + QSBR 四件套

- 代价：单线程 1%~8% 开销，CPU 密集线性加速

- 现状：C 扩展未适配会自动重开 GIL；生产短期仍用多进程/asyncio



**口诀**

A：GIL 锁住一灶台，703 打通满灶开；
