...
- **Q：哪招能「真并行」绕过 GIL？** A：只有 **multiprocessing**（每进程独立 GIL）和 **C 扩展释放 GIL**（NumPy/Pandas 底层）能真正用多核；asyncio 只是等待 IO 时让出控制权，**不并行**。
- **Q：CPU 密集 vs IO 密集 怎么选？** A：CPU 密集 → 多进程 / C 扩展；IO 密集 → asyncio（首选）或多线程。
- **Q：asyncio 绕过 GIL 了吗？** A：没有。单线程内靠 `await` 让出控制权给别的协程，不阻塞但不并行。
- **Q：CPython 3.13+ 之后呢？** A：可 `-X nogil` 实验性关闭 GIL。
- **口诀**：CPU 密集 → 多进程 / NumPy(C 释放 GIL)｜IO 密集 → asyncio(让出非并行) / 多线程。

```mermaid
flowchart TD
    A["任务类型?"] -->|CPU 密集| B{"能用 NumPy 等 C 库?"}
    B -->|能| C["C 扩展释放 GIL<br/>真并行 🚀"]
    B -->|不能| D["multiprocessing 多进程<br/>每进程独立 GIL"]
    A -->|IO 密集| E{"全异步生态?"}
    E -->|是| F["asyncio 协程<br/>单线程让出·非并行"]
    E -->|否| G["多线程<br/>~8MB/线程"]
