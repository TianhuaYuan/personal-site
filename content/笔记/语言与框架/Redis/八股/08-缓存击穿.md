...
## 第二章：缓存击穿——热点数据刚好过期的那一秒

### 2.1 场景还原：秒杀开始的瞬间发生了什么？
...
### 2.4 解法二：逻辑过期——"数据永远在 Redis 里，内容旧了偷偷换"
...
**Q1：一句话讲清「===== 系统启动时的预热——非常重要！=====」到底是什么？**
A：**一句话定义**：缓存击穿 = 一个热点 key 在过期瞬间，大量并发请求同时打到数据库。
**核心思路**：缓存未命中时，只有**第一个请求**去查数据库并写回缓存，其他请求**排队等着**（自旋重试查 Redis），等缓存写好了一起从 Redis 拿。
**一句话讲清**："互斥锁不是银弹。高并发下锁竞争本身也消耗 Redis 性能。

**Q2：第二章：缓存击穿——热点数据刚好过期的那一秒 —— 怎么理解？**
A：**一句话定义**：缓存击穿 = 一个热点 key 在过期瞬间，大量并发请求同时打到数据库。
**核心思路**：缓存未命中时，只有**第一个请求**去查数据库并写回缓存，其他请求**排队等着**（自旋重试查 Redis），等缓存写好了一起从 Redis 拿。
**一句话讲清**："互斥锁不是银弹。高并发下锁竞争本身也消耗 Redis 性能。更优雅的解法是逻辑过期——缓存物理上永不过期，用后台异步更新代替主动查库。

**Q3：核心速记主线有哪些？**
A：抓住这几根：第二章：缓存击穿——热点数据刚好过期的那一秒。
