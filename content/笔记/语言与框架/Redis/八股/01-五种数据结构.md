...
A：**Redis 不是普通键值库——它的 value 有 5 种类型：String（字符串）、List（列表）、Hash（哈希）、Set（集合）、ZSet（有序集合）。选对类型，一行命令顶十行代码。**
---
...
**Q3：一、String — 最基础，啥都能存 —— 怎么理解？**
A：最简单的键值对——key 对应一个字符串 value。
| 存什么 | 例子 | 最大限制 |
| -------- | ------ | --------- |
| 普通字符串 |  | 512MB |
| 数字 |  | Redis 自己处理加减 |
...
**Q4：二、List — 消息队列/最新消息 —— 怎么理解？**
A：有序可重复的字符串列表，底层是双向链表或压缩列表。
**quicklist**（3.2+）：压缩列表 + 双向链表的混合体，Redis 3.2 之后唯一实现
每个节点是一个 ziplist（压缩列表），节点之间双向指针连接
**省内存**：ziplist 存小数据时比普通链表省 ~90% 内存
| 场景 | 做法 |
...
**Q5：三、Hash — 存对象/用户资料 —— 怎么理解？**
A：键值对集合，适合存储对象（一个用户的所有字段）。
**ziplist**（小数据）：字段数 < 512 且每个字段长度 < 64 字节
**hashtable**（大数据）：超过阈值时升级
| 场景 | String | Hash |
| ------ | -------- | ------ |
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、一、String — 最基础，啥都能存、二、List — 消息队列/最新消息、三、Hash — 存对象/用户资料、四、Set — 标签/去重/共同好友、五、ZSet — 排行榜/延时队列/限流。
