...
A：**缓存空间有限，放不下所有数据，必须踢走一些。怎么踢？看策略——有的一视同仁按过期时间踢，有的按"多久没用了"踢（LRU），有的按"用得少"踢（LFU）。关键是选对策略，别把马上要用的数据踢了。**
---
...
**Q4：8 种淘汰策略一览 —— 怎么理解？**
A：Redis 提供了 8 种策略，分成三类：
| 类别 | 策略 | 行为 | 踢谁 |
| ------ | ------ | ------ | ------ |
| **不淘汰** |  | 满了直接报错，不写新数据 | - |
| **过期删除** |  | 挑过期时间最短的先踢 | 设置了 TTL 的 key |
| |  | 在设置了 TTL 的 key 里随机踢 | 设置了 TTL 的 key |

**Q5：必问：LRU vs LFU —— 怎么理解？**
A：这两个是最核心的策略，必须区分清楚。
**核心思想：踢走最长时间没被访问的那个。**
冰箱里有一堆菜：
鸡蛋：每天早上吃 → 最近用过
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、🌰 理解为什么需要淘汰策略、8 种淘汰策略一览、必问：LRU vs LFU、其他策略说明、实际项目怎么选？。
