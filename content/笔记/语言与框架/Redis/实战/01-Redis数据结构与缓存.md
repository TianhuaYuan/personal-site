...
> 从"为什么需要它"开始讲，配合通俗比喻和完整可运行命令。所有 Redis 命令都可以在 `redis-cli` 中直接敲。

> **本文定位**：本篇是 [[笔记/语言与框架/Redis/八股/01-五种数据结构|Redis 五种数据结构（理论篇）]] 的**实战落地** companion——聚焦五种结构的命令实操、缓存三大坑（穿透/击穿/雪崩）的代码防护与常见踩坑；概念原理以理论篇为准。
...
> 一个很小的数据结构，能告诉你"这个 key 肯定不存在"或"可能存在"。就像图书馆门口的检索电脑——查不到的书名肯定没有，查到了再进库（Redis 缓存/MySQL）。Redis 4.0+ 有内置的布隆过滤器模块 `RedisBloom`。
...
**Q1：一句话讲清「Redis 数据结构与缓存」到底是什么？**

A：这篇讲 Redis 为什么快、5 种数据结构怎么用，以及缓存三大坑（穿透/击穿/雪崩）怎么防。

**Q2：一、Redis 是什么（为什么快）—— 怎么理解？ —— 怎么理解？**

A：MySQL 是仓库（空间大、走磁盘慢），Redis 是工作台工具箱（纯内存、伸手就拿）。快的四点：纯内存、单线程无锁竞争、IO 多路复用、简单 key-value。量级上 Redis QPS 10 万+，比 MySQL 快两个数量级——瓶颈不在 CPU 在带宽。

**Q3：二、五种数据结构与命令 —— 怎么理解？ —— 怎么理解？**

A：String 计数/分布式锁（INCR/SETNX）；Hash 按字段读写对象；List 双端做队列（BLPOP 阻塞）；Set 交并差做共同好友；ZSet 跳表做排行榜。选型口诀：缓存 JSON 用 String、按字段用 Hash、队列用 List、去重交集用 Set、排行榜用 ZSet。

**Q4：三、缓存穿透 / 击穿 / 雪崩 —— 怎么理解？ —— 怎么理解？**

A：穿透=查不存在的数据，用布隆过滤器或缓存空值；击穿=热点 key 过期瞬间请求涌向 DB，用互斥锁或逻辑过期（物理不设 TTL，value 里写过期时间，过期先返旧数据再异步更新）；雪崩=大量 key 同时过期，用随机过期时间+多级缓存。

**Q5：四、常见踩坑 —— 怎么理解？ —— 怎么理解？**

A：①生产禁用 KEYS *（O(n) 阻塞，用 SCAN）；②key 用冒号分层命名；③永远设过期时间（否则 OOM）；④避免 Big Key（拆小 key）；⑤热 key 做本地缓存或拆分；⑥删大 key 用 UNLINK 别 DEL；⑦别用 List 当正经消息队列（没 ACK）。

**Q6：核心速记主线有哪些？**

- Redis 快：纯内存+单线程+IO 多路复用+简单结构

- 五种结构：String/List/Hash/Set/ZSet 各司其职

- 缓存三兄弟：穿透/击穿/雪崩 及各自解法

- 7 个踩坑：KEYS 禁用、大 key 拆、UNLINK 删等
...
- 对应理论（概念详解）：[[笔记/语言与框架/Redis/八股/01-五种数据结构|Redis 五种数据结构（理论篇）]]
