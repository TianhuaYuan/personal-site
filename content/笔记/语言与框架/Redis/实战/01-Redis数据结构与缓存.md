...
# 6. 查看用户 1001 所有信息
HGETALL user:1001
# 期望: name=张三, age=26, email=zhangsan@qq.com, city=深圳
...
**Q3：Redis 是什么 —— 怎么理解？**
A：你在电商网站搜"笔记本电脑"，页面要显示商品列表。如果每次搜索都去查 MySQL：
MySQL 是仓库（空间大、拿东西慢），Redis 是工作台上的工具箱（空间小、伸手就拿）。常用的螺丝刀放工具箱，偶尔用的大电钻才去仓库取。

**Q4：安装运行 —— 怎么理解？**
A：Redis 官方不直接支持 Windows。以下三种方式任选一种。
下载地址：https://www.memurai.com/ → 安装后自动启动，直接  连接。
---
...
| key 区分大小写 |  ≠  | |
| 字符串值带引号时含空格 |  | 双引号保护空格 |
| 默认操作 0 号数据库 | Redis 有 0~15 共 16 个库 |  切换 |
|  退出 | 或直接关终端 | |
| 命令 | 作用 | 示例 |
|:---|:---|:---|

**Q6：核心速记主线有哪些？**
A：抓住这几根：目录、Redis 是什么、安装运行、前置知识：命令行基础、一、String — 字符串、二、Hash — 哈希。
