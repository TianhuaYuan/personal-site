...
A：**缓存穿透**：查询一个一定不存在的数据，由于缓存不命中，每次都要去数据库查询，但数据库也没有这条数据，所以也不会写入缓存。这导致每次请求都打到数据库。
**常见场景**：
恶意攻击：故意查询不存在的数据
业务Bug：查询错误的数据ID
数据未初始化：新数据还没写入数据库
...
**Q3：2. 缓存击穿 —— 怎么理解？**
A：**缓存击穿**：某个热点key在某个时间点过期，恰好在这个时间点有大量并发请求访问这个key，这些请求都打到数据库。
**常见场景**：
热点数据过期：如秒杀商品、热门新闻
突发流量：如明星微博、热点事件
**优点**：保证数据一致性
...
**Q4：3. 缓存雪崩 —— 怎么理解？**
A：**缓存雪崩**：大量key在同一时间过期，或者Redis服务宕机，导致大量请求打到数据库。
**常见场景**：
批量设置相同过期时间
Redis服务宕机
流量突增
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：1. 缓存穿透、2. 缓存击穿、3. 缓存雪崩、4. 对比总结、核心要点。
