...
A：**SELECT 和 UPDATE 走的路径不同——SELECT 到存储引擎拿数据就返回，UPDATE 要额外写日志（redo/undo/binlog）。但前两步（连接层→Server 层）完全一样。**
---
...
**Q3：🌰 整体路线 —— 怎么理解？**
A：一条 SQL 从客户端到磁盘，经过四层：
---

**Q4：一条 SELECT 语句的一生 —— 怎么理解？**
A：**原因：缓存失效太频繁。只要表有更新，整张表的查询缓存全清掉。命中率极低，维护成本高。**
**优化器可能选错索引。**
统计信息不准时，优化器可能选了全表扫描而不是索引。这时可以用  或  强制指定。
---

**Q5：一条 UPDATE 语句的一生 —— 怎么理解？**
A：前 5 步（连接→分析→预处理→优化→执行）和 SELECT 完全一样。
不同从**执行器**开始：
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、🌰 整体路线、一条 SELECT 语句的一生、一条 UPDATE 语句的一生、SELECT vs UPDATE 对比、一句话讲清。
...
- 📋 目录：[[00-MySQL]]
- 📚 学习清单：[[八股文学习清单]]
- 🔗 [[计算机基础/操作系统/13-IO模型四种|IO模型]] — SQL执行涉及磁盘IO和网络IO
- 🔗 [[语言与框架/Redis/八股/10-RDB与AOF与混合持久化|Redis持久化]] — MySQL与Redis的持久化机制对比
