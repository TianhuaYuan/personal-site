...
**Q1：EXPLAIN 看哪两列最重要？**
A：type 列（访问方式，决定快不快）和 Extra 列（找的时候还干了啥）。优化目标把 type 往左推（接近 const），躲开 ALL（全表扫描）。

**Q2：type 从好到坏怎么排？**
A：system > const > eq_ref > ref > range > index > ALL。const=主键/唯一索引查一行；ref=普通索引等值；range=范围（IN/BETWEEN/>）；index=扫整棵索引树（假积极）；ALL=全表扫描死刑。

**Q3：Extra 里哪些信号要警惕？**
A：Using index（覆盖索引，好事）；Using filesort / Using temporary（排序/临时表，说明 ORDER BY/GROUP BY 没走索引）；filesort + temporary 同现=必改。

**Q4：key 为 NULL 说明什么？**
A：一个索引都没用上，回去看 WHERE 条件列有没有索引或索引是否失效。

> 🔑 口诀：EXPLAIN 看 type + Extra；type 往左推躲 ALL；覆盖索引 Using index 最香，filesort+temporary 同现必改。
