...
**Q1：一句话讲清「索引失效 5 大场景」到底是什么？**

A：索引失效指 MySQL 宁可全表扫描也不用索引，根因是索引列被函数、计算或隐式转换"穿了衣服"。

**Q2：🌰 理解思路 —— 怎么理解？**

A：建索引像给书按拼音做目录（B+ 树），你非要按笔画查，目录就帮不上忙。B+ 树存的是索引列原始值，你对它套函数/做计算/和数字比（触发 CAST），MySQL 拿到"改过的值"对不上，只能翻书（全表扫描）。铁律：索引列保持原样比。

**Q3：五大场景（背这一张表就够了） —— 怎么理解？**

A：五大致命伤：① 隐式类型转换（字符串列=数字被 CAST 套列）；② 索引列参与计算（salary*12，计算放右边）；③ 索引列用函数（LEFT(name,1)，改前缀匹配）；④ LIKE 以 % 开头（不知开头没法二分）；⑤ OR 连非索引条件（一边没索引两边全废）。

**Q4：逐条拆解 —— 怎么理解？**

A：逐条避坑：mobile VARCHAR 写 =123 失效、写 '123' 走索引；salary*12>100000 失效、salary>100000/12 有效；LEFT(name,1)='张' 失效、name LIKE '张%' 有效；LIKE '%华' 失效、'张%' 有效；OR 非索引用 UNION 或给另一侧建索引。

**Q5：加分：为什么"联合索引"容易踩坑？ —— 怎么理解？**

A：联合索引 (a,b,c) 守"最左前缀"：WHERE a=1 用到，WHERE a=1 AND b=2 用到，跳 b 只用 a；WHERE b=2 从最左就不在——完全用不了。它不是"失效"，是"只能用左边部分"，记住第一列必须出现。

**Q6：核心速记主线有哪些？**

- 核心规律：索引列别套函数/计算/隐式转换

- 五大场景：隐式转换、计算、函数、左模糊、OR 非索引
...
LIKE 百分号在左就完蛋，OR 一边无索引也崩；

验证靠 EXPLAIN，key 为 NULL 全表扫。
