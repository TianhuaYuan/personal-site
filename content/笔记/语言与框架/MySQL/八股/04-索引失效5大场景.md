...
A：**索引不是建了就有用的——用不对姿势，MySQL 宁可全表扫描也不用你的索引。最核心的规律：让索引列独自参与比较，别给它"穿衣服"（函数、计算、隐式转换）。**
---
...
**Q3：🌰 理解思路 —— 怎么理解？**
A：建索引就像给书做目录。目录有用，前提是你在**按目录的结构**查。
目录按拼音排的（B+ 树），你非要按笔画查——目录帮不上忙，只能一页页翻。
**索引失效 = MySQL 说："这目录我用不了，我直接翻书吧。"**
---

**Q4：五大场景（背这一张表就够了） —— 怎么理解？**
A：| # | 场景 | 典型写法 | 为什么失效 | 怎么修 |
| --- | ------ | --------- | ----------- | -------- |
| 1 | **隐式类型转换** |  | 字符串列跟数字比，MySQL 偷偷加了 CAST | 类型保持一致 |
| 2 | **索引列参与计算** |  | 索引存的是 salary，不是 salary*12 | 计算放右边 |

**Q5：逐条拆解 —— 怎么理解？**
A：**规律：字符串列跟数字比，MySQL 把字符串转数字（CAST 套在索引列上）。反之数字列跟字符串比，MySQL 把字符串转数字（CAST 套在值上，不影响索引）。**
**规律：索引列别动它，要算放右边。**
**规律：索引列前面别套函数。

**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、🌰 理解思路、五大场景（背这一张表就够了）、逐条拆解、加分：为什么"联合索引"容易踩坑？、快速记忆。
