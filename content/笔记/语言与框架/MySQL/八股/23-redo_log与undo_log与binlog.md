...
A：**redo log 保"持久性"（崩溃后数据不丢），undo log 保"原子性"（失败能回滚到之前的样子），binlog 保"备份和同步"（主从复制和恢复）。三个日志分工不同，缺了谁 MySQL 都可能丢数据或崩了修不回来。**
---
...
A：**物理日志**，记录的是"对哪个数据页的哪个偏移量做了什么修改"。
MySQL 不会直接改磁盘上的数据（太慢了）。流程是：
**如果第 5 步之后、第 6 步之前停电了：**
内存里的数据没了（balance=1000 丢了）
但 redo log 在磁盘上
...
A：**逻辑日志**，记录的是"修改之前的数据是什么"，用于回滚和 MVCC。
**作用 1：事务回滚**
**没有 undo log，ROLLBACK 就不知道改回什么。**
**作用 2：MVCC 多版本并发控制**
事务 B 需要看到旧值（500），新值（1000）还没提交。
**每个事务通过 undo log 构建自己"可见"的数据版本。

**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、🌰 三兄弟各司其职、一、redo log——"我干了什么"、二、undo log——"我准备改之前是什么样"、三、binlog——"整个数据库发生了什么"、三张日志的执行流程（一条 UPDATE 语句）。
