...
**Q1：三种子查询类型及执行次数？**
A：标量子查询（SELECT 里）外层 N 行执行 N 次；派生表（FROM 里）执行 1 次；相关子查询（WHERE 引外层）外层 N 行执行 N 次；非相关子查询执行 1 次。

**Q2：为什么标量子查询不推荐？**
A：外层每行触发一次子查询（DEPENDENT SUBQUERY），10 万行=10 万次；改成 JOIN 一次搞定。

**Q3：NOT IN 遇 NULL 为什么返回 0 行？**
A：三值逻辑——NULL 比较结果为 UNKNOWN，WHERE 中 UNKNOWN 当 FALSE 处理，所以子查询只要有一个 NULL，整条 SQL 返回空。这是 SQL 标准行为。

**Q4：IN vs EXISTS 怎么选？**
A：子查询小且确定无 NULL→IN 快；子查询大或可能含 NULL→EXISTS（找到即停，NULL 安全）。

**Q5：怎么查"没下过单的客户"？**
A：用 NOT EXISTS 或 LEFT JOIN ... IS NULL，别用 NOT IN。

> 🔑 口诀：标量/相关子查询每行跑一次，改 JOIN；NOT IN 遇 NULL 全军覆没，用 NOT EXISTS；IN 小快、EXISTS 大稳。
