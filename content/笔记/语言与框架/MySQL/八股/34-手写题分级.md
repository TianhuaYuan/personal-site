...
A：**窗口函数 = 不减少行数的"分组计算"。普通聚合 GROUP BY 把 10 行压成 1 行；窗口函数 10 行进来 10 行出去，计算结果作为新列附加。**
**窗口函数语法结构：**
**一句话讲清**："ROW_NUMBER 给每行唯一编号，即使排序字段相同；RANK 处理并列但会跳过名次（1,1,3）；DENSE_RANK 处理并列不跳名次（1,1,2）。

**Q2：第三章：窗口函数 — 区分度的核心 —— 怎么理解？**
A：**窗口函数 = 不减少行数的"分组计算"。普通聚合 GROUP BY 把 10 行压成 1 行；窗口函数 10 行进来 10 行出去，计算结果作为新列附加。**
**窗口函数语法结构：**
**一句话讲清**："ROW_NUMBER 给每行唯一编号，即使排序字段相同；RANK 处理并列但会跳过名次（1,1,3）；DENSE_RANK 处理并列不跳名次（1,1,2）。

**Q3：核心速记主线有哪些？**
A：抓住这几根：第三章：窗口函数 — 区分度的核心。
