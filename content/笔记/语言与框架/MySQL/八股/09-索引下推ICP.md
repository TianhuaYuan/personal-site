...
**Q1：索引下推 ICP 是什么，解决了什么？**  
A：MySQL 5.6+ 的优化。没有 ICP 时，二级索引找到 id 后全部回表，再到 server 层过滤其他条件；有 ICP 时，把能用索引判断的条件（如联合索引里的 age）"下推"到存储引擎层，在索引遍历时就过滤掉，减少回表次数。核心是"过滤从 server 层下推到存储引擎层"。

**Q2：ICP 和覆盖索引有什么区别？**  
A：覆盖索引直接不回表（Extra: `Using index`）；ICP 仍然回表，但只回表满足条件的行（Extra: `Using index condition`）。覆盖索引省掉回表，ICP 减少回表次数，两者叠加效果最好。

**Q3：什么条件下 ICP 才有收益？**  
A：① 联合索引（单列无意义）；② 索引能定位到范围（`LIKE '张%'`、`>`、`<`、`BETWEEN`）；③ 条件涉及索引里的额外列；④ InnoDB / MyISAM 都支持。主键索引、条件列不在索引里时不生效。

**Q4：怎么从 EXPLAIN 看 ICP 生效了？**  
A：Extra 列显示 `Using index condition`。范围匹配越宽泛、后续条件过滤性越强，ICP 省得越多（极端可省 90% 回表）。

> 🔑 口诀：ICP = 回表前先过滤，少做无用功；Joint 索引范围查，后续条件下推用。
