...
flowchart TD
    ...
    A["WHERE name LIKE '张%' AND age>25"] --> B{有 ICP?}

    B -->|否| C[拿所有'张%'的id 回表100次]

    C --> D[server层过滤age 丢70行 白回表]

    B -->|是| E[遍历索引时顺手查 age>25]

    E --> F[只回表满足两条件的30次]

    F --> G[少70次IO ✅]

    H["覆盖索引: 列全在索引"] --> I[0次回表 最快]
...
**Q1：一句话讲清「索引下推 ICP（MySQL 5.6+）」到底是什么？**

A：索引下推 ICP 把 WHERE 里能用索引判断的条件，从 server 层下推到存储引擎，回表前先过滤少做无用功。

**Q2：一、没有 ICP 的世界 —— 怎么理解？ —— 怎么理解？**

A：没 ICP 时，二级索引找到匹配的 id 就全部回表（查整行），到 server 层才过滤 age>25，结果 100 次回表 70 次白干。就像快递员把整栋楼的包裹都搬下楼，到了门口才看收件人名字——搬错了 70 个。

**Q3：二、有 ICP 的世界 —— 怎么理解？ —— 怎么理解？**

A：有 ICP，遍历索引时顺手检查 age>25，只回表满足两个条件的行，回表从 100 次降到 30 次。等于快递员在货架上就瞄一眼名字，只搬该搬的。核心：过滤下推（Index Condition Pushdown）。

**Q4：三、ICP vs 覆盖索引 —— 怎么理解？ —— 怎么理解？**

A：覆盖索引直接不回表（Extra: Using index），ICP 仍回表但只回表需要的行（Extra: Using index condition）。覆盖索引省掉回表，ICP 减少回表次数，两者叠加最香。看生效：EXPLAIN 的 Extra 列出现 Using index condition。

**Q5：四、ICP 适用条件与收益 —— 怎么理解？ —— 怎么理解？**

A：生效需满足：联合索引（单列无意义）、能定位范围（LIKE/BETWEEN）、条件涉及索引额外列、InnoDB/MyISAM。主键索引、条件列不在索引里不生效。范围越宽、后续过滤越强，省得越多（极端省 90% 回表）。

**Q6：核心速记主线有哪些？**

- 没有 ICP：二级索引全回表，server 层才过滤

- 有 ICP：回表前在索引层先过滤，少回表

- 覆盖索引不回表，ICP 少回表，二者叠加最佳

- 看 EXPLAIN Extra: Using index condition 即生效
