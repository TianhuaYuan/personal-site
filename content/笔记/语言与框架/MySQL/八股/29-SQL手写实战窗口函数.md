...
**Q1：LEFT JOIN 后 WHERE 过滤右表会怎样？**
A：LEFT JOIN 退化为 INNER JOIN——右表为 NULL 的行被 WHERE 过滤掉。过滤条件应写进 ON 子句。

**Q2：NOT IN 遇 NULL 会怎样？**
A：返回 0 行！因为 NULL 参与比较得 UNKNOWN，WHERE 中被视为 FALSE。生产永远用 NOT EXISTS。

**Q3：窗口函数和 GROUP BY 区别？**
A：GROUP BY 合并行（减少行数，10 行压成 1 行）；窗口函数不减少行数（10 进 10 出），结果作为新列附加。

**Q4：ROW_NUMBER / RANK / DENSE_RANK 区别？**
A：ROW_NUMBER 唯一序号无并列；RANK 并列跳号（1,1,3）；DENSE_RANK 并列不跳（1,1,2）。

**Q5：窗口函数能放 WHERE 里吗？**
A：不能。窗口函数在 WHERE 之后执行，必须套子查询在外层 WHERE 过滤（如 WHERE rn <= 3）。

**Q6：连续登录怎么算？**
A：连续日期 − 它在组内的 ROW_NUMBER = 同一个常数。去重→编号→DATE_SUB→GROUP BY 该常数→HAVING COUNT >= N。

> 🔑 口诀：LEFT JOIN 过滤右表写 ON 别写 WHERE；NOT IN 遇 NULL 全军覆没用 NOT EXISTS；窗口函数不减量、排名三剑客记跳号；放 WHERE 报错就套子查询。
