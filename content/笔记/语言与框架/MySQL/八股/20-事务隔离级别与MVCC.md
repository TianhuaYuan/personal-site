...
flowchart TD
    A[并发事务] --> B{隔离?}
    B -->|不隔离| C[脏读/不可重复读/幻读]
    B -->|隔离 RR 默认| E[REPEATABLE READ 默认]
    E --> F{读的方式}
    F -->|快照读 SELECT| G[MVCC: 一个 Read View 用到底<br/>防脏读+不可重复读+快照幻读<br/>读不加锁, 读写不阻塞]
    F -->|当前读 UPDATE/DELETE| H[加锁: 行锁+间隙锁<br/>Next-Key Lock 防当前读幻读]
    G --> I[解决 读写冲突]
    H --> J[解决 写写冲突]
```
...
**Q1：ACID 各自靠什么实现？**
A：原子性=undo log（回滚旧值）；一致性=A+I+D 共同保证（是 boss）；隔离性=MVCC+锁；持久性=redo log（崩溃恢复）。

**Q2：RC 和 RR 的根本区别在哪？**
A：Read View 的创建时机。RC 每次 SELECT 都新建快照 → 可能看到不同结果；RR 事务开始建一次、用到底 → 整个事务看到的数据一致（防不可重复读）。

**Q3：MVCC 的可见性判断怎么走？**
A：按 DB_TRX_ID 判断：自己的可见；trx_id < min_trx_id（早提交了）可见；trx_id >= max_trx_id（还没出生）不可见，往前找旧版本；在 [min,max) 区间内，在 m_ids（还活跃没提交）不可见、不在则可见。

**Q4：RR 能完全防幻读吗？**
A：快照读能（MVCC 读事务开始版本）；当前读（UPDATE/DELETE/SELECT FOR UPDATE）不能，需靠间隙锁/Next-Key Lock 防。

**Q5：InnoDB 行锁锁的是什么？**
A：锁的是索引。WHERE 没走索引时，行锁退化成表锁，全表被锁。

> 🔑 口诀：原子 undo、持久 redo、隔离 MVCC+锁；一致性是 boss。RC 每次新快照、RR 一个用到底。行锁锁索引，没索引变表锁。
...
- 📋 目录：[[00-MySQL]]
- 📚 学习清单：[[八股文学习清单]]
- 🔗 [[计算机基础/操作系统/03-线程同步锁机制|线程同步锁机制]] — MVCC是MySQL层面的并发控制，操作系统层面用锁机制
- 🔗 [[计算机基础/操作系统/01-进程vs线程vs协程对比|进程vs线程vs协程]] — 并发的多个层次：OS级、数据库级、应用级
- 🔗 [[语言与框架/MySQL/八股/21-锁机制行锁表锁间隙锁|锁机制行锁表锁间隙锁]] — MVCC解决读写冲突，锁机制解决写写冲突
