...
# 第四步：取结果
row = cursor.fetchone()
# row 是一个元组：(1, 'zhangsan', 'zhang@qq.com')
...
```python
# ForeignKey 做的事：保证 orders.user_id 的值在 users.id 里一定存在
#   → 如果 users 里没有 id=999 的用户，你就不能插入 user_id=999 的订单
...
# SessionLocal 是一个"工厂"——每次调用 SessionLocal() 就生产一个新 Session
...
A：| 前几天学的 | 今天怎么用 |
|-----------|-----------|
|  + 数据类型 / 约束 | SQLAlchemy 用 Python 类定义表结构，自动生成  |
|  |  → ORM 帮你生成 INSERT |
|  |  → ORM 帮你生成 SELECT + WHERE |
|  | 先查出来改属性， → ORM 帮你生成 UPDATE |
...
**返回值是元组**——、 完全靠记忆，加一列全崩
**SQL 拼接容易出错**——少一个引号、少一个逗号，运行时才炸
**不同数据库方言不同**——MySQL 用 ，SQLite 用 ，PostgreSQL 又不一样
**连接管理繁琐**——每次都要  / ，忘了关就泄漏
**ORM（Object-Relational Mapping，对象-关系映射）**：你说 Python（对象、属性），它帮你翻译成 SQL 交给数据库；

**Q4：二、环境搭建 —— 怎么理解？**
A：| 包 | 作用 | 一句话理解 |
|----|------|-----------|
|  | Web 框架，写 API 接口 | 你的"厨房"，处理点单和做菜 |
|  | ASGI 服务器，运行 FastAPI | 你的"服务员"，把顾客请求端进厨房 |
|  | ORM，Python 对象 ↔ 数据库表 | 你的"翻译器" |
|  | Python 连 MySQL 的驱动 | 你的"通往仓库的路" |

**Q5：三、数据库连接与引擎 —— 怎么理解？**
A：连接字符串就是告诉 SQLAlchemy "去哪里找数据库"的地址：
逐段拆解：
—— 数据库类型（MySQL）
—— 用哪个驱动连
—— 固定分隔符
...
—— 连哪个库
[!CAUTION] 密码里有特殊字符（如 、）会出错！
密码  中的  会被误认为主机地址开始。解决：用  编码。
**引擎 = 数据库的"前台总机"。

**Q6：核心速记主线有哪些？**
A：抓住这几根：🔁 前置复习、一、为什么要用 ORM？、二、环境搭建、三、数据库连接与引擎、四、定义 ORM 模型、五、Session：与数据库对话的"窗口"。
