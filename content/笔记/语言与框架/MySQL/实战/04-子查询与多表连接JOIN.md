...
A：需求："列出所有订单，显示用户名 + 商品 + 金额。"
订单表里有 ，用户表里有 ——你需要把两张表"拼"在一起。
**这就是 JOIN：通过关联字段把多张表的行连成一行。**
**集合视角：**
**INNER JOIN 结果：只保留两表都有匹配的行，id=7 的 wujiu 被排除。**
**输出：**
...
A：需求："列出所有用户和他们的订单情况。没有订单的用户也要显示出来。"
如果用 INNER JOIN，wujiu（id=7，0 笔订单）就会消失。
**LEFT JOIN：保留左表的所有行，右表匹配不上的填 NULL。**
**输出：**
**wujiu（id=7）没有订单，order_id、product、total 全是 NULL，但他没有被丢掉。**
...
A：**RIGHT JOIN = 保留右表所有行，左表匹配不上的填 NULL。**
**LEFT JOIN 就能覆盖所有场景**——把表顺序换一下就等价
**可读性差**——人们习惯"主表在左，附属表在右"
**多表 JOIN 时从左往右读很自然**：A LEFT JOIN B LEFT JOIN C
**结论：学会 RIGHT JOIN 的语法看懂别人代码即可，自己写统一用 LEFT JOIN。**
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、为什么需要多表查询？、二、内连接（INNER JOIN）、三、左连接（LEFT JOIN）、四、右连接（RIGHT JOIN）、五、JOIN 对比总结、六、子查询。
