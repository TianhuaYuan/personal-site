...
A：Pydantic 用类型注解自动校验进出数据，v2 换 Rust 核心快了一个数量级。

**Q2：Pydantic 像什么、干哪三件事 —— 怎么理解？**

A：请求体像入境旅客，Pydantic 是海关：你贴出规则（字段+类型+约束），旅客一来逐个核对，不合格当场遣返还告诉你哪本护照、哪个字段、为什么。核心三件事：校验（Validation）、精确报错、序列化（Serialization）（Pydantic / ValidationError）。

**Q3：v1 到 v2 到底改了啥 —— 怎么理解？**

A：核心是校验引擎换成 Rust（pydantic-core），快数倍。配套现代化：class Config 变 model_config；@validator 升 @field_validator；@root_validator 变 @model_validator；orm_mode 变 from_attributes；min_items/regex 改名 min_length/pattern（Rust core / model_config）。

**Q4：v2 亮点 API 怎么用 —— 怎么理解？**

A：@field_validator 装饰类方法校验单字段（如首字母大写）；model_config+TypeAdapter 让任意类型也能校验、无需 BaseModel；@model_validator(mode="after") 做跨字段校验如 start<=end。声明式校验从单字段覆盖到裸类型（TypeAdapter / model_validator）。

**Q5：比手写 if 强在哪 —— 怎么理解？**

A：五点：声明式代码短、错误精确到字段、自动出 JSON Schema 驱动 OpenAPI、序列化反序列化一体、v2 Rust 性能高。敏感字段用 exclude 或 response_model 只暴露该暴露的，别把密码漏进响应（JSON Schema / OpenAPI）。
...
- API 改名：model_config / @field_validator / @model_validator / from_attributes

- 任意类型：TypeAdapter 校验裸类型，无需 BaseModel

**口诀**

A：声明模型定规则，BaseModel 来塑模；

进来校验出去序列化，字段报错精确到；

v2 换上 Rust 引擎，model_config 替旧套；

field_validator 守单字，TypeAdapter 任意校验好。
