...
**Q1：一句话讲清「SQLAlchemy 2.0 异步集成」到底是什么？**

A：SQLAlchemy 2.0 异步集成用 create_async_engine 与 AsyncSession 把 ORM 操作改成 await 异步风格。

**Q2：一句话总结 —— 怎么理解？**

A：异步三件套像给借书卡升级：create_engine 换 create_async_engine、Session 换 AsyncSession、查询用 select() 还要 await（AsyncSession 异步会话）。

**Q3：生活类比：图书馆借阅 —— 怎么理解？**

A：AsyncSession 像图书馆借书卡：借卡进馆查资料，看完还卡连接回池；await 时馆员去服务别人不干站（connection pool 连接池）。

**Q4：三件套 —— 怎么理解？**

A：三件套像异步咖啡机三零件：异步引擎 create_async_engine 管连接、async_sessionmaker 造 AsyncSession、查询用 select() 配 await（async_sessionmaker 会话工厂）。

**Q5：FastAPI 集成完整流程 —— 怎么理解？**

A：Depends+yield 像给每个请求发一张借书卡：路由借到卡查库，yield 结束 async with 自动还卡，连接回池（dependency injection 依赖注入）。

**Q6：核心速记主线有哪些？**

- 三件套：create_async_engine / AsyncSession / select()+await

- 驱动加 +asyncpg（PG）或 +aiomysql（MySQL），同步驱动用不了

- FastAPI 用 Depends+yield 管"每请求一会话"，async with 用完即关

- 四大坑：别混 Session、expire_on_commit=False、用 selectinload 替 lazy、始终 async with 收尾

**口诀**

A：SQLAlchemy 异步三件，引擎加 async Session 变 Async；

查询 select 加 await，add scalar 不用等；

expire_on_commit 设 False，lazy 换 eager 不崩；

async with 管生命周期，连接回池不泄露。
