...
A：**中间件是“每个请求进路由前、出路由后都会经过的一道门”：统一加日志、计时、CORS、压缩、HTTPS 重定向。它像洋葱——请求从外往里穿，响应从里往外穿；后注册的中间件包在最外层。**
---
...
**Q4：中间件长什么样 —— 怎么理解？**
A：：进来的请求
：一个函数，把请求传下去，返回路由生成的响应
在  **前**改请求，在 **后**改响应
自定义专有响应头可用  前缀；要让浏览器 JS 能读到，得在 CORS 的  里声明。

**Q5：执行顺序：洋葱模型（必考） —— 怎么理解？**
A：代码顺序： 先， 后 →
| 阶段 | 执行顺序 |
| :--- | :--- |
| 请求 | B 前 → A 前 → 路由 |
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、生活类比：小区门禁 + 快递柜、中间件长什么样、执行顺序：洋葱模型（必考）、内置中间件（都来自 Starlette）、中间件 vs 依赖注入：何时用谁？。
