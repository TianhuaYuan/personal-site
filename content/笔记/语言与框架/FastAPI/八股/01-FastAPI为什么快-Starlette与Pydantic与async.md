...
A：**FastAPI 快，不是它自己写得快，而是它站在三个巨人的肩膀上：Starlette（ASGI 异步内核）+ Pydantic v2（Rust 写的校验核心）+ Python 类型注解（自动文档与校验）。但“快”有个前提——你别在 async 里干阻塞的事，否则事件循环一冻，全员陪跑。**
---
...
A：把 Web 框架想象成一家**餐厅**：
**WSGI 老餐厅（Flask/Django 同步）**：一个服务员（线程）只服务一桌，点完单就站在厨房门口干等菜好，期间啥也不干。来 100 桌就得雇 100 个服务员，成本高、并发低。
**ASGI 新式餐厅（FastAPI）**：服务员点完单把单子递进去，转身去服务别的桌；等菜好了厨房喊一声，再回来端走。一个服务员能同时照看几十桌的“等待期”。

**Q4：技术栈：FastAPI 不是从零造的 —— 怎么理解？**
A：**ASGI**（Asynchronous Server Gateway Interface，异步服务器网关接口）是 WSGI 的继任者，允许一个进程内用单线程事件循环并发处理多个请求；**WSGI**（Web Server Gateway Interface）是老一代同步接口，一个请求占一个线程。

**Q5：三大支柱逐个拆 —— 怎么理解？**
A：FastAPI 本身几乎不碰 HTTP 细节，全部交给 Starlette——一个轻量 ASGI 工具箱。每个  路由在事件循环里跑：遇到  或  就**主动让出控制权**，事件循环转去处理别的请求。于是“等待数据库/外部 API”的空闲时间被压缩成了并发能力。
请求体进来要先校验。

**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、先讲个生活类比、技术栈：FastAPI 不是从零造的、三大支柱逐个拆、同步 vs 异步：一张图看懂“为什么快”、基准对比（行业通用压测，单进程）。
