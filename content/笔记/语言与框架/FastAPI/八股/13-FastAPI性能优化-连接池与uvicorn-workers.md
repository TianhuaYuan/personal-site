...
A：**连接池 = 数据库连接不每次新建而是反复用（省建连开销）；uvicorn workers = 开多个进程同时吃满多核（绕过 GIL 的 CPU 限制）。两件事搞对，QPS 从几百到几千。但别忘了：数据库才是总瓶颈，总连接数 = workers × pool_size，别把数据库压爆。**
---
...
**Q3：一、连接池（Connection Pool） —— 怎么理解？**
A：每次新建数据库连接都要 TCP 握手 + 认证，开销大。连接池**预先建好一批连接反复复用**。
| 参数 | 含义 | 建议 |
| :--- | :--- | :--- |
|  | 平时保持的常驻连接数 | 核数×2 左右 |
|  | 高峰最多额外建几个 | 如 10 |
|  | 拿连接前先探活，防拿到断连 |  |
|  | 连接超过 N 秒强制回收重建 | 如 3600 |
池太小 → 请求等连接（瓶颈在池）；

**Q4：二、uvicorn workers（多进程吃满多核） —— 怎么理解？**
A：FastAPI 是单进程单事件循环，受 GIL 限制只能用一个核跑 Python。开多个 worker 进程才能用满多核：
| 模式 | 命令 | 行为 |
| :--- | :--- | :--- |
| 单进程 |  | 1 进程 1 核 |
| 多进程 |  | 4 进程 4 核 |
**经验值：**，不是越多越好（进程多→上下文切换+内存涨）。
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、一、连接池（Connection Pool）、二、uvicorn workers（多进程吃满多核）、三、更多优化清单、四、最重要的纪律：先测量再优化、原理：为什么需要多 workers。
