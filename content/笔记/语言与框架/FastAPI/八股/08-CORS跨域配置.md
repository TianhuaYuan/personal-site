...
A：**CORS（Cross-Origin Resource Sharing，跨域资源共享）是浏览器的“门禁规则”：当前端 JS 想访问“另一个源”的后端时，浏览器先替后端问一句“你允许这个源吗？”。FastAPI 用  回这张放行条（响应头  等），否则浏览器直接拦掉响应。**
---
...
**Q3：先搞清“源”是什么 —— 怎么理解？**
A：**源（Origin）= 协议 + 域名 + 端口**。三者任一不同就是跨域：
哪怕都在 localhost，端口不同也算不同源。详见 。

**Q4：两种请求 —— 怎么理解？**
A：| 类型 | 条件 | 浏览器行为 |
| :--- | :--- | :--- |
| **简单请求** | GET/POST/HEAD + 安全头（无自定义授权头） | 直接发，响应带 CORS 头即可 |
| **预检请求（preflight）** | 带 、自定义头、非简单方法 | 先发  问权限，过了才发真请求 |
只要你用 （JWT 鉴权几乎必用），就触发**预检**。FastAPI 的  会自动应答 OPTIONS 预检。

**Q5：核心速记主线有哪些？**
A：抓住这几根：一句话总结、先搞清“源”是什么、浏览器怎么处理跨域请求、两种请求、FastAPI 配置（标准写法）、参数速查。
