...
A：** = 同步（FastAPI 丢进线程池跑）， = 异步（在事件循环里跑）。IO 密集型用 ，CPU 密集型用 ；最致命的坑是：在  里调同步阻塞 IO = 冻结整个事件循环，等于白写异步。**
---
...
**Q3：核心：阻塞 vs 非阻塞 —— 怎么理解？**
A：| 操作类型 | 例子 | 该用 |
| :--- | :--- | :--- |
| **IO 密集型** | 查数据库、调外部 API、读文件 |  |
| **CPU 密集型** | 图像处理、PDF 生成、复杂计算 | （或线程池） |
| **混合型** | 先查 DB 再处理数据再调 API | 看瓶颈在哪段 |

**Q4：FastAPI 里两者的真实行为 —— 怎么理解？**
A：**请求全程在一个事件循环线程里处理**
适合 IO 密集型
主事件循环线程不被阻塞，其他异步请求照常
适合 CPU 密集型 + 同步库（不支持 async 时）
** 不保证异步**——只有里面所有  都是真正的异步库（asyncpg、httpx.AsyncClient）时才有收益。否则一个同步阻塞调用把整个事件循环冻住，所有并发请求排队。

**Q5：选择决策树 —— 怎么理解？**
A：可以把同步/CPU 密集代码“借”线程池跑，避免卡住事件循环：

**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、核心：阻塞 vs 非阻塞、FastAPI 里两者的真实行为、选择决策树、性能对比（量级参考）、原理深入：事件循环视角。
