...
A：防止浏览器 MIME 类型嗅探，强制按照 Content-Type 响应头解析内容。

**Q4：3. X-Frame-Options —— 怎么理解？**
A：防止网页被嵌入到 iframe 中，防止点击劫持攻击。
| 值 | 说明 |
|----|------|
| DENY | 完全禁止嵌入 |
| SAMEORIGIN | 允许同源嵌入 |
...
**Q5：4. X-XSS-Protection —— 怎么理解？**
A：启用浏览器的 XSS 过滤器，防止反射型 XSS 攻击。
| 值 | 说明 |
|----|------|
| 0 | 禁用过滤器 |
| 1 | 启用过滤器（默认） |
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：1. 常见安全响应头、2. X-Content-Type-Options、3. X-Frame-Options、4. X-XSS-Protection、5. Strict-Transport-Security (HSTS)、6. Content-Security-Policy (CSP)。
