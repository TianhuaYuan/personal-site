...
**Q1：一句话讲清「请求体与Pydantic」到底是什么？**

A：用 Pydantic 模型当函数参数，FastAPI 自动把 JSON 请求体解析、校验成 Python 对象，省去手写解析。

**Q2：一、请求体 Request Body —— 怎么理解？ —— 怎么理解？**

A：路径/查询参数塞在 URL 里，复杂或敏感的数据（用户名、邮箱）就放 HTTP body 里传 JSON。好比寄快递——地址写面单（URL），物件塞箱子里（body）。FastAPI 靠参数类型判断来源：BaseModel 子类自动当请求体。

**Q3：二、Query / Path / Body / Field 显式声明 —— 怎么理解？ —— 怎么理解？**

A：当自动判断不够精确，用 Query()/Path()/Body() 给参数加约束（必填、长度、范围）。就像快递单填"重量 1-5kg、保价≤1000"。Field() 写在模型里约束字段本身（min_length、gt），Body() 用在函数参数上解决歧义。

**Q4：三、FastAPI 怎么判断参数从哪来 —— 怎么理解？ —— 怎么理解？**

A：判断规则像查表：参数名在路径 {xxx} → 路径参数；类型是 BaseModel 子类 → 请求体；其他 → 查询参数。一张表定生死，调用方写 `user: UserCreate` 就自动从 body 取 JSON，返回 422 还会贴心地列出哪个字段不合法。

**Q5：四、Field vs Body 的区别 —— 怎么理解？ —— 怎么理解？**

A：Field() 管"这个字段长什么样"（长度、范围、描述），写在模型里，用到哪都生效；Body() 管"这个参数从哪来"，用在函数参数上，解决多模型混用时的歧义。一句话：Field 守字段，Body 守来源。

**Q6：核心速记主线有哪些？**

- 请求体放 JSON：存复杂/敏感数据，不在 URL 里

- Pydantic 模型当参数：FastAPI 自动解析校验

- Query/Path/Body/Field：显式加约束（必填、长度、范围）

- 参数来源判定：路径参数 / 请求体 / 查询参数 三选一
...
- [[笔记/语言与框架/FastAPI/实战/05-错误处理与数据校验|错误处理与数据校验]]
