...
A：后台任务让耗时操作在响应返回后执行；CORS 是浏览器跨域请求的放行规则。

**Q2：一、后台任务 BackgroundTasks 为什么需要 —— 怎么理解？**

A：有些活耗时但用户不在乎结果——发邮件、写日志、生成报表。比喻：去餐厅点菜，服务员立刻回"菜马上来"（响应），厨师在后台做菜（后台任务），你不用站厨房门口等。同步写法用户要等 2 秒，后台写法 0.01 秒就收到。英文：BackgroundTasks（异步任务）。

**Q3：二、BackgroundTasks 怎么用 —— 怎么理解？**

A：路由声明参数 background_tasks: BackgroundTasks（FastAPI 自动注入），用 add_task(函数名, 参数) 注册。⚠️ 传的是函数名不是 send_email()——别加括号！重活该用 Celery/RQ 这类真队列，别用 BackgroundTasks 扛大梁。英文：add_task、依赖注入。

**Q4：三、CORS 的"域"是什么、为何拦 —— 怎么理解？**

A："域"=协议+域名+端口，任一不同即跨域（localhost:3000 vs :8000 端口不同）。拦截来自浏览器同源策略——JS 只能请求同源，防恶意网站偷数据。比喻：小区门禁只放本小区住户（同源）进，外人（跨域）拦门口；CORS 就是告诉门禁"这快递员是我叫的"。英文：Same-Origin Policy（同源策略）、CORSMiddleware。

**Q5：四、FastAPI 怎么配 CORS、有哪些坑 —— 怎么理解？**

A：app.add_middleware(CORSMiddleware, allow_origins=[...], allow_credentials=True...)。致命错：allow_origins=["*"] + allow_credentials=True 启动直接 ValueError（规范禁止任意源+凭证）。带 Authorization 头的 JWT 是非简单请求，浏览器必发 OPTIONS 预检，CORSMiddleware 自动应答。英文：preflight（预检）。

**Q6：核心速记主线有哪些？**

- 后台任务=响应后执行，add_task 传函数名不加括号

- 重活别用 BackgroundTasks，交给 Celery/RQ 真队列

- 域=协议+域名+端口，拦截源于浏览器同源策略

- CORS 用 CORSMiddleware 放行；*+credentials 报错

**口诀**

A：后台任务响应后跑，add_task 函数名别加括号；
...
- [[笔记/语言与框架/FastAPI/实战/01-FastAPI入门与环境搭建|FastAPI入门与环境搭建]]
