...
**Q1：一句话讲清「依赖注入与路由分组」到底是什么？**

A：依赖注入用 Depends 把鉴权/分页等公共逻辑抽出来自动注入；APIRouter 把接口按功能拆文件再组装。

**Q2：为什么需要依赖注入 —— 怎么理解？**

A：10 个接口都要校验身份，手写 10 遍 verify_token，改要改 10 处极易漏。DI 把公共逻辑抽成 get_current_user，路由里写 `current_user: dict = Depends(get_current_user)`，FastAPI 请求进来先调依赖、返回值塞进参数，抛异常直接中断。就像厨师说"我需要鸡蛋"，采购员自动买好放桌上。这叫 dependency injection（依赖注入）。

**Q3：yield 依赖与多层嵌套 —— 怎么理解？**

A：依赖要"用完清理"就用 yield 代替 return：`yield db` 前备资源、后收摊，包 try...finally 保证异常也关。依赖还能嵌套——require_admin 依赖 get_current_user 搭出权限链，分别抛 401（不认识你）和 403（没权限）。这叫 yield dependency（带生命周期的依赖）。

**Q4：APIRouter 路由分组 —— 怎么理解？**

A：项目变大全写 main.py 会爆炸。APIRouter 让你在 routers/users.py 里 `router = APIRouter()` 定义接口，主文件 `app.include_router(users_router, prefix="/users", tags=["用户"])` 注册并加统一前缀。还能给整组加 dependencies 公共依赖。这叫 APIRouter（路由分组）+ include_router（注册组装）。

**Q5：依赖注入 vs 中间件 —— 怎么理解？**

A：依赖注入管"路由专属资源"（每个接口各自要的：当前用户、分页、DB 连接），生命周期随请求进出；中间件管"全局横切"（日志、CORS、鉴权统一套在所有请求上）。职责分离：局部复用用 DI，全局一刀切用中间件。这叫 middleware（中间件）与 DI 的分工。

**Q6：核心速记主线有哪些？**

- DI：Depends 抽公共逻辑自动注入，抛异常即中断

- yield 依赖：前备料后收摊；多层嵌套搭权限链（401/403）

- APIRouter：按功能拆文件，include_router + prefix/tags 组装

- 分工：DI 管路由专属资源，中间件管全局横切
