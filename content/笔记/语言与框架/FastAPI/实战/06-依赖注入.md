...
A：假设你有 10 个接口都需要校验用户身份，每个接口都写一遍校验逻辑：
**依赖注入的作用：把公共逻辑抽出来，声明"我需要什么"，FastAPI 自动帮你注入。**
**发生了什么？**
请求进来时，FastAPI 先调用 
返回的结果，作为  参数传入你的函数
...
**Q3：二、路由分组（APIRouter） —— 怎么理解？**
A：假设你在做一个用户管理系统，有这些接口：
如果全部写在一个  里，文件会越来越大。**APIRouter 让你按功能拆分成多个文件。**
**访问路径对照**：
| routers/users.py 里写的 | 实际访问路径 | 说明 |
|------------------------|------------|------|
|  |  | prefix 自动拼在前面 |
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一、依赖注入（Dependencies）、二、路由分组（APIRouter）、速查表、。
