...
sequenceDiagram
  ...
  Note over Loop,Callback: 时刻0ms: 循环开始

  Loop->>Q: i=0 setTimeout回调A 0ms<br/>回调A记住外面同一个i

  Loop->>Q: i=1 setTimeout回调B 100ms<br/>还是同一个i

  Loop->>Q: i=2 setTimeout回调C 200ms

  Loop->>Q: i=3 setTimeout回调D 300ms

  Loop->>Q: i=4 setTimeout回调E 400ms

  Loop->>Loop: 循环结束 i=5



  Note over Loop,Callback: 时刻0ms
  ...
  Note over Loop,Callback: 时刻100ms

  Q->>Callback: 回调B执行 → 同上 i=5 → 输出5



  Note over Loop,Callback: 后面三个回调都是5
...
- 🔗 [[八股文笔记/React-TS-JS/React/08-闭包陷阱深度剖析|React闭包陷阱]] — React Hooks中的闭包陷阱详解

- 🔗 [[笔记/八股文笔记/Python/并发/10-生成器与yield原理|Python生成器yield]] — 闭包与生成器都涉及函数状态保持

- 🔗 [[八股文笔记/操作系统/21-用户态与内核态|用户态与内核态]] — 闭包在用户态管理内存
