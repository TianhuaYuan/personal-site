...
> 用户等一个 10 秒的回答会很焦躁，但看到逐字"打字机"输出就舒服很多。本篇谈 **SSE 推送 + Backpressure 控制 + 流失败降级**。SSE 实现见 [[笔记/八股文笔记/FastAPI-Web/07-SSE流式响应StreamingResponse|SSE流式响应]]；推理原理见 [[笔记/八股文笔记/AI-LLM-RAG-Agent/73-LLM推理优化|LLM推理优化]]。
...
- FastAPI 用 `StreamingResponse` 实现，见 [[笔记/八股文笔记/FastAPI-Web/07-SSE流式响应StreamingResponse|SSE流式响应]]。

## ② Backpressure 控制
- 服务端生成快、客户端消费慢时，中间缓冲会爆。需要背压：
  - 限制单连接未确认数据量，超阈值暂停生成/丢弃非关键事件。
  - Agent 步骤事件可做"合并/节流"，避免高频小事件淹没网络。
- 与推理批处理配合：Continuous Batching 见 [[笔记/八股文笔记/AI-LLM-RAG-Agent/73-LLM推理优化|LLM推理优化]]。
...
## 相关链接
- [[笔记/八股文笔记/FastAPI-Web/07-SSE流式响应StreamingResponse|SSE流式响应]]
- [[笔记/八股文笔记/AI-LLM-RAG-Agent/73-LLM推理优化|LLM推理优化]]
