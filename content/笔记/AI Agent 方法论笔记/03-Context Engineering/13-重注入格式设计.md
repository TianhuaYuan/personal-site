...
重注入格式 = 在重注入关键约束时采用的呈现方式。它复用 [[07-Recency Effect|Recency Effect]]（末尾高权重）+ [[05-首末重注入|首末重注入]]（首尾双钉），目标是最大化约束的「视觉—语义显著性」。Anthropic（2025-09）建议用清晰结构（XML 标签 / Markdown 分节）组织指令。
...
- **进阶**：[[14-路径作用域|路径作用域]] · [[09-长任务重注入|长任务重注入]] · [[07-Recency Effect|Recency Effect]]
