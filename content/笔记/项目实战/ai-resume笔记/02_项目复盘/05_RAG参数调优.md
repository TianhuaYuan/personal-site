...
**Q1：一句话讲清「参数调优完整复盘」到底是什么？**
A：项目原本的调优方式是一个单脚本 ：需要手动改代码、手动跑、手动记结果。存在四个问题：① 无法系统扫描参数（ 组手动跑不现实）；②  只有  维度且无噪声控制；③ 串行跑要 5-10 小时；④ 无 checkpoint，跑一半断则从头来。
所以全部调优工作分两条线推进：
**框架A**（）：模块化实验框架，用于  单变量快速扫描
**框架B**（）：轻量串行框架，用于 Phase 1-6 全量调优
...
| 位置 |   模块 |  |
| 评分维度 |  维 |  维 composite（avg_score + reject_f1） |
| 调度方式 | Model C声明式依赖图+拓扑排序 | 贪心串行  链式继承 |

**Q4：二、预飞检查：先验证再放量 —— 怎么理解？**
A：第二轮调优开始前做了预飞检查——把提示词声称的「就绪」状态和代码真实状态对齐，而不是直接盲跑。
| 问题 | 现象 | 根因 |
|------|------|------|
| B4：as_completed KeyError | 冒烟  崩溃 | 产出的协程≠原始future，字典回查不到 |

**Q5：三、框架A：搭建  评估模块化框架 —— 怎么理解？**
A：参考 RAGAS/DeepEval 设计，所有组件通过  数据契约通信：
所有组件只通过类型通信，互不知道对方实现。换执行器只需拔掉  插上 。
LLM-as-Judge 有温度（temperature>0），同一样本两次评分可能差 0.1-0.2。 每条并发调  次，各维度取 。失败的  不参与中位计算，全部失败才降级 0.5。
**代价**：3x  费用。

**Q6：核心速记主线有哪些？**
A：抓住这几根：零、背景、一、两套评估框架对比、二、预飞检查：先验证再放量、三、框架A：搭建  评估模块化框架、四、框架B：Phase 1-6 串行调参执行、五、实验数据总览。
