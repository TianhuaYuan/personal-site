...
**Q3：retry.py 逐行走读（49 行全量） —— 怎么理解？**
A：📍 
****：泛型标注。 的返回值类型和  类型绑定于传入函数  的返回类型。这么说可能抽象，看调用处就明白了：
**6 种不可重试异常的出处**：全是 Python 内置异常。没有自定义异常。
**函数签名分析**：
: 一个 async 函数，接受  和 
: 3（默认），测试时可以调小
...
**Q4：with_retry 的 8 个调用点 —— 怎么理解？**
A：**fallback=question**：改写失败，返回原始问题。不会让检索无查询可用。
**fallback=None**：调用方检查 None 后打印日志降级。注意这里传的是 （一个 async 闭包函数），不是直接传 lambda。
**fallback=中文字符串**：重试耗尽后告诉用户"服务挂了"。这是用户体验层面的降级——不暴露技术细节。

**Q5：NON_RETRYABLE 机制 —— 怎么理解？**
A：**为什么这 6 种异常不重试？**
**延伸追问**：如果把  从 NON_RETRYABLE 里移除行不行？
→ 可以，但不好。如果 LLM API 返回了  且 SDK 抛出 ，你希望重试。但系统设计者认为 ValueError 不应该来自外部 API——如果外部 API 抛 ValueError，那是 SDK 的 bug，不是瞬时故障。
在实际项目中，更稳健的方式是定义一个"可重试异常"白名单（如 , ），其他都直接抛出。

**Q6：核心速记主线有哪些？**
A：抓住这几根：目录、retry.py 逐行走读（49 行全量）、with_retry 的 8 个调用点、NON_RETRYABLE 机制、Fallback 传播路径、异常层次结构。
