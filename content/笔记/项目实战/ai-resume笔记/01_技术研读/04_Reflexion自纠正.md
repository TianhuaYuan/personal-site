...
**Q4：LLM-as-Judge：evaluate_node 逐行走读 —— 怎么理解？**
A：📍 
**关键设计**： 先读 trace 而不是读 chunks。它通过  之前写入 trace 的  标记判断是否被拒答。
**为什么 **：拒答说明简历中确实没信息，再反思也找不到——直接输出。
注意这里的逻辑和 graph.py 条件边的配合：
条件边  检查 
这里  时**强制设 **
双重保险：即使条件边误判，evaluate_node 自己也会阻断裂环
...
**Q5：反思引擎：self_reflection_node 逐行走读 —— 怎么理解？**
A：📍 
📍 
**关键设计**：不是从某个 State 字段读历史反思，而是从 **trace** 里回溯。因为每次反思都在 trace 里写了一个  条目。
这样设计的好处：不需要额外的 State 字段存历史。但代价是强耦合于 trace 的命名格式 。
📍 
** 只保留最近 2 轮**——太长的话历史反思本身占 token，也会让 LLM 困惑。

**Q6：核心速记主线有哪些？**
A：抓住这几根：目录、涉及文件、LLM-as-Judge：evaluate_node 逐行走读、反思引擎：self_reflection_node 逐行走读、补充查询如何注入搜索、路由节点：什么时候不进入 RAG。
