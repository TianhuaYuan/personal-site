...
**Q3：RagParams 数据类 — rag_params.py 逐行走读 —— 怎么理解？**
A：**文件**：
**一句话**：一个 frozen dataclass 封装了 RAG 流水线的全部 11 个可调参数，外加参数网格定义和冲突校验。
**设计意图**：
把 rag_service.py 里硬编码的参数提取为单一定义源，一份改两处生效（参数定义 + 默认值）
是不动原始对象、创建新副本的惯用法，适合实验场景——每次实验创建一个新的 RagParams 副本，防止参数污染
 被导入但实际没用。

**Q4：evaluate.py — 实验框架主入口 —— 怎么理解？**
A：**位置**：
**第 1-13 行：CLI 使用文档**
一个模块级的 docstring 就是完整的 CLI 使用手册，这是 Python 项目里的好习惯。注意  用  的格式——不是标准 argparse 风格，但更适合快速从实验笔记本粘贴。
**第 27-31 行：Windows GBK 兼容**
这段代码只在该项目运行在 Windows 时生效。

**Q5：run_concurrent.py — 并发加速 Runner —— 怎么理解？**
A：**文件**：
原始的 evaluate.py 里， **逐条串行**评估 120 条 QA：
8 分钟跑一组参数，Phase 2 有 15 组 = 2 小时。不能忍。
**逐行拆解**：
——同时最多 10 个协程在跑。为什么要限制？LLM API 通常有 rate limit（每分钟请求数 / 每分钟 token 数）。10 是一个合理的经验值
——预分配列表，确保线程安全地按索引写入。

**Q6：核心速记主线有哪些？**
A：抓住这几根：目录、RagParams 数据类 — rag_params.py 逐行走读、evaluate.py — 实验框架主入口、run_concurrent.py — 并发加速 Runner、6 阶段实验哲学、实验结果速查。
