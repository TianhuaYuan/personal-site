...
- [[AI与Agent/知识/八股/37-有副作用工具的权限控制|有副作用工具]]

## 速记卡（面试闪卡）

**Q1：一句话讲清「02 — Worker 与 Template Method 模式」到底是什么？**
A：**三个阶段**：
****：注入防护，组装 system_prompt + 定界符代码 + 输出格式
****：LLM 调用 + tracing + retry（重试 1 次，超时不重试）
****：JSON 解析 + confidence 清洗 + 字段补全
**容错策略**：
超时 → info 降级 finding（不抛异常，不阻塞 graph）

**Q2：记忆级：BaseWorker ABC — Template Method 模式 —— 怎么理解？**
A：**三个阶段**：
****：注入防护，组装 system_prompt + 定界符代码 + 输出格式
****：LLM 调用 + tracing + retry（重试 1 次，超时不重试）
****：JSON 解析 + confidence 清洗 + 字段补全
...
**Q3：原理级：4 个 Worker 的 role 与 system_prompt —— 怎么理解？**
A：| Worker | role | 审查范围 | system_prompt 关键点 |
|--------|------|----------|---------------------|
| **QualityWorker** |  | 代码规范/风格 | 命名规范、注释质量、函数长度、PEP8/ESLint |

**Q4：了解级：registry.py — Worker 注册表 —— 怎么理解？**
A：**设计要点**：
**单一来源**：graph.py（装配 StateGraph）和 MCP Server 都复用同一个  dict，避免两处维护两份映射。
**模块级单例**：Worker 是无状态的，一个实例可复用。
**SUPPORTED_ROLES**：有序元组，供路由白名单和校验使用。
作为白名单使用，任何未在  中的角色名都会被  过滤掉。
---

**Q5：常见疑问 —— 怎么理解？**
A：**Q1：为什么用 Template Method 不用 @abstractmethod？**
A：因为 review 的主流程（build→call→parse）是共享的，不是每个子类各写各的。如果用 @abstractmethod，4 个子类会重复写 30 行一模一样的调用逻辑。

**Q6：核心速记主线有哪些？**
A：抓住这几根：记忆级：BaseWorker ABC — Template Method 模式、原理级：4 个 Worker 的 role 与 system_prompt、了解级：registry.py — Worker 注册表、常见疑问、相关笔记、技术学习笔记。
