...
**Q1：一句话讲清「04 - 三层容错与并发 bug」到底是什么？**
A：**位置**：  + 
**逻辑**：（默认 3）→ 条件边跳过所有 Worker 直达 。
通过 state 传入，每个请求可自定义覆盖
内检测熔断状态，报告追加  警告（）
当前是单趟架构所以不会真触发，但机制已就位，后续加迭代循环即生效
**位置**： 
默认 ，测试可覆写为 
超时捕获  + ，返回 1 条  的降级 finding
**不抛异常、不阻塞其他 Worker**

**Q2：核心机制（重点 / 要会画图） —— 怎么理解？**
A：**位置**：  + 
**逻辑**：（默认 3）→ 条件边跳过所有 Worker 直达 。
通过 state 传入，每个请求可自定义覆盖
内检测熔断状态，报告追加  警告（）
...
A：**位置**： 
（编程错误）不重试，直接 raise： /  /  /  /  / 
重试耗尽可传  返回值，不传则 raise 最后错误
同步/异步 callable 通用（内部检测 ）
---
...
|------|------|---------|
|  |  → 熔断 |  + 报告含"熔断" |
|  |  +  | 降级 finding 含 "timeout" |
|  | 混合客户端：1 个慢 Worker + 3 个正常 |  |

**Q5：核心速记主线有哪些？**
A：抓住这几根：核心机制（重点 / 要会画图）、了解级、测试策略、Mermaid：三层容错嵌套、Q&A、相关笔记。
