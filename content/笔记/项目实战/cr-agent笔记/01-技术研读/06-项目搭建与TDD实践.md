...
📍位置：
- `extract_json_array`：先剥 JSON 代码围栏 → 正则 `\[.*\]` → JSON 解析 → 兜底匹配「无问题」关键词（`common.py:13`）
...
- [[工程化与运维/工程化部署/08-pytest异步测试基础设施|pytest异步测试]]
- unittest.mock
- [[工程化与运维/工程化部署/07-pre-commit代码规范|pre-commit代码规范]]
