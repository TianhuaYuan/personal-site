...
**Q1：一句话讲清「Langfuse 可观测性搭建」到底是什么？**
A：用户反馈"这次审查漏报了"，只能看平铺日志——哪个 Worker 跑了多久、LLM 调用花了多少 token、哪个节点失败重试，一概不知。日志是平的，不是结构化的调用链。
但有个现实约束：我当时没装 langfuse SDK（依赖冲突 / 用户环境差异），不能强制依赖。这引出了核心设计原则——**可选依赖 + 静默降级**。
...
A：**问题**：Langfuse SDK 可能不存在，但 tracing 模块不能让业务挂掉。
**怎么想**：抽象 Span/Tracer 接口，NoOp + Langfuse 双 backend。Span 支持  上下文管理（资源自动释放 + 异常安全）。 桥接模式——NoOp 时纯本地，Langfuse 时同步到 backend。
...
**Q5：四、踩坑（值得讲的故事） —— 怎么理解？**
A：**单例跨测试泄漏**排查了最久。 测试拿到 NoOpTracer 不是因为代码错了——是因为前一个测试先创建了 NoOp 单例缓存在模块变量里。这是"测试顺序决定结果"的经典陷阱。 函数 + autouse fixture 解决。
**Python import 引用的坑踩了三次**。graph.py 用  风格时，patch 目标必须是 graph 模块的  引用。base.py 用  +  风格时，patch 目标却是源模块的 。
