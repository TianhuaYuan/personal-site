...
A：httpx AsyncClient 是支持异步的现代 Python HTTP 客户端，自带连接池、超时与流式响应能力。

**Q2：1. httpx 简介 —— 怎么理解？**

A：像升级版 requests：httpx 同时支持同步和异步、支持 HTTP/2、带连接池与超时控制；相比只同步的 requests、只异步的 aiohttp，它更全能，AI 调用外部 API 常用。HTTPX（异步 HTTP 客户端）。

**Q3：2. 基础用法 —— 怎么理解？**

A：像借工具两用法：同步直接 httpx.get/post；异步用 async with httpx.AsyncClient() 配合 await client.get，在 async 函数里发请求。AsyncClient（异步客户端）。

**Q4：3. 连接池 —— 怎么理解？**

A：像共享单车桩：预先建一批连接重复使用，省去每次 TCP 握手。用 httpx.Limits 配最大连接数、保活数与超时，多个请求复用同批连接提速。Connection Pool（连接池）。

**Q5：4. 超时控制 —— 怎么理解？**

A：像给每段等待设闹钟：httpx.Timeout 把 connect/read/write/pool 四个阶段分别设超时，避免卡死；捕获 TimeoutException 可重试或降级。Timeout（超时控制）。

**Q6：核心速记主线有哪些？**

- 简介：同步+异步、HTTP/2、连接池、超时控制

- 用法：同步 get/post，异步 AsyncClient + await

- 连接池：Limits 配连接数，复用省握手

- 超时：分级四阶段 Timeout，异常可重试

**口诀**

A：httpx 异步全能手，同步并发都能走
