...
**Q1：一句话讲清「bcrypt 密码哈希：加盐 + CPU 密集型防爆破 + hash_password / verify_password」到底是什么？**

A：bcrypt 用随机盐 + 可调成本因子的 CPU 密集型哈希存密码，让暴力破解和彩虹表都失效。

**Q2：为什么需要密码哈希 —— 怎么理解？**

A：明文存密码等于数据库一泄露全网裸奔，还会被彩虹表（预计算哈希）和暴力破解攻破。哈希要满足单向、确定、抗碰撞、雪崩。这就像把钥匙熔了只留锁的形状——能验但不能还原，这叫 one-way hashing（单向哈希）。

**Q3：加盐机制 —— 怎么理解？**

A：bcrypt 给每个密码生成随机 salt，混进哈希里（哈希值自带 `$2b$12$...` 盐）。同密码+不同盐=不同哈希，黑客没法用一份彩虹表通杀所有人。就像每人一把独一无二的锁芯，配钥匙必须对着自己的锁。

**Q4：成本因子与性能 —— 怎么理解？**

A：cost factor（默认 12，2026 建议 14-16）决定哈希多慢——因子每加 2 计算时间约翻 4 倍。慢正是目的：拖垮暴力破解的算力，但你的登录只多几百毫秒。这叫 adaptive cost（自适应成本），用时间换安全。

**Q5：实际应用与最佳实践 —— 怎么理解？**

A：用 `bcrypt.hashpw`/`checkpw` 做 hash_password/verify_password，盐被自动存进哈希串。错误时统一回"用户名或密码错误"（别区分哪个错），成本因子随硬件升级调高。记住：别用 MD5/SHA1，别复用盐，别明文存储。
...
- 成本因子：越大越慢越安全（12→14-16）

- 实践：用 hashpw/checkpw；错误统一提示；禁 MD5/SHA1
