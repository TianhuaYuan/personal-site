...
**Q1：一句话讲清「308 永久重定向：API 版本迁移（/api/v1）时防 POST 请求丢失」到底是什么？**

A：308 永久重定向在 API 版本迁移时保持请求方法不变，防止 POST 变 GET 丢数据。

**Q2：题目：API 搬家保 POST（308 Permanent Redirect） —— 怎么理解？**

A：旧版 /api/v1 要废弃、客户端还在用，得平滑迁到 /api/v2。就像店铺搬迁，老顾客照常 POST 下单，不能因为搬家用 301 把订单悄悄变成 GET 弄丢。

**Q3：思路：308 vs 301/302（method-preserving redirect） —— 怎么理解？**

A：301/302 浏览器可能把 POST 改成 GET，请求体和 body 全丢；307/308 严格保持方法。308 是"永久"版，还提示客户端更新书签。选型记住：要保方法用 307 临时、308 永久。

**Q4：代码：FastAPI 中间件（RedirectResponse 308） —— 怎么理解？**

A：中间件里 `if path.startswith("/api/v1/")` 拼出新 URL，返回 `RedirectResponse(url, status_code=308)`，务必带 `Location` 头。别忘了把查询参数 `?x=1` 也拼回去，否则 GET 参数丢失。

**Q5：实战：渐进迁移与监控（gradual migration） —— 怎么理解？**

A：三阶段：先两版共存、再 v1 返回 308、最后删 v1。客户端 requests/fetch 默认跟随，可用 allow_redirects=False 或 redirect:'manual' 手动处理。记得打日志监控重定向量。

**Q6：核心速记主线有哪些？**

- 题目：API v1→v2 迁移用 308 防 POST 丢失

- 思路：308/307 保方法，301/302 可能改 GET

- 代码：RedirectResponse status_code=308，带 Location 与查询参数

- 实战：渐进三阶段迁移，客户端手动可关

- 坑点：勿用 302 替代、查询参数要保留
