...
A：Docker Compose 用一份 YAML 声明多容器应用，并用 .dev/.local/.prod 多文件分离不同环境的配置。

**Q2：1. Docker Compose 基础 —— 怎么理解？**

A：像一份"多容器组装说明书"：docker-compose.yml 里写 services（web/db/redis），build 镜像、ports 映射、environment 变量、depends_on 依赖。一条 docker-compose up 就把整套服务拉起，省去手动敲 docker run。

**Q3：2. 多环境配置 —— 怎么理解？**

A：像三套"换装方案"：基础 docker-compose.yml 放共享配置，再分别叠加 .dev/.local/.prod。启动时 -f 拼两份文件，后者覆盖前者。开发开 DEBUG、映射端口；生产关 DEBUG、开 deploy 副本与资源限制——同一套服务各取所需。

**Q4：3. 环境变量管理 —— 怎么理解？**

A：像给说明书贴"便利贴"：用 .env / .env.dev 存 APP_ENV、SECRET_KEY 等，compose 里 env_file 引入；environment 优先级高于 env_file，敏感值写在 environment 不会被 .env 覆盖错。命令行走 --env-file 也行。

**Q5：4. 常用命令 —— 怎么理解？**

A：四条最常用：up -d 后台启动、down 停止（加 -v 删卷）、logs -f 看日志、exec 进容器跑命令。多环境就在 up 时 -f docker-compose.yml -f docker-compose.dev.yml 拼文件，切环境只换第二份。

**Q6：核心速记主线有哪些？**

- 基础配置放 docker-compose.yml，环境差异放 .dev/.local/.prod

- 启动用 -f 叠加文件，后文件覆盖前文件

- environment 优先级高于 env_file

- depends_on 只保顺序不保就绪，用 healthcheck 兜底

**口诀**

A：Compose 一份 YAML 起多服，
