...
A：给定一个字符串 s 和一个字符串数组 words。words 中所有字符串长度相同。s 中的串联子串是指一个包含 words 中所有字符串以任意顺序排列连接起来的子串。返回所有串联子串在 s 中的起始索引。
**示例：**
---
...
**Q3：思路 —— 怎么理解？**
A：你有一堆拼图碎片（words），每块大小一样（单词长度相同）。你要在一幅大拼图（s）里找到所有能放下这些碎片的位置。
因为碎片可以从任意位置开始放，所以你得尝试所有可能的起始偏移量。每个偏移量下，你用一个"取景框"（滑动窗口）在大拼图上滑动，检查框内碎片是否和你的碎片堆完全匹配。
---
 中所有单词长度相同（），要找  中恰好由  所有单词各出现一次拼接成的子串。
**核心洞察**：把问题转化为「固定大小的块滑动窗口」。
