...
A：912 题要求把给定的整数数组升序排列，是面试高频的手撕快速排序题。

**Q2：题目 —— 怎么理解？**

A：像整理散乱扑克牌：随手抽一张当基准（pivot），比它小的放左、大的放右，左右再各自选基准。Quick Sort（快速排序）就是“分而治之”地选队长分队。

**Q3：思路 —— 怎么理解？**

A：三道工序层层递进：基础快排（固定末位 pivot）→ 随机 pivot（避免有序数组退化成 O(n²)）→ 三路快排（相等元素不递归）。Partition（分区）用双指针 i、j 把 ≤pivot 的换到左边。

**Q4：代码 —— 怎么理解？**

A：核心是 partition 函数：随机选 pivot 与末位交换，j 遍历、i 守“≤区”边界，遇到小的就交换；返回 pivot 位置不再参与递归。随机化把最坏 O(n²) 概率压到极低。

**Q5：复杂度 —— 怎么理解？**

A：平均 O(n log n)、最坏 O(n²)（随机化后几乎不出现）、空间 O(log n)（递归栈）、不稳定（跨位交换）。手撕 Top3 必考题，字节/阿里/美团一面常考。

**Q6：核心速记主线有哪些？**

- 核心思想：选 pivot 分区，左右子数组各自递归

- 随机 pivot 防有序数组退化到 O(n²)

- 三路快排把等于 pivot 的放中间，应对大量重复

- 平均 O(n log n)、空间 O(log n)、不稳定
...
- [[笔记/Leetcode笔记/07_排序/手撕堆排序|手撕堆排序]]

- [[笔记/Leetcode笔记/07_排序/手撕归并排序|手撕归并排序]]

- [[笔记/Leetcode笔记/07_排序/00-解题模板|排序 解题模板]]

- [[笔记/Leetcode笔记/01_数组与哈希/88合并两个有序数组|合并两个有序数组]]

- [[笔记/Leetcode笔记/08_二分查找/33搜索旋转排序数组|搜索旋转排序数组]]
