...
A：已有能生成 1~7 均匀随机数的 rand7()，要实现生成 1~10 均匀随机数的 rand10()。核心是概率里的拒绝采样（Rejection Sampling）。

**Q2：题目与抽奖转盘 —— 怎么理解？**

A：像你有个 7 格转盘想做 10 格转盘——直接映射不行，7 和 10 不整除，概率不均匀。办法是转两次 7 格组成 7×7=49 格大转盘，只取前 40 格映射到 1~10，剩下 9 格不要、重新转，这就是拒绝采样。

**Q3：构造均匀分布 —— 怎么理解？**

A：用 `(rand7()-1)*7 + rand7()` 生成 1~49 均匀分布——第一个 rand7 决定 7 个区间、第二个决定区间内偏移。只要 `num <= 40`，就 `(num-1) % 10 + 1` 映射到 1~10；41~49 拒绝重转。

**Q4：代码与易错 —— 怎么理解？**

A：外层 `while True` 循环采样。易错点：`(rand7()-1)*7` 不是 `rand7()*7`；num 范围是 1~49 不是 0~48；映射要用 `(num-1)%10+1` 而不是 `num%10`。

**Q5：复杂度与变体 —— 怎么理解？**

A：每次成功概率 40/49，期望调用约 49/40 ≈ 1.225 次，所以期望 O(1)。反向也能做：用两个 rand5 构造 1~25，取 1~21（7 的倍数）实现 rand7。

**Q6：核心速记主线有哪些？**

A：题目、拒绝采样思想、49 格构造、前 40 映射、期望次数、反向变体。
