...
A：把链表按升序排好，用自底向上归并做到对数时间、常量空间。

**Q2：题目理解 —— 链表排序难在哪？ —— 怎么理解？**

A：像整理一副乱牌却不能铺满桌子：链表不能随机访问，排序还得常数空间。只能靠合并已排好的小段。英文：Sort Linked List。

**Q3：核心思路 —— 为什么选归并？ —— 怎么理解？**

A：像不断把相邻两沓牌合并：快排要随机访问、堆排要维护堆，链表都不擅长；合并两个有序链表天然 O(n)。英文：Merge Sort。

**Q4：代码实现 —— 自底向上怎么拼？ —— 怎么理解？**

A：像流水线分段：cut 截断 n 个节点、merge 返回头尾，step 从 1 翻倍到 length，每轮把相邻段合并。英文：Bottom-up Merge。

**Q5：复杂度与实战 —— 递归版差在哪？ —— 怎么理解？**

A：像两条路：递归版优雅但占 O(log n) 栈；迭代版严格 O(1) 空间。易错在 cut 要断开、merge 要返回尾。英文：Iterative vs Recursive。

**Q6：核心速记主线有哪些？**

- 归并排序最适合链表：合并有序段 O(n)、无需随机访问

- 自底向上：step 从 1 翻倍，cut 截断、merge 拼接

- 自顶向下递归简洁但占 O(log n) 栈空间

- 自底向上迭代严格 O(1) 空间，最优

- 易错：cut 忘了断链、merge 没返回尾节点
...
- [[笔记/Leetcode笔记/04_链表/23合并K个升序链表|合并K个升序链表]]
