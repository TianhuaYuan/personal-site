...
A：按根→左→右的顺序输出二叉树所有节点的值。

**Q2：题目 —— 怎么理解？**

A：像 CEO 汇报：给你二叉树根节点，返回节点值的前序遍历（根先、再左、再右）。示例 root=[1,null,2,3] 输出 [1,2,3]。

**Q3：思路 —— 怎么理解？**

A：像层层汇报：CEO（根）先讲，再让左团队讲、最后右团队讲，就是根→左→右。递归三行最直白；迭代用栈模拟——栈是 LIFO，所以入栈先右后左，保证左先弹出。还有 Morris 遍历借空闲右指针做到 O(1) 空间，了解即可。

**Q4：代码 —— 怎么理解？**

A：迭代：栈压 root，循环 pop 出来先访问，再压右、压左（左后压→先出）。递归：访问根→dfs 左→dfs 右。Morris：找前驱节点（左子树最右）临时右指针线索化，访问完复原，O(1) 空间但会临时改树。

**Q5：复杂度 —— 怎么理解？**

A：像每个节点看一眼：时间 O(n) 每个节点访问一次；空间迭代/递归 O(h)（h 为树高，栈深=树高），Morris O(1) 借指针不另开栈。树太深递归会爆栈，改用迭代。

**Q6：核心速记主线有哪些？**

- 前序顺序：根 → 左 → 右

- 迭代栈关键：右孩子先入、左孩子后入，保证左先出

- 递归最简洁但考迭代是看你对系统栈的理解

- 三种 DFS 里前序迭代最简单；深度大用迭代防栈溢出
...
- [[笔记/Leetcode笔记/05_二叉树/94二叉树的中序遍历|二叉树的中序遍历]]
