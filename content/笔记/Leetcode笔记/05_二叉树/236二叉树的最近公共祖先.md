...
**Q1：一句话讲清「236. 二叉树的最近公共祖先（Lowest Common Ancestor of a Binary Tree）」到底是什么？**

A：在二叉树里找两个节点最近的共同祖先，后序 DFS 自底向上第一个命中的就是。

**Q2：一、题目与 LCA 定义 —— 怎么理解？**

A：像翻老族谱找你与远亲最近的共同祖宗。给定二叉树和两个节点，返回它们深度最大的公共祖先（Lowest Common Ancestor）。

**Q3：二、后序 DFS 思路 —— 怎么理解？**

A：像派对上派两个助手左右找人，谁两边各喊找到谁就是碰头点。自底向上，左右都命中目标的节点即 LCA（Post-order DFS）。

**Q4：三、四行递归逻辑 —— 怎么理解？**

A：像递归先到底：遇空或命中就返回，否则 left/right 各找。left and right 成立则 root 是 LCA，否则返回非空的那个（Recursive Return）。

**Q5：四、复杂度与易错点 —— 怎么理解？**

A：像最坏遍历整棵树，时间 O(n) 空间 O(h) 递归栈。易错在漏终止条件、left and right 判断放最后（Time/Space Complexity）。

**Q6：核心速记主线有哪些？**

- LCA：两节点深度最大的公共祖先

- 后序 DFS，自底向上第一个左右都命中的即答案

- 核心：left and right 成立→root 是 LCA

- 时间 O(n) 空间 O(h)，BST 版可 O(1)
