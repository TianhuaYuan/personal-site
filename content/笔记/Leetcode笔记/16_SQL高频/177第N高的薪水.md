...
A：表（Id, Salary），编写一个 SQL 函数 ，返回第 N 高的薪水。如果不存在，返回 NULL。
**示例**
---
...
**Q3：思路 —— 怎么理解？**
A：上次颁奖典礼（176题）是请第二名上台。这次老板说："我不想只请第二名，我要一个**通用模板**——第 N 名都行。"
你发现模板和上次几乎一样，只是 OFFSET 从 1 变成了 N-1。但 MySQL 函数里有个坑： 不能直接写表达式 ，必须先用变量存起来。
