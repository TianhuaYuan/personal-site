...
A：实现  函数，将字符串转换为 32 位有符号整数。规则如下：
忽略前导空格
检查下一个字符是  还是 ，确定符号（默认正号）
读取连续的数字字符，直到遇到非数字字符或字符串结束
如果整数超过 32 位有符号整数范围 ，则截断到边界值
返回最终整数
**示例**
---
...
**Q3：思路 —— 怎么理解？**
A：你站在一台自动售货机前，投币口只接受硬币。你先擦掉投币口上的灰尘（跳过空格），然后判断是正投入还是负投入（符号位），接着一个一个投硬币（读数字），如果投太多机器装不下了就截断到最大容量（溢出处理）。
**atoi = 擦灰（跳空格）→ 判断方向（符号位）→ 投硬币（读数字）→ 检查容量（溢出）**
**第 1 步：跳过前导空格**
**第 2 步：读取符号位**
**第 3 步：逐位读取数字并构建结果**
