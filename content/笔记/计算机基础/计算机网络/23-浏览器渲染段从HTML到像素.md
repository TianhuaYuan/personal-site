...
**Q1：一句话讲清「浏览器渲染段从HTML到像素」到底是什么？**

A：浏览器把 HTML/CSS 字节流经解析、合并、布局、绘制、合成五步，最终变成屏幕上的像素。



**Q2：解析建树 —— 怎么理解？**

A：浏览器先把 HTML 字节流解码成字符，再切成 Token（词法分析），搭成节点对象、挂成 DOM 树；CSS 同理建 CSSOM 树。这跟编译原理的词法→语法→AST 一模一样，只是产物叫 DOM。就像按菜谱读清单、把食材归类摆好。这叫 parsing（解析）与 DOM tree（文档对象树）。



**Q3：合并 Render 树 —— 怎么理解？**

A：DOM + CSSOM 配对出 Render 树：可见节点带样式进来，`display:none`、`<head>`、`<script>` 不进；`visibility:hidden` 占坑但看不见。就像菜谱+厨房规范合成"最终做菜方案"，决定什么上桌、什么不上。这叫 render tree（渲染树）。



**Q4：布局 Layout 重排 —— 怎么理解？**

A：Layout 从根算每个节点的几何信息（宽高 x/y、盒模型、文档流、百分比转像素），产出 Layout Tree。改宽高/位置会触发重排，代价最大——前端最该避免的操作。就像重新算每样食材占盘子多大、摆哪。这叫 layout / reflow（重排）。



**Q5：绘制与合成 —— 怎么理解？**

A：Paint 把布局变成像素（背景→边框→文字→阴影，分层绘制）；Composite 用 GPU 把各图层按 z-index 拼成最终画面。transform/opacity 只触发合成，跳过 Layout/Paint，最丝滑。就像下锅炒菜（绘制）+ 装盘叠层（合成）。这叫 paint（重绘）与 composite（合成）。



**Q6：核心速记主线有哪些？**

- 解析：字节→字符→Token→DOM/CSSOM 树

- 合并：DOM+CSSOM→Render 树（display:none 不进）

- 布局：算几何信息，重排代价最大

- 绘制+合成：Paint 画像素，Composite 用 GPU 拼层
