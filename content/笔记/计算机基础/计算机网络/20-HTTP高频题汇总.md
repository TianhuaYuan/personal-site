...
**Q1：GET 和 POST 的区别？**
A：语义（查询 vs 提交）、参数位置（URL vs Body）、幂等性（GET 幂等、POST 不幂等）。安全误区：明文下都不安全，加密靠 HTTPS。

**Q2：Cookie / Session / Token 区别？**
A：Cookie 是客户端容器（4KB、可篡改）；Session 是服务端状态（需共享）；Token(JWT) 是自包含无状态凭证，天然跨域、难吊销。

**Q3：为什么混合加密？**
A：对称加密（AES）快但密钥配送难；非对称加密（RSA）安全但慢。握手用非对称传密钥，业务用对称加密数据，取长补短。

**Q4：HTTP/1.1 vs 2 vs 3 核心区别？**
A：H1.1 文本+持久连接；H2 二进制帧+多路复用（仍 TCP）；H3 换 QUIC(UDP)、解决传输层队头阻塞、支持连接迁移。

**Q5：强缓存 vs 协商缓存？**
A：强缓存不发请求（Cache-Control/Expires，200 from cache）；协商缓存发请求，ETag/Last-Modified 比对，304 省 Body。ETag 精度高于 Last-Modified。

**Q6：为什么需要 WebSocket？**
A：HTTP 轮询浪费资源、实时性差；WebSocket 全双工、轻量头部、持久连接，服务器可主动推送。

> 🔑 口诀：GET 查 POST 提交、明文都不安全；Cookie 容器 Session 状态 Token 自包含；混合加密扬长避短；H2 解应用层 HOLB、H3 解传输层；强缓存不发请求、协商缓存 304。
