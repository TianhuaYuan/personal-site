...
**Q1：一句话讲清「DNS 解析过程」到底是什么？**

A：DNS解析就是浏览器从域名查到IP的过程：先查自身和系统的缓存，没有再找本地DNS，本地DNS一层层问根、顶级、权威服务器，拿到IP后逐级缓存。

**Q2：递归还是迭代：本地DNS替你跑腿 —— 怎么理解？**

A：本地DNS服务器（LDNS, Local DNS Server）像你的私人助理：你只问它一次，它替你跑腿问根、问顶级、问权威，最后把IP带回给你，你不用操心中间过程。

**Q3：根/顶级/权威：三级户口查询 —— 怎么理解？**

A：DNS 体系像查户口：根服务器（Root DNS）只知道".com 在哪"，顶级（TLD, Top-Level Domain）知道"example.com 的权威在哪"，权威（Authoritative DNS）才掌握真实 IP——层层下放，各管一段。

**Q4：缓存是省钱主力 —— 怎么理解？**

A：DNS 缓存（DNS Cache）是性能关键：浏览器、操作系统、本地DNS各缓存一份结果并设 TTL（Time To Live，存活时间）。命中缓存就跳过整套查询，TTL 过期才重新问。

**Q5：hosts 文件是本地霸王 —— 怎么理解？**

A：Hosts 文件（Hosts File）是操作系统级别的"插队通道"：系统在查 DNS 前先翻 hosts，里面写死的域名-IP 直接生效，常用于本地调试和屏蔽广告，优先级高于所有DNS查询。

**Q6：核心速记主线有哪些？**

- DNS 把域名解析成 IP，是 TCP 握手前的第一步

- 查询顺序：浏览器缓存→系统缓存→LDNS→根→TLD→权威

- 各级结果按 TTL 缓存，命中即跳过链路

- 根只指 TLD，TLD 指权威，权威给真 IP
