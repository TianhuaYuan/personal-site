...
A：**TIME_WAIT = 主动关闭方最后等 2MSL 的"保险期"；粘包/拆包 = TCP 是字节流没消息边界，应用层得自己划界。**
---
...
**Q3：一、TIME_WAIT 状态 —— 怎么理解？**
A：（详见 。这里只做速记。）
**MSL**（Maximum Segment Lifetime，最大报文生存时间，Linux 默认 60s）。等 2MSL 两个原因：
**保证最后的 ACK 能到达对端**：若对端没收到 ACK 会重发 FIN，这期间还能补确认。
**让本次连接的所有报文从网络消失**：避免旧连接的延迟报文串到新连接里。

**Q4：二、粘包 / 拆包（TCP 特有的坑） —— 怎么理解？**
A：**根因：TCP 是面向字节流的协议，不保留"消息边界"。** 它只保证字节顺序和可靠，不管你应用层一条"消息"从哪到哪。UDP 相反——面向数据报，有保护消息边界，**不存在粘包**。
像什么？TCP 像水流，你倒进去"ABC""DEF"两瓢，水流出来可能是混在一起的"ABCDEF"；UDP 像快递，一箱一箱分开送。
**Nagle 算法**：为减少小包数量，把多个小数据合并成一个大包再发。

**Q5：核心速记主线有哪些？**
A：抓住这几根：一句话总结、一、TIME_WAIT 状态、二、粘包 / 拆包（TCP 特有的坑）。
