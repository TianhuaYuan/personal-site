...
**Q3：二、网络段 —— 怎么理解？**
A：浏览器先把输入拆解：协议（scheme）、主机（host）、端口（port）、路径（path）、查询参数（query）、锚点（hash）。
浏览器先干两件事：
**HSTS 强制 HTTPS**：浏览器内置了一个"只走 HTTPS"的域名名单，即使你输入 ，浏览器内部直接改成 
**缓存查找**：还没发请求之前，先翻自己的 HTTP 缓存，强缓存命中了直接返回
互联网靠 IP 通信，但人记不住 IP，只记得域名。DNS 就是翻译官。

**Q4：三、浏览器渲染段 —— 怎么理解？**
A：四步过程：
**字节 → 字符**：根据 HTTP 响应头的  和 charset，把字节流解码成字符
**字符 → Token（词法分析）**：逐个字符扫描，碰到  就知道"开始标签来了"
**Token → 节点对象（语法分析）**：按顺序消费 Token，遇到 StartTag 创建节点，遇到 EndTag 弹出栈
**节点 → DOM 树**：所有节点挂好，形成树状结构
**DOM 解析是渐进的** — 浏览器边下载边解析。
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一、全流程概览、二、网络段、三、浏览器渲染段、四、重排 vs 重绘、五、CSS 放 head、JS 放 body 底部的原因、六、async vs defer。
