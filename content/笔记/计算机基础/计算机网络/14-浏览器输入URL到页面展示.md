...
**Q1：一句话讲清「从输入 URL 到页面展示全流程」到底是什么？**

A：这道题讲从输入 URL 到页面渲染的全流程，分网络段（DNS/TCP/TLS/HTTP）和渲染段（DOM/CSSOM/绘制）。

**Q2：一、全流程概览 —— 怎么理解？**

A：祖传第一题分两大段：网络段 URL→DNS→TCP→TLS→HTTP→响应；渲染段 HTML→DOM→CSSOM→Render Tree→Layout→Paint→Composite。前半段解决“数据怎么来”，后半段解决“数据怎么画”。

**Q3：二、网络段 —— 怎么理解？**

A：URL 先 HSTS 强转 HTTPS、查缓存；DNS 走四级缓存链（浏览器→OS→路由器→ISP）再分层递归迭代查 IP；TCP 三次握手防历史连接；TLS 用非对称协商对称密钥（1.2 约 2 RTT、1.3 约 1 RTT）；HTTP 有强缓存/协商缓存。

**Q4：三、浏览器渲染段 —— 怎么理解？**

A：并行建 DOM 树与 CSSOM 树，合并成只含可见节点的 Render Tree；Layout（重排）算位置尺寸，Paint（重绘）画像素，Composite（合成）按层叠成画面。改布局触发重排>重绘>合成，动画用 transform/opacity 最优。

**Q5：四、重排 vs 重绘 —— 怎么理解？**

A：重排（Reflow）改布局属性（宽高/位置/display）→ 贵且必带重绘；重绘（Repaint）只改外观（颜色/背景）；合成（Composite）只 transform/opacity 走 GPU 最便宜。CSS 放 head 防白屏、JS 放 body 底或 defer 防阻塞解析。

**Q6：核心速记主线有哪些？**

- 网络段：DNS 四级缓存 + 三次握手 + TLS 协商密钥

- 渲染流水线：DOM→CSSOM→RenderTree→Layout→Paint→Composite

- 重排 > 重绘 > 合成，动画用 transform/opacity

- CSS 放 head、JS 放底/defer 防白屏
...
- 🔗 [[语言与框架/TypeScript-React/八股/React/02-虚拟DOMVirtualDOM|虚拟DOM]] — 浏览器渲染与 React 虚拟 DOM 的关联

- 🔗 [[计算机基础/计算机网络/34-DNS解析过程|DNS解析过程]] — URL 解析的第一步是 DNS

- 🔗 [[计算机基础/操作系统/19-零拷贝sendfile_mmap_splice|零拷贝]] — 浏览器渲染涉及零拷贝技术
