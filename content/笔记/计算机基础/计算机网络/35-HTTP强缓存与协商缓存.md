...
**Q1：一句话讲清「强缓存 vs 协商缓存」到底是什么？**

A：强缓存是浏览器自己说了算——过期前直接用本地缓存、不跟服务器打招呼；协商缓存是过期后问服务器"还能用吗"。

**Q2：强缓存怎么控制 —— 怎么理解？**

A：像看保质期吃剩饭：浏览器看响应头里的 Cache-Control: max-age（相对时间，从拿到起倒计时 3600 秒，现主流）或老旧的 Expires（绝对时间点，怕时钟不一致）。没过期就直接从硬盘加载，连网络请求都不发。

**Q3：协商缓存两种问法 —— 怎么理解？**

A：像两种核对方式：时间派带 If-Modified-Since 问"这文件改过没"（比 Last-Modified 最后修改时间，秒级精度有坑）；指纹派带 If-None-Match 问"指纹还是 abc123 吗"（ETag 是内容哈希，内容微调也能识别，更精准）。

**Q4：304 和 200 怎么区分 —— 怎么理解？**

A：服务器说"没变"回 304 Not Modified（不传资源，省流量，浏览器用旧缓存）；说"变了"回 200 OK + 新内容。两种都正常，304 恰恰是协商缓存生效、帮用户省了一次下载的好消息。

**Q5：Cache-Control 其他指令 —— 怎么理解？**

A：像缓存的精细开关：no-cache 不是不缓存，是每次都要协商；no-store 才彻底不缓存（银行卡号密码用）；public 谁都能存（CDN/代理），private 只有浏览器能存（用户隐私）；must-revalidate 过期必须回服务器验。

**Q6：核心速记主线有哪些？**

- 强缓存自己定：没过期直接读本地，控制头 Cache-Control / Expires

- 协商缓存问服务器：ETag 指纹 vs Last-Modified 时间

- 304 省流量、200 换新：两者都正常，304 是好事

- 其他指令：no-cache 必协商、no-store 真不存、public/private 定范围
