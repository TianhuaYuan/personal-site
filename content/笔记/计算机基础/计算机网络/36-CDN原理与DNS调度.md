...
A：**CDN 就是在全国各地（甚至全世界）提前部署一大堆缓存服务器，你访问网站时自动把你引到离你最近的那一台，省得每次都要跑到源站去取。DNS 调度就是这个"把你引到最近节点"的导航系统。**
---
...
**Q4：二、CDN 是怎么工作的 —— 怎么理解？**
A：**源服务器（Origin Server）**
真正存着你网站原始数据的地方
通常只有一台（或几台做负载均衡）
**CDN 节点（Edge Server / PoP）**
分布在各地的小型数据中心，每个节点里有一堆缓存服务器
...
**Q5：三、DNS 调度（GSLB） —— 怎么理解？**
A："最近"不是物理距离最近。是网络距离最近。
网络距离 ≈ 延迟最小（ping 值最低）的节点。
判断方式有两种：
CDN 的 DNS 服务器有一张 IP 归属地表：
1.2.3.4 → 广州市，中国电信
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一句话总结、🌰 先搞懂"为什么要用 CDN"、二、CDN 是怎么工作的、三、DNS 调度（GSLB）、四、Anycast——CDN 的终极武器、五、CDN 缓存策略。
