...
A：HTTP 最大的特点之一就是**无状态（Stateless）**——服务器不记得你是谁，每次请求对服务器来说都是一个"新朋友"。但这显然不现实——你登录淘宝后刷新页面，服务器肯定得记得你是谁。怎么做到的？Cookie 和 Session 就是解决方案。
**一句话：HTTP 协议本身不会记录用户的任何信息，两次请求之间没有任何关联。

**Q2：六、HTTP 无状态、Cookie、Session（必考题） —— 怎么理解？**
A：HTTP 最大的特点之一就是**无状态（Stateless）**——服务器不记得你是谁，每次请求对服务器来说都是一个"新朋友"。但这显然不现实——你登录淘宝后刷新页面，服务器肯定得记得你是谁。怎么做到的？Cookie 和 Session 就是解决方案。
**一句话：HTTP 协议本身不会记录用户的任何信息，两次请求之间没有任何关联。

**Q3：核心速记主线有哪些？**
A：抓住这几根：六、HTTP 无状态、Cookie、Session（必考题）。
