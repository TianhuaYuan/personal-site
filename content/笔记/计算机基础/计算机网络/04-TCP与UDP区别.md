...
**Q2：TCP vs UDP vs QUIC（高频对比题） —— 怎么理解？**
A：|  维度  | TCP          | UDP          | QUIC          |
| :--: | :----------- | :----------- | :------------ |
| 连接方式 | 面向连接（三次握手）   | 无连接，直接发      | 基于 UDP，1-RTT 握手（含 TLS） |

**Q3：延伸追问 —— 怎么理解？**
A：视频直播对**实时性**要求极高，丢一两帧画面可以接受（花屏一下），但如果用 TCP，丢包后会**重传 + 等待**，导致画面卡顿延迟，体验更差。UDP 丢了就丢了，继续播新的帧。
**本质原因——队头阻塞（Head-of-Line Blocking, HOLB）**：TCP 严格按序交付，如果第 5 个包丢了，第 6、7、8 包即使已到达接收端缓冲区，应用层也拿不到，必须等第 5 包重传到达。

**Q4：核心速记主线有哪些？**
A：抓住这几根：TCP vs UDP vs QUIC（高频对比题）、延伸追问。
