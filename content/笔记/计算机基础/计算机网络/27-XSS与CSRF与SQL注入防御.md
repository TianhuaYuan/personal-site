...
**Q1：XSS 是什么？怎么防？**
A：用户输入的内容被当成 JS 代码执行（偷 cookie、改页面）。防：输出前转义 `<` `>` 为 `&lt;` `&gt;`，配合 CSP 限制脚本来源、HttpOnly 保护 Cookie。

**Q2：CSRF 是什么？怎么防？**
A：别的网站利用你的登录态冒充你发请求（如偷偷转账）。防：关键操作加 CSRF Token，现代浏览器默认 SameSite=Lax 兜底。

**Q3：SQL 注入是什么？怎么防？**
A：用户输入拼进 SQL 语句改了查询逻辑（如 `' OR '1'='1`）。防：参数化查询（用 `?` 占位符，数据单独传），永远不拼字符串。

**Q4：XSS 和 CSRF 本质区别？**
A：XSS 是在你网站插间谍，能读写当前页面一切（含 token）；CSRF 是冒充你的手发请求，收不到响应。XSS 危害更大，且能窃 token 使 CSRF 防御失效。

**Q5：三者共同点？**
A：都源于"不信任用户输入"——要么转义展示、要么参数化传库、要么加 token 验身份。

> 🔑 口诀：XSS 是脚本被执行，转义+CSP+HttpOnly；CSRF 是借登录态发请求，Token+SameSite；SQL 注入是拼字符串改逻辑，参数化查询别拼串。共同点是别信用户输入。
