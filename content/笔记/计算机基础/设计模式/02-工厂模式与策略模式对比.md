...
# 问题：
# ① if-elif 链越来越长，改一个可能改错另一个
# ② 每种折扣的算法被"揉"在一个函数里，没法单独测试
...
**Q1：一句话讲清「场景：支付系统，支持微信支付和支付宝。没工厂模式时：」到底是什么？**
A：**执行流程（if-else 版本的逐步追踪）**：
**执行流程（简单工厂逐行追踪）**：
**简单工厂**：一个工厂类根据 if-else 创建不同产品。
**工厂方法**：一个抽象工厂类定义"创建产品"的接口，具体创建什么由**子类**决定。

**Q2：二、工厂模式与策略模式对比 —— 怎么理解？**
A：**执行流程（if-else 版本的逐步追踪）**：
**执行流程（简单工厂逐行追踪）**：
**简单工厂**：一个工厂类根据 if-else 创建不同产品。
**工厂方法**：一个抽象工厂类定义"创建产品"的接口，具体创建什么由**子类**决定。
