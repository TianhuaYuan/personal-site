...
**Q1：一句话讲清「Kubernetes GPU 调度：nvidia-device-plugin 原理与配置」到底是什么？**
A：K8s 调度器原生只认识 CPU 和 Memory 这两种资源。GPU 属于**扩展资源（Extended Resources）**——它不是 K8s 内核的一部分，而是通过 Device Plugin 框架暴露给调度器的。
---

**Q2：一、为什么 K8s 原生不支持 GPU？ —— 怎么理解？**
A：K8s 调度器原生只认识 CPU 和 Memory 这两种资源。GPU 属于**扩展资源（Extended Resources）**——它不是 K8s 内核的一部分，而是通过 Device Plugin 框架暴露给调度器的。
---

**Q3：二、nvidia-device-plugin 工作原理 —— 怎么理解？**
A：nvidia-device-plugin 的生命周期遵循标准的 Device Plugin 协议：
| 阶段 | gRPC 方法 | 做什么 |
|------|-----------|--------|
| **发现** |  | 持续向 kubelet 上报 GPU 设备列表和健康状态 |
| **分配** |  | kubelet 决定把 GPU 分配给某 Pod 时，调用此方法 |

**Q4：三、配置实战 —— 怎么理解？**
A：DaemonSet 确保每个 GPU 节点都运行一个 device-plugin Pod。新节点加入集群时自动部署。
**节点亲和性**——只调度到带 GPU 的节点：
**污点与容忍**——GPU 节点加 taint，防止普通 Pod 占用：
---

**Q5：四、GPU 共享方案 —— 怎么理解？**
A：默认一个 GPU 只能给一个容器，独占模式在推理场景下浪费严重。三种共享方案：
| 方案 | 原理 | 隔离性 | 适用场景 |
|------|------|--------|----------|
| **Time-Slicing** | 多个容器轮流使用同一 GPU | 无隔离，互相影响 | 开发测试 / 推理小模型 |

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、为什么 K8s 原生不支持 GPU？、二、nvidia-device-plugin 工作原理、三、配置实战、四、GPU 共享方案、五、延伸追问、一句话总结。
