...
A：**Reflexion**（读「瑞-弗莱克-申」，直译「反射/反思」）= Actor 干活 + Evaluator 打分 + Reflector 修正，三步循环。
类比考试：普通 Agent 是「读完题直接写答案交卷」；Reflexion Agent 是「先写草稿 → 自己检查一遍 → 发现哪里不对 → 改一遍再交」。多了个**显式的自我评估步骤**。
---

**Q2：一、什么是 Reflexion？ —— 怎么理解？**
A：**Reflexion**（读「瑞-弗莱克-申」，直译「反射/反思」）= Actor 干活 + Evaluator 打分 + Reflector 修正，三步循环。
类比考试：普通 Agent 是「读完题直接写答案交卷」；Reflexion Agent 是「先写草稿 → 自己检查一遍 → 发现哪里不对 → 改一遍再交」。多了个**显式的自我评估步骤**。
---

**Q3：二、Reflexion vs ReAct vs Self-RAG —— 怎么理解？**
A：三个概念都带「自我改进」的味道，但机制不同：
| 模式 | 核心机制 | 反思在哪一步 | 适用场景 |
|:----|:--------|:-----------|:--------|
| **ReAct** | Reason → Act → Observe，循环 | 没有显式反思，靠 Observe 结果自然调整 | 工具调用、多步推理 |

**Q4：三、三步循环详解 —— 怎么理解？**
A：Actor 拿到任务描述和当前上下文，生成第一版输出。这一步和普通 LLM 调用没区别。
Evaluator 拿到 Actor 的输出，按预设标准打分。**LLM-as-Judge 是 Evaluator 最常见的实现方式**——用另一个 LLM 调用来做评估。

**Q5：四、完整 Reflexion 循环 —— 怎么理解？**
A：---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、什么是 Reflexion？、二、Reflexion vs ReAct vs Self-RAG、三、三步循环详解、四、完整 Reflexion 循环、五、为什么限制 2-3 轮？。
...
- 项目实践：[[项目实战/ai-resume笔记/01_技术研读/03_LangGraph状态机#Checkpoint 机制|ai-resume: LangGraph状态机]]（Reflexion ≤2 轮 in StateGraph）
