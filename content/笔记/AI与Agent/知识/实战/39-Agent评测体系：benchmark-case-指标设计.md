...
A：传统软件测试是确定性的： 永远返回 ，断言一次就够了。
Agent 有三个「不确定性来源」：
| 维度 | 传统软件测试 | Agent 评测 |
|:----|:----------|:---------|
| 输出确定性 | 确定（1+1=2） | 不确定（同一问题两次回答不同） |
| 断言方式 |  | 语义等价判断（需要 LLM 辅助） |

**Q2：一、为什么 Agent 评测比传统测试难？ —— 怎么理解？**
A：传统软件测试是确定性的： 永远返回 ，断言一次就够了。
Agent 有三个「不确定性来源」：
| 维度 | 传统软件测试 | Agent 评测 |
|:----|:----------|:---------|
...
**Q3：二、三层评测体系 —— 怎么理解？**
A：一份完整的 Agent 评测不是「跑完看对不对」，而是分三层逐级下钻：
| 层级 | 回答的问题 | 谁来评 | 频率 |
|:----|:----------|:------|:----|
| **单元级** | 「Agent 的每一个子动作做对了吗？」 | 规则 + 断言 | 每次 commit |
| **任务级** | 「Agent 完成了一个完整任务吗？」 | LLM-as-Judge + 人工抽检 | 每次部署前 |
...
**Q4：三、核心指标一览 —— 怎么理解？**
A：**「成功」怎么定义？** 这是 Agent 评测最难的问题。不能只看「最终答案对不对」，要看「过程对不对」：
**分模型统计**才有意义——不能把 deepseek-v4（0.3元/1M token）和 Claude Opus（50元/1M token）的成本混在一起算平均。

**Q5：四、Benchmark 设计：三层测试用例 —— 怎么理解？**
A：一个靠谱的 Agent benchmark 必须覆盖三类场景：
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、为什么 Agent 评测比传统测试难？、二、三层评测体系、三、核心指标一览、四、Benchmark 设计：三层测试用例、五、评测流水线：从 run 到 report。


## 相关链接

- 目录：[[00-AI]]
- 项目实践：[[项目实战/cr-agent笔记/01-技术研读/09-LLM评测体系搭建#第一级：Judge 核心（judge.py 🔴）|cr-agent: LLM评测体系搭建]]
- 项目实践：[[项目实战/cr-agent笔记/01-技术研读/00-17条架构决策总览#决策 15: 评测用「软硬双指标」——LLM-as-Judge + PRF|cr-agent: 决策15-软硬双指标]]
