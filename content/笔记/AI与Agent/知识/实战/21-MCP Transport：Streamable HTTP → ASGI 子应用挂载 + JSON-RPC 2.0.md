...
A：MCP 可以拆成三层看（别被术语吓到，就是一个"信封—管道—内容"的关系）：
| 层 | 是什么 | 生活类比 |
|-|-|-|
| **协议层（Protocol）** | 定义消息长啥样、有哪些方法（tools/call、resources/read…） | 信封上写"收件人/寄件人/正文格式"的规矩 |
...
**Q3：三、三种传输横向对比 —— 怎么理解？**
A：| 维度 | stdio | HTTP+SSE（旧，2024-11-05） | **Streamable HTTP（新，2025-03-26）** |
|-|-|-|-|
| 端点数 | 无（走 stdin/stdout） | 2 个： 长连接 +  POST | **1 个：（POST 为主）** |
| 长连接 | 不需要 | **必须常年保持 /sse** | 不需要，按需开 SSE 流 |

**Q4：四、Streamable HTTP 核心机制拆解 —— 怎么理解？**
A：客户端**每个 JSON-RPC 消息都是一次新的 HTTP POST** 到 。服务器看情况二选一返回：
能立刻答：返回  的**单个 JSON 对象**；
要慢慢推（进度、多消息）：返回  的 **SSE 流**，报完关掉。
就像去窗口办事：能马上办完就当场给回执（单 JSON）；要排队慢慢办的，就给你开个"实时进度广播"（SSE 流），办完广播就关。

**Q5：五、JSON-RPC 2.0 在 HTTP 上怎么跑 —— 怎么理解？**
A：MCP 所有消息都是 **JSON-RPC 2.0**（一种无状态的远程调用格式：一个  字段 +  + 可选  + ）。在 Streamable HTTP 上：
**请求**：，（必须同时声明两种，表明"你回单个 JSON 或开流我都接"）。
**响应**：单 JSON 对象，或 SSE 流（流里先发  这类进度通知，最后发最终响应，随后关流）。

**Q6：核心速记主线有哪些？**
A：抓住这几根：二、它是什么 / 为什么需要它、三、三种传输横向对比、四、Streamable HTTP 核心机制拆解、五、JSON-RPC 2.0 在 HTTP 上怎么跑、六、ASGI 子应用挂载、七、演进时间线（🔴 高优）。
