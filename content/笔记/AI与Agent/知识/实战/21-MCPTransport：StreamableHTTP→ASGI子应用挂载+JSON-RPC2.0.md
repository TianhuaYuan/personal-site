...
### 1.2 会话用 `Mcp-Session-Id` header（稳定版 2025-03-26 \~ 2025-11-25）
...
### 1.3 🔴 2026-07-28 无状态化巨变（当前是 RC，最终版 2026-07-28 发布）
...
**Q1：一句话讲清「MCP Transport：Streamable HTTP → ASGI 子应用挂载 + JSON-RPC 2.0」到底是什么？**

A：Streamable HTTP 是 MCP 的默认传输——单端点 /mcp、POST 为主、按需开 SSE 流、可无状态水平扩容，用 ASGI mount 挂进现有服务，一个进程一个端口搞定。



**Q2：二、它是什么 / 三层拆解——怎么理解？**

A：MCP 分三层：协议层（Protocol，定义消息长啥样、有哪些方法，像信封规矩）、传输层（Transport，定义消息怎么在网线上跑，像用平信还是快递）、能力层（Capabilities，具体提供哪些 Tools/Resources/Prompts，像仓库里有哪些货）。关键认知：传输层只负责把 JSON-RPC 文本可靠送达，不关心里面是调工具还是读资源——所以换传输（stdio → Streamable HTTP）上层一行都不用改，这就是分层的价值。



**Q3：三、三种传输横向对比——怎么理解？**

A：stdio 走 stdin/stdout，本地调试无敌但必须同机同进程、不跨网络、一对一；HTTP+SSE（2024-11-05）有 /sse + /message 双端点，已废弃（2025-11-25 官方标记）；Streamable HTTP（2025-03-26）单端点 /mcp、POST 为主、按需开 SSE 流、断哪段重哪段、可无状态、负载均衡友好，是官方推荐默认。版本红线：网上说「2025-05 废弃 SSE」不准确，准确是 2025-11-25。



**Q4：四、Streamable HTTP 核心机制 / 五、JSON-RPC 2.0——怎么理解？**

A：每个 JSON-RPC 消息都是一次新 HTTP POST 到 /mcp：能立刻答就回 application/json 单对象，要慢慢推就回 text/event-stream 的 SSE 流、办完关流。JSON-RPC 2.0 是「jsonrpc:2.0 + method + 可选 id + params」的远程调用格式；通知（无 id）回 202 Accepted，取消就是关 SSE 流。准确性硬约束：当前稳定版（2025-11-25）不支持批处理（2025-06-18 已移除），每个 POST 只发单个消息。2026-07-28 RC 还有无状态化巨变：去握手、去 Mcp-Session-Id、新增 MRTR 与 Mcp-Method 头、移除 Resumable SSE。



**Q5：六、ASGI 子应用挂载——怎么理解？**

A：很多教程把 MCP 当独立服务跑（单独端口、auth、CORS、监控），等于多一套攻击面。正解是把 MCP 当子应用挂进现有 FastAPI/Starlette：mcp.http_app(path="/", transport="streamable-http") 吐出标准 ASGI app，app.mount("/mcp", mcp_app)，复用外层 auth/CORS/日志/lifespan，一个进程一套鉴权。ASGI（Asynchronous Server Gateway Interface，异步服务器网关接口）是 Python Web 异步标准，所以能被挂。还能 mount 多个 MCP 子应用做多租户模块化。



**Q6：核心速记主线有哪些？**

A：二、三层拆解（协议/传输/能力）、三、三种传输对比（stdio / HTTP+SSE 已废弃 / Streamable HTTP 默认）、四、Streamable HTTP 机制（单端点 POST + Mcp-Session-Id + 无状态化 RC）、五、JSON-RPC 2.0（无批处理）、六、ASGI 子应用挂载（复用外层 auth）、七、演进时间线（2024-11-05 → 2026-07-28 RC）、八、生产细节（Origin 校验防 DNS 重绑定）。
...
- [[笔记/AI与Agent/知识/实战/20-MCPServer实现：FastMCP+JWT认证中间件+contextvars|MCP Server 实现：FastMCP + JWT 认证中间件 + contextvars]]
