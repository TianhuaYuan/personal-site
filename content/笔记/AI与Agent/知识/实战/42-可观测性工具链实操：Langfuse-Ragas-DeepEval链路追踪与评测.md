...
A：传统后端服务的可观测性只需要三件套：Metrics + Logs + Traces。Agent 多了一层——你得看见 LLM 的「思考过程」。
| 层次 | 需要追踪什么 | 用什么工具 | 谁来看 |
|:----|:----------|:---------|:------|
...
A：生产环境不可能实时跑 Ragas——LLM Judge 本身要花钱花时间。Ragas 适合**离线批量评测**，定期（如每天/每次发布前）跑一次。
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、Agent 可观测性的三个层次、二、端到端集成架构、三、Langfuse：全链路追踪核心、四、Ragas：离线批量评测、五、DeepEval：CI 流水线中的质量门禁、六、三工具完整集成工作流。
...
- 项目实践：[[项目实战/cr-agent笔记/01-技术研读/09-LLM评测体系搭建#第三级：周边能力（🟢 一句话总结 + 位置）|cr-agent: Langfuse可观测性]]
- 项目实践：[[项目实战/cr-agent笔记/01-技术研读/00-17条架构决策总览#决策 16: 成本控制用 TokenMeter 包 client（不侵入 Worker）|cr-agent: 决策16-成本控制]]
