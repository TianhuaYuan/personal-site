...
## 速记卡（面试闪卡）
**Q1：一句话讲清「Tool Description 设计铁律」到底是什么？**
A：Tool Description（工具描述）是 LLM 决定「什么时候调哪个工具」的唯一依据——写得好 Agent 稳准狠，写得烂 AI 就变成瞎指挥。

**Q2：一、为什么 Tool Description 如此重要？**
A：LLM 选工具不是靠「理解代码逻辑」，而是靠你写在工具定义里的三样东西：name（指定调哪个，名字太像会选错）、description（判断何时该调，写烂了该调不调）、parameters（知道传什么参数，写错 LLM 就瞎编）。核心是换位思考——一个只有文本理解力、没有业务直觉的 LLM，看到你的定义能不能准确判断「现在该用它吗、参数怎么填」。

**Q3：二、工具设计五条铁律——怎么理解？**
A：① 少而精 > 多而全：超过 30~50 个工具选错率线性上升，优先合并为任务级工具；② 高内聚任务级工具 > 零散 CRUD（schedule_event 比 list/add 零散工具好）；③ 枚举约束 > 自由文本（enum 防 LLM 填「驳回」「OK」各种变体）；④ 数字必须设上下限（minimum/maximum，否则 LLM 敢填 -999999）；⑤ 参数命名无歧义（from_account_id 比 from/to 不易搞反）。

**Q4：三、工具返回错误的方式——怎么理解？**
A：最容易踩的坑：抛异常（raise）会让 Agent 循环直接炸，因为 LLM 看不到；正确做法是返回错误字符串，LLM 读到后能自己纠正。区分两种异常：业务错误（金额超限、参数不合法，LLM 能纠正 → 返回错误字符串）；系统错误（网络超时、数据库连不上，LLM 无法处理 → 抛异常走重试/熔断）。工具的错误信息是给 LLM 看的，不是给日志系统看的。

**Q5：四、description 怎么写 / 五、上下文成本真相——怎么理解？**
A：好的 description 写「一句话概括 + 何时用 + 何时不用（指定替代工具）」，准确率能从 60% 提到 95%+；参数 description 要说明值从哪来，防 LLM 编造（幻觉）。成本真相：Manus 团队 2026 生产数据显示工具返回结果占 Agent Token 的 67.6%，优化它比优化 System Prompt 重要 20 倍——返回要精简，工具定义本身也有成本（每次请求都带全部定义）。

**Q6：核心速记主线有哪些？**
A：一、为什么重要（三要素是 LLM 的唯一依据）、二、五条铁律（少而精/任务级/枚举/上下限/无歧义命名）、三、返回错误方式（字符串 vs 异常）、四、description 写法（场景 + 排除）、五、上下文成本真相（工具返回占 67.6%）。

**口诀**
A：工具描述写给 LLM 看，不是给同事看；
少而精、任务级，枚举上下限；
错误返回字符串，别让循环炸；
返回占掉七成 Token，精简最划算。
