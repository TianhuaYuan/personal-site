...
A：向量检索找「意思相近」，BM25 找「字面对得上」，RRF（Reciprocal Rank Fusion，倒数排名融合）把两路结果拼成一份靠谱清单——三剑合璧，召回率和精准率同时起飞。

**Q2：一、为什么一种检索不够？——「盲人摸象」怎么理解？**
A：想象你和两个朋友一起摸一头大象：BM25（关键词检索，Sparse Retrieval）像摸到象鼻的朋友，精确描述「长长的管子、粗糙纹路」这类局部特征；向量检索（Dense Retrieval，语义检索）像摸到象腿的朋友，理解「粗壮的柱子支撑庞大身躯」这种整体语义。只信一个人，对大象的认知就是残缺的。比如「报销制度」这种标题原词 BM25 稳稳命中，但「如何推销保险产品」这种口语改写只有向量能懂——两路各有翻车现场，合起来才完整。

**Q3：二、向量检索（Dense Retrieval）——「找意思相近的」怎么理解？**
A：把查询和文档都转成高维向量（Embedding，用模型编码成 768/1024/1536 维的数字串），在向量空间里算余弦相似度（Cosine Similarity），意思越近距离越近。它懂同义词（「推销」≈「销售」）、能处理口语化查询；弱点是专业术语、错误码、编号这类精确匹配容易漂移，而且依赖 Embedding 模型质量。常用模型有 OpenAI 的 text-embedding-3、开源的 bge-m3（中文优选）。

**Q4：三、BM25 检索（Sparse Retrieval）——「找字面对得上的」怎么理解？**
A：BM25（Best Match 25，最佳匹配 25）是信息检索的概率模型打分算法，相关性分数 = TF 因子 × IDF 因子 × 长度归一化因子。TF（Term Frequency，词频）会饱和，出现 10 次和 100 次差别不大；IDF（Inverse Document Frequency，逆文档频率）让稀有词更值钱（「发票」比「的」区分度高）；长度归一化避免长文档靠词多占便宜。它和 TF-IDF 的区别是：TF-IDF 词频线性增长，BM25 用饱和函数、长度归一更精细，是工业标准。对标题、编号、术语极稳，但换种说法就找不到。

**Q5：四、混合检索架构 + RRF 融合——「双塔并行，合纵连横」怎么理解？**
A：流程是五步——Query 预处理 → BM25 和向量两路并行检索各取 Top-50 → 范围过滤 → RRF 融合成 Top-20 → Cross-Encoder 重排（ReRank）精排到 Top-5 给 LLM。RRF 聪明在完全抛弃原始分数、只看排名：RRF_score(d) = Σ 1/(k + rank)，k 通常取 60（工业标准），只数「两路都排前面的文档更靠前」。关键记牢：融合（Fusion）解决「有没有把证据带回来」，重排（ReRank）解决「带回来的排得好不好」——重排救不回漏召回，所以初次检索要多取候选。

**Q6：核心速记主线有哪些？**
A：一、为什么一种检索不够（盲人摸象）、二、向量检索（Dense Retrieval，找意思）、三、BM25（Sparse Retrieval，找字面）、四、混合检索架构与 RRF 融合（只看排名不看分数）、五、加权融合 vs RRF（要不要归一化）、八、参数调优（每路 Top-K 20~50，RRF k=60）。
...
- [[笔记/AI与Agent/知识/实战/07-分块策略：chunk_sizeoverlap选择、语义分块|分块策略]]
