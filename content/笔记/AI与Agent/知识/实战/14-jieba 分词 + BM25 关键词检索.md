...
# ['深度', '深度学习', '学习', '是', '人工', '人工智能', '智能', '的', '核心', '技术']

# ===== 搜索引擎模式 =====
# 在精确模式基础上，对长词再次切分（提高召回）
words_search = jieba.lcut_for_search(text)
# ['深度', '学习', '深度学习', '是', '人工', '智能', '人工智能', '的', '核心', '技术']
...
**Q1：一句话讲清「jieba 分词 + BM25 关键词检索：让机器「按字找资料」也能找得准」到底是什么？**
A：**关键词检索（Keyword Search）**：把查询和文档都拆成词，然后做**字面匹配**——谁的词重合多、重合得关键，谁就相关。它擅长「精确标识符」类查询：
产品型号：、
错误码：
专业术语：、
这些词，语义向量（把文字变成一串数字那种）反而「认不准」——因为训练数据里太少，向量编不进去。但关键词检索一字不差就能命中。**所以它是语义检索必不可少的互补项**。
...
**Q3：三、原理拆解：从一句话到排行榜 —— 怎么理解？**
A：关键词检索的内部流程，本质是先建一张「词 → 哪些文档有它」的表（叫**倒排索引，Inverted Index**），查询时查表、打分、排序。
**倒排索引长啥样**（来源：CSDN 同上）：文档1「猫吃鱼」→ ；文档3「狗吃骨头」→ 。建出「猫→[doc1]」「吃→[doc1,doc3]」「鱼→[doc1]」。查「猫吃鱼」时，doc1 命中 3 个词 → 最相关。就是这么朴实。

**Q4：四、混合检索：两路并行 + RRF 融合 —— 怎么理解？**
A：既然语义和关键词各有盲区，那就**两路都跑，再把结果合并**。合并最常用 **RRF（Reciprocal Rank Fusion，倒数排名融合）**：不看各自分数绝对值，只看「排名」——排第 1 的权重高，排第 100 的权重低。
k 通常取 60（Milvus 默认），**你项目里取的是 k=100**（来源：你项目  调优记录 ）。排名越靠前，分母越小、分数越高；两路都靠前的文档会被推到最顶。

**Q5：核心速记主线有哪些？**
A：抓住这几根：二、它是什么 / 为什么需要它、三、原理拆解：从一句话到排行榜、四、混合检索：两路并行 + RRF 融合。


## 相关链接

- 目录：[[00-AI]]
- 上一篇：[[13-RAG 参数调优：网格搜索实验框架]]
- 下一篇：[[15-Agent架构与工具调用]]
- 项目实践：[[项目实战/ai-resume笔记/01_技术研读/02_RAG流水线#第 4 层：检索链路|ai-resume: RAG检索链路]]
