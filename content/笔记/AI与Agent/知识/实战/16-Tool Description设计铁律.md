...
**Q1：一句话讲清「Tool Description 设计铁律：清晰描述 + 参数约束 + 错误示例」到底是什么？**
A：LLM 选工具不是靠「理解代码逻辑」，而是靠你写在工具定义里的三样东西：
| 要素 | LLM 用它做什么 | 写烂了的后果 |
|------|---------------|-------------|
|  | 指定「我要调哪个工具」 | 名字太像另一个工具 → 选错 |
|  | 判断「什么时候该调」 | 该调不调，不该调乱调 |

**Q2：一、为什么 Tool Description 如此重要？ —— 怎么理解？**
A：LLM 选工具不是靠「理解代码逻辑」，而是靠你写在工具定义里的三样东西：
| 要素 | LLM 用它做什么 | 写烂了的后果 |
|------|---------------|-------------|
|  | 指定「我要调哪个工具」 | 名字太像另一个工具 → 选错 |
|  | 判断「什么时候该调」 | 该调不调，不该调乱调 |
|  | 知道「要传什么参数」 | 参数名写错 → LLM 瞎编 |

**Q3：二、工具设计五条铁律 —— 怎么理解？**
A：不是工具越多越强大——每多一个工具，LLM 的决策空间就大一圈。工具越多，选错概率越高。**优先合并为任务级工具**。
任务级工具封装了完整的业务流程，LLM 只需要做一次决策。零散 CRUD 工具不仅增加数量，还让 LLM 自己编排步骤——更容易出错。
每个  参数，问自己：**这个值真的是任意字符串吗？** 如果是有限选项，永远用 。
LLM 没有「常识」判断数字的合理范围。你不约束，它就敢填 。
...
**Q5：四、工具的 description 怎么写 —— 怎么理解？**
A：Description 是 LLM 判断「该不该用这个工具」的唯一依据。写得好能让选准确率从 60% 提到 95%+。
每个参数的  也很关键。你要告诉 LLM 这个参数的值从哪里来：
尤其注意：**LLM 会自己编造参数值**（幻觉）。如果参数值必须从上下文获取，一定在 description 里说明。
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、为什么 Tool Description 如此重要？、二、工具设计五条铁律、三、工具返回错误的方式、四、工具的 description 怎么写、五、上下文成本真相。
...
- 项目实践：[[项目实战/cr-agent笔记/01-技术研读/12-项目规格与TechStack#Code Style|cr-agent: 工具系统设计]]
