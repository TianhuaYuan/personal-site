...
created: "2026-07-21"
---
# 降级路径（Degradation）：某环节失败→回退到次优但可用方案
...
**Q1：一句话讲清「降级路径（Degradation）：某环节失败→回退到次优但可用方案」到底是什么？**
A：降级路径指某环节失败时回退到次优但可用的方案，保证用户至少拿到能用的结果。

**Q2：一、原理速览：降级 vs 重试 vs 熔断 —— 怎么理解？**
A：降级（Degradation）就像餐厅主菜卖完了，厨师不空手把你打发走，而是端上一盘还不错的炒饭——你没吃上最理想的，但肚子填饱了。重试（Retry）是挂了再拨一次电话碰碰运气；熔断（Circuit Breaker）是看到对方一直挂你电话就干脆先别打了。三者各治一种病。

**Q3：二、多级降级链（Fallback Chain） —— 怎么理解？**
A：生产级降级不是二选一，而是像电梯一样有多层：主模型挂了切备用模型，再挂切本地模型，最后才到静态话术兜底。检索级从向量库→BM25→缓存逐级降，生成级从完整流程→关 Rerank→关 Reflexion→静态兜底，这就是 Fallback Chain（降级链）。

**Q4：三、降级触发条件 —— 怎么理解？**
A：什么时候该降级？就像身体亮红灯：显式失败（抛异常）、超时（P95 延迟 +2σ，如 >3s）、质量不达标（rerank_score<0.3 的幻觉）、成本超限（token 超 80%）、高峰期（CPU>80% 主动关非核心）。阈值一踩，自动往下降一级。

**Q5：四、降级披露与 Hard-Fail 白名单 —— 怎么理解？**
A：降级最忌静默——检索挂了模型硬答却不说，等于给你端盘假菜还假装新鲜，这是最大反模式。正确做法是在输出里明说「当前降级模式」。但支付、PII、权限这类操作绝不能降级，必须 Hard-Fail（硬失败）直接报错，否则后果更严重。

**Q6：核心速记主线有哪些？**
- per-feature 降级：每个功能独立降级，Search 挂了不影响 LLM 自己答
- 多级 Fallback Chain：模型 / 检索 / 生成各自成链，逐级兜底
- 必须披露：降级不能静默，要向用户说明当前服务状态
- Hard-Fail 白名单：支付 / PII / 权限类操作硬失败，绝不降级
