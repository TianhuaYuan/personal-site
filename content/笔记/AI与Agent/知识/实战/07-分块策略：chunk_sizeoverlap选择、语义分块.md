...
A：分块是 RAG 的第一公里，在上下文完整、检索精度、成本三者间切出大小刚好的块。

**Q2：一、为什么分块 —— 怎么理解？**
A：像吃自助牛不能整头塞、也不能剁成肉泥，要把文档切成带口感的牛排块，检索才精准（Chunk 分块）。

**Q3：二、chunk_size 与 overlap —— 怎么理解？**
A：太大噪声稀释、太小语义腰斩；overlap 是块间留拼接缝，防关键信息恰好落在切点上被切断（Overlap 重叠窗口）。

**Q4：三、分块策略全景 —— 怎么理解？**
A：从朴素固定到递归（工业 Baseline），再到结构感知、父子索引、语义分块层层递进（Recursive Chunking 递归切分）。

**Q5：四、语义分块与前沿 —— 怎么理解？**
A：语义分块顺纹理切、相似度突变才下刀；Late Chunking 先嵌入后切、Contextual 给每块贴说明书（Semantic Chunking 语义分块）。
...
- [[笔记/AI与Agent/知识/实战/33-上下文治理：多轮任务上下文越来越长的处理策略|上下文治理：多轮任务上下文越来越长的处理策略]]
