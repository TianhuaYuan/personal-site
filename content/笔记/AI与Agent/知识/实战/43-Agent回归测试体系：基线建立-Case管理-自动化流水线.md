...
A：传统软件回归测试的逻辑是：改了模块 A，不影响模块 B。Agent 的问题是——**改了 Prompt 的一句话，可能把之前 100 条都能跑通的用例干掉 30 条**。
| 维度 | 传统软件回归 | Agent 回归 |
|:----|:----------|:---------|
| 回归信号 | 测试从绿变红（assert 失败） | 分数从 0.88 掉到 0.

**Q2：一、为什么 Agent 要比传统软件更重视回归测试？ —— 怎么理解？**
A：传统软件回归测试的逻辑是：改了模块 A，不影响模块 B。Agent 的问题是——**改了 Prompt 的一句话，可能把之前 100 条都能跑通的用例干掉 30 条**。
| 维度 | 传统软件回归 | Agent 回归 |
|:----|:----------|:---------|
| 回归信号 | 测试从绿变红（assert 失败） | 分数从 0.88 掉到 0.
...
**Q4：三、Golden Test Cases：格式与设计 —— 怎么理解？**
A：Golden Test Cases 不是随便攒几条——每条必须有明确的「为什么重要」：
---

**Q5：四、Case 管理：版本控制 + 分类目录 —— 怎么理解？**
A：---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、为什么 Agent 要比传统软件更重视回归测试？、二、回归测试三组件、三、Golden Test Cases：格式与设计、四、Case 管理：版本控制 + 分类目录、五、自动化流水线：回归测试 Runner、六、CI 集成：GitHub Actions 回归门禁。
...
- 项目实践：[[项目实战/cr-agent笔记/01-技术研读/00-17条架构决策总览#决策 17: 评测 CLI 的 token 计量开关 + 模块函数还原（第三版收尾补丁）|cr-agent: 决策17-评测CLI]]
