...
> Embedding 模型 Self-Attention 计算量 O(n²·d)，所以输入长度翻倍，计算量翻 4 倍。长文档需分块处理。
...
> Embedding 向量经过 L2 归一化后，余弦相似度、点积、欧氏距离三者的排序结果完全等价。选余弦相似度只是因为值域固定[-1,1]，容易设阈值。
...
A：Embedding 就是把"意思"变成"位置"——让语义相近的文字在高维空间里挨着，语义无关的文字离得远。
类比：**把每个词/句看成地球上的一个地点，Embedding 就是它的经纬度坐标。** "北京"和"上海"的坐标很近，"北京"和"巴黎"的坐标远。你搜"首都"，系统找到的坐标附近就是北京——哪怕你完全没提"北京"两个字。
---
...
**Q3：二、Embedding 模型的工作原理 —— 怎么理解？**
A：**Embedding 模型和 LLM 本质上是同一类 Transformer 网络（一种专门处理序列数据的神经网络结构，核心是 Self-Attention 机制），区别在输出层和训练目标。**
| 对比维度 | LLM | Embedding 模型 |
|---------|-----|---------------|
| 输出 | 下一个 token 的概率分布 | 一个固定长度的向量 |

**Q4：三、相似度计算：余弦 vs 欧氏 —— 怎么理解？**
A：**用一个"买东西"的例子秒懂：**
**关键洞察：**"A 和 B 都是衣服党" 这件事，余弦能抓住，欧氏抓不住——因为欧氏只看"绝对值差距"。
| 对比 | 余弦认为 | 欧氏认为 |
|------|---------|---------|
...
**Q5：四、对称 vs 非对称 Embedding —— 怎么理解？**
A：| 场景      | 查询和文档的形态    | 例子                   | 典型模型做法              |
| ------- | ----------- | -------------------- | ------------------- |
| **对称**  | 查询≈文档（同形态）  | 句子相似度、文本聚类、去重        | 同一个模型，不加区分          |

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、一句话理解、二、Embedding 模型的工作原理、三、相似度计算：余弦 vs 欧氏、四、对称 vs 非对称 Embedding、五、Embedding 模型选型与对比（2026 年更新）、六、稠密向量 vs 稀疏向量。
