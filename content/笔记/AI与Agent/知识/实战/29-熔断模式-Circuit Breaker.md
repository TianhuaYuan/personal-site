...
A：重试治的是「偶发瞬态故障」——网络偶尔抖一下、API 偶尔限流，等一两秒重试就好了。
但如果对方服务是**真宕机了**（持续故障），还一遍遍重试只会：
**让用户干等** —— 每次重试等 1s→2s→4s，用户眼睁睁看着转圈
**雪上加霜** —— 把已经半死的服务彻底压垮
**自 DDoS** —— 多 Agent 并行时几十个实例同时重试 = 自己打自己
...
A：熔断器名字来自家里的「保险丝/空气开关」：电流过大自动断电，保护整条线路。
它是一个经典的**三态状态机**（源自 Martin Fowler）：
| 状态 | 含义 | 行为 |
|:----|:----|:-----|
| **CLOSED（闭合）** | 正常状态 | 请求照常放行，计数器记录失败次数 |
...
A：传统熔断器是「一个服务一个」。但 Agent 同时调一堆东西——搜索工具、向量库、多个大模型、代码沙箱……每个的「正常故障率」天差地别：
| 组件 | 正常故障率 | 熔断阈值怎么设 |
|:----|:----------|:-------------|
| 开放网页搜索 | 5-8% | 宽松（如 15%） |
| 向量库检索 | 0.

**Q5：四、代码实现：用 aiobreaker 实现熔断 —— 怎么理解？**
A：[!tip] **什么异常不计入熔断？**
只把「系统错误」（网络超时、500、连接拒绝）计为熔断失败。
「业务错误」（参数不合法、查无结果）不计入——这些是 LLM 能自己纠正的。

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、为什么光有重试不够？、二、熔断器原理：三态状态机、三、Agent 专属进阶：分层多熔断器、四、代码实现：用 aiobreaker 实现熔断。
...
- 项目实践：[[项目实战/ai-resume笔记/01_技术研读/06_韧性工程#with_retry 的 8 个调用点|ai-resume: 韧性工程]]
- 项目实践：cr-agent: 三层容错
