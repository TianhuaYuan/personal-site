...
- 项目实践：[[项目实战/cr-agent笔记/01-技术研读/10-安全加固四道防线#第二级：防线详解|cr-agent: 安全加固四道防线]]
...
**Q1：一句话讲清「恢复: agent.invoke(Command(resume={"approved": True}), config)」到底是什么？**
A：**HITL（Human-in-the-loop）** = 在 Agent 自动执行流程里预设**暂停点**，到了这些点就停下来等人类决策（批准/拒绝/修改/直接回答），人类给完信号再从原处接着跑。它把「要不要让人把关」从写在 prompt 里的软约束升级成**框架级别的硬性原语**。

**Q2：一、原理速览 —— 怎么理解？**
A：**HITL（Human-in-the-loop）** = 在 Agent 自动执行流程里预设**暂停点**，到了这些点就停下来等人类决策（批准/拒绝/修改/直接回答），人类给完信号再从原处接着跑。它把「要不要让人把关」从写在 prompt 里的软约束升级成**框架级别的硬性原语**。

**Q3：二、代码实现 —— 怎么理解？**
A：核心就三样： 抛、 分流、 续命。checkpointer 负责存盘。
| 框架 | 审批 API | 默认姿态 |
|------|---------|---------|
| Microsoft Agent Framework | 工具审批中间件 | 技能提供方带来的工具**默认需审批** |
