...
（详见 [[46-LangGraph MemorySaver Checkpoint|下一篇]]）
...
### ai-resume 项目：9 节点 StateGraph

你的 ai-resume 项目中的 `graph.py` 用了一个 **9 节点 StateGraph**：

```mermaid
flowchart LR
    START --> Q["query_rewrite<br/>Query改写"]
    Q --> S["search<br/>向量检索"]
    S --> R["rerank<br/>重排序"]
    R --> G["generate<br/>生成回答"]
    G --> F{"faithfulness<br/>忠实度检查"}
    F -->|"不通过"| Q
    F -->|"通过"| END
```

详见：[[项目实战/ai-resume笔记/01_技术研读/03_LangGraph状态机#graph.py逐行走读|ai-resume: LangGraph]]

### cr-agent 项目：Supervisor-Worker StateGraph

cr-agent 用了 **Supervisor-Worker 模式**——一个 Supervisor 节点做路由，多个 Worker 节点并行执行：

```mermaid
flowchart TD
    S["supervisor<br/>Supervisor路由"] --> W1["worker_1<br/>代码风格检查"]
    S --> W2["worker_2<br/>安全漏洞扫描"]
    S --> W3["worker_3<br/>性能分析"]
    W1 --> A["aggregator<br/>结果汇总"]
    W2 --> A
    W3 --> A
    A --> END
```

详见：[[项目实战/cr-agent笔记/01-技术研读/01-Supervisor-Worker编排与StateGraph|cr-agent: Supervisor-Worker编排]]

## 速记卡（面试闪卡）

**Q1：一句话讲清「LangGraph框架：StateGraph-Node-Edge-Checkpoint」到底是什么？**
A：你已经用 LangChain 写出了  的管道。管道是**线性的**——数据从左流到右，一条路走到底。
但 Agent 不是线性的：
这是一棵**决策树**，不是一条管道。LangGraph 用**图（Graph）**来建模这种分支和循环。
管道是**单向线**，图可以有**循环和分支**。这就是 LangGraph 存在的理由。
---

**Q2：一、先搞清楚：为什么需要 LangGraph？ —— 怎么理解？**
A：你已经用 LangChain 写出了  的管道。管道是**线性的**——数据从左流到右，一条路走到底。
但 Agent 不是线性的：
这是一棵**决策树**，不是一条管道。LangGraph 用**图（Graph）**来建模这种分支和循环。
管道是**单向线**，图可以有**循环和分支**。这就是 LangGraph 存在的理由。
...
**Q3：二、四个核心概念 —— 怎么理解？**
A：**大白话：** State 就像手术室里挂在墙上的白板——每个医生（Node）进来看一眼、写上自己的发现、传给下一个医生。所有医生看的是同一块板。
每个 Node 是一个 Python 函数，签名为：
Node 可以是：
| Node 类型 | 做什么 | 例子 |
|-----------|--------|------|
...
**Q4：三、完整的 StateGraph 代码示例 —— 怎么理解？**
A：一个带条件边的 3 节点 Agent：
---

**Q5：四、图结构可视化 —— 怎么理解？**
A：上面代码构建出的 Graph 长这样：
关键点：
**循环**： 形成了 Agent 的核心循环（ReAct 模式）
**条件边**： 后的分支完全由 LLM 输出决定
**终止**： 是唯一的出口
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、先搞清楚：为什么需要 LangGraph？、二、四个核心概念、三、完整的 StateGraph 代码示例、四、图结构可视化、五、StateGraph vs 手写循环、六、Checkpoint 机制预览。
...
- 项目实践：[[项目实战/ai-resume笔记/01_技术研读/03_LangGraph状态机#State 设计逐字段解读|ai-resume: LangGraph状态机]]
