...
- 项目实践：[[项目实战/ai-resume笔记/01_技术研读/05_MCP协议#server.py — FastMCP 实例 + JWT 认证中间件|ai-resume: MCP协议]]
...
A：第 8 篇讲了 MCP 是 AI 世界的「USB-C 标准插槽」。但标准再好，得有人去**实现那个插头**——也就是**自己写一个 MCP Server**，把你的能力按规范暴露出去。
生活类比接着用：你给自己家做了个超好用的榨汁机（你的检索 / 查询能力），别人（AI 应用）想用。
不按标准？那每台想用它的机器都得给你单独配一根线（N×M 噩梦）。

**Q2：一、开篇：你家的专属电器，也得能插上标准口 —— 怎么理解？**
A：第 8 篇讲了 MCP 是 AI 世界的「USB-C 标准插槽」。但标准再好，得有人去**实现那个插头**——也就是**自己写一个 MCP Server**，把你的能力按规范暴露出去。
生活类比接着用：你给自己家做了个超好用的榨汁机（你的检索 / 查询能力），别人（AI 应用）想用。
不按标准？那每台想用它的机器都得给你单独配一根线（N×M 噩梦）。

**Q3：二、FastMCP 是什么：把协议细节降到装饰器 —— 怎么理解？**
A：**FastMCP** 是目前用 Python 构建 MCP Server 最顺手的**生产级框架**（源码即 FastMCP 3）。官方 SDK 当然也能写，但 FastMCP 把底层协议（JSON-RPC 2.0、握手、能力发现）全封装了，开发者只写业务逻辑。

**Q4：三、为什么 MCP Server 必须认证（不装锁 = 裸奔） —— 怎么理解？**
A：暴露一个 MCP Server，等于把一个能力入口挂到网上。不认证的后果：
**任意人可调用**：你的  工具谁都能触发。
**多租户数据泄露**：A 公司的请求读到了 B 公司的知识库（如果你的 RAG 服务多客户共用）。
**无法审计**：出了事查不到是谁干的。
**JWT = JSON Web Token（JSON 网络令牌）**，是一种把「身份信息 + 权限」编码进令牌、并用密码学签名防篡改的开放标准。

**Q5：四、JWT 认证中间件：在工具之前验明正身 —— 怎么理解？**
A：中间件（middleware）夹在「请求到达工具」之前，统一做一件事：**先验身份，再放行**。它是 ASGI 层的一个包装函数，拦截每个  请求。
骨架（感觉级）：
中间件做的四步（对应上面注释）：
取 ；
用 **PUBLIC_KEY** 验 **RS256** 签名（篡改即失败 → 401）；
校验业务声明：、所需 （如 ）满足；
把原始 token + 解码 payload 写进 **contextvars**，放行请求。

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、开篇：你家的专属电器，也得能插上标准口、二、FastMCP 是什么：把协议细节降到装饰器、三、为什么 MCP Server 必须认证（不装锁 = 裸奔）、四、JWT 认证中间件：在工具之前验明正身、五、contextvars：每请求隔离的「上下文保险箱」、六、三者怎么串成一条链（核心架构）。
