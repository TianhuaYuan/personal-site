...
- 下一篇：[[03-多模型后端抽象]]
- 项目实践：[[项目实战/ai-resume笔记/01_技术研读/01_架构概览#API 层详细解读|ai-resume: 架构概览]]
...
A：学了 Token、Temperature、System Prompt，知道怎么用，但 LLM 底层到底是什么？
不是比喻。DeepSeek V4 Flash ≈ 2840 亿个数字，存成一个几百 GB 的文件。
没有思考，没有逻辑模块，从头到尾就是数学运算。
不是学"事实"，是学"统计规律"——给定上文，下一个字大概率是什么。
"推理"是副产品。
...
A：| 维度 | DeepSeek V4 | 通义千问 (Qwen 3.7) |
|------|-------------|---------------------|
| **注册地址** | platform.deepseek.

**Q4：三、LLM vs RAG vs Agent — 三个层次 —— 怎么理解？**
A：学了 Embedding，自然引出 RAG；RAG 再进一步就是 Agent。三者到底什么关系？
LLM 不知道你的私有文档 → 你把文档 Embedding 存向量库 → 提问时搜相关段落 → 拼进 Prompt → LLM 照着回答。
关键澄清：
**LLM 参数 ≠ 向量数据库**。两个完全独立的系统。
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一、LLM 的本质、二、LLM API 调用、三、LLM vs RAG vs Agent — 三个层次、。
