...
详见：[[项目实战/cr-agent笔记/01-技术研读/01-Supervisor-Worker编排与StateGraph#记忆级graph.py+state.py逐行精读|cr-agent: StateGraph]]

## 速记卡（面试闪卡）

**Q1：一句话讲清「LangGraph MemorySaver Checkpoint」到底是什么？**
A：上一讲  讲了 Node 之间靠 State 传数据。但有一个问题没解决：**State 存在哪里？**
Checkpoint 就是 **Agent 的存档点**——跟玩游戏存盘一个道理。
---
...
A：MemorySaver 在每次 Node 执行后做两件事：
**合并 State**：把 Node 返回的  合并到现有 State
**序列化并存储**：把完整的 State 存到内存字典 
---

**Q4：三、thread_id 隔离机制 —— 怎么理解？**
A：**要点：**  就是会话 ID。每个用户/对话一个 thread_id，State 互不干扰。这不叫"多租户"，叫**会话隔离**。
---
...
| 字段 | 含义 | 用途 |
|------|------|------|
|  | 当前完整的 State 字典 | 看 LLM 的中间输出、工具结果 |
|  | 下一个要执行的节点名 | 判断流程走到哪了 |
|  | step 序号、source、写入时间 | 追踪执行路径 |
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、为什么需要 Checkpoint？、二、MemorySaver 工作原理、三、thread_id 隔离机制、四、get_state：调试和状态检查、五、三种持久化后端对比、六、Human-in-the-loop：利用 Checkpoint 实现人工审批。
