...
A：很多人以为 LLM API 的费用大头是「模型生成文本」。实际上，**输入 token（把上下文喂给模型）才是真正的成本黑洞**。
一次典型 Agent 任务（以代码审查为例）的 Token 流向：
| 消耗来源 | 占比 | 说明 |
|:----|:----|:-----|
| **工具返回结果** | ~67.
...
A：核心思想：不是每个 LLM 调用都需要最贵的模型。把调用按复杂度分层，便宜模型做简单活，贵模型做关键活。
| 层级 | 模型选择 | 单价（约） | 适用场景 | Token 特点 |
|:----|:--------|:---------|:--------|:----------|
| **L1: 路由层** | deepseek-v4-lite / gpt-4o-mini | ¥0.05-0.

**Q4：三、语义缓存：避免重复花钱 —— 怎么理解？**
A：Agent 的输入很少一模一样——「提取这份简历的技能」和「帮我看看这份简历会什么技术」语义相同但字符串完全不同。精确匹配（）命中率极低。
| 场景 | 缓存命中率预估 | 建议 |
|:----|:------------|:-----|
| 意图分类（"这是什么类型的问题"） | 60-80% | 强烈推荐缓存 |
| RAG 检索 + 摘要 | 20-40% | 推荐缓存（检索结果相似时命中） |
...
**Q5：四、其他省钱技巧 —— 怎么理解？**
A：OpenAI 和 Anthropic 都支持 Prompt Caching——重复出现的 System Prompt 和 Tool Definitions 只计费一次。
对于不需要实时响应的任务（如批量评测、离线分析），用 Batch API 可节省 50% 费用。
...
- 项目实践：[[项目实战/cr-agent笔记/02-项目复盘/15_多模型后端支持#二、逐个讲：问题 → 怎么想 → 怎么解|cr-agent: 多模型后端]]
- 项目实践：[[项目实战/cr-agent笔记/01-技术研读/00-17条架构决策总览#决策 16: 成本控制用 TokenMeter 包 client（不侵入 Worker）|cr-agent: 架构决策]]
