...
A：MCP Client 是 Host 与 Server 之间的接线员——连接池复用省握手、幂等键防重复执行、断线退避重连，三道防线保生产级可靠调用。

**Q2：二、为什么需要「自定义」客户端？——怎么理解？**
A：官方 Client 90% 场景够用（Client(url) 一行 + async with 管理生命周期），但生产里它默认不兜底几件事，逼你包一层工程化外壳：连接池/复用（HTTP 每次 async with 重建）、重连+幂等（自动重连有，但幂等靠你自己）、可观测性（自己埋 trace_id）、多 Server 路由（ConnectionPool 按名管理）、自定义头部/认证（动态 JWT、租户头）。所以「自定义」不是重写协议，而是在官方 Client 之上管起连接、重试、幂等、池化、监控。

**Q3：三、JSON-RPC over HTTP——怎么理解？**
A：JSON-RPC（JSON Remote Procedure Call，JSON 远程过程调用）是 MCP 的「说话格式」，HTTP（HyperText Transfer Protocol，超文本传输协议）是「送信的邮路」，拼起来就是用 HTTP 寄 JSON-RPC 格式的信。请求带 id（本次唯一编号，Server 原样带回让 Client 对上信）、method（initialize/tools/list/tools/call…）、params；通知（Notification）不带 id，Server 收到就处理不回。稳定版不支持批处理，每个 POST 只发单个消息。Mcp-Session-Id 像会员会话号，后续请求都得带。

**Q4：四、调用全流程 / 五、连接池——怎么理解？**
A：自定义 Client 四步：建会话 initialize → 问能力 list_tools → 调工具 call_tool → 断线重连。连接池解决「每调一次都新建连接」的开销爆炸：TCP 三次握手 + TLS（Transport Layer Security，传输层安全协议，HTTPS 加密层）握手几百毫秒起步，再加 MCP initialize 一轮往返，高频下握手比工具执行还长。连接池提前建好一批连接谁用谁拿；httpx.AsyncClient 自带连接池（用 httpx.Limits 控大小），ConnectionPool 按名管理多 Server。

**Q5：六、幂等重连——怎么理解？**
A：网络抖动常见，Server 已执行退款但响应半路丢了，Client 重发就会退两次钱。幂等（Idempotent，多次执行副作用只发生一次）就是解药。三道防线：① 请求 ID 去重（JSON-RPC 自带，同 id 重复响应直接丢弃）；② 业务幂等键 Idempotency-Key（写操作由 Client 生成、Server 去重，谁发起谁负责）；③ 重试预算 + 指数退避（Exponential Backoff：1s→2s→4s）+ 抖动（Jitter，防重试风暴）。稳定版有 Resumable SSE（Last-Event-ID 重放），但 2026-07-28 RC 移除了它，状态恢复要靠应用层显式 handle。

**Q6：核心速记主线有哪些？**
A：二、为什么自定义（官方够用但生产要掌控连接）、三、JSON-RPC over HTTP（说话格式 + 邮路）、四、调用全流程（建会话→问能力→调工具→重连）、五、连接池（省握手开销）、六、幂等重连（三道防线）、七、手写 vs 官方（先官方再包一层）。

**口诀**
A：Client 接线员，连接池省握手；
id 去重 JSON-RPC，幂等键防重单；
指数退避加抖动，重试预算别打爆。
