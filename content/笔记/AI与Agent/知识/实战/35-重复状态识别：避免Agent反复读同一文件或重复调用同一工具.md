...
A：Agent 经常像金鱼一样——刚读完的文件忘了，再读一遍；刚调完的工具忘了，再调一遍。重复状态识别用「调用记录 + 哈希指纹」让 Agent 知道「这事我已经干过了，结果在这儿」。
...
**Q3：二、两层去重机制 —— 怎么理解？**
A：**L1：短期去重**：哈希指纹 + 调用缓存，同一会话内，相同参数不重复执行，低；**L2：长期去重**：visited-set + 持久化调用日志，跨会话识别「之前已经分析过这个模块」，中。
...
**口诀**
A：重复状态识别：问题本质：Agent 为什么容易「卡进循环」？先想；
两层去重机制配L1：基于哈希的去重（Hash-Based Dedup），
L2：Visited-Set 模式（跨文件/跨模块去重）不能忘，
面试对答底气壮。
