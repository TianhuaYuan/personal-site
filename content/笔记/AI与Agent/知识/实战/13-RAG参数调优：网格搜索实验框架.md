...
**Q1：一句话讲清「RAG 参数调优：网格搜索实验框架」到底是什么？**
A：RAG 调优是科学实验：先定参数空间与指标，再分阶段网格搜索选最优。

**Q2：一、为什么 RAG 需要调优：组合爆炸 —— 怎么理解？**
A：像调一桌菜的三类火候：分块、检索、生成各有好几个旋钮，光 chunk_size×top_k×检索类型就 75 种组合——手动调参等于盲选。英文：Parameter Space。

**Q3：二、分块与检索参数：先测检索再调生成 —— 怎么理解？**
A：像先选对食材再决定炒法：chunk_size 优先试 512、overlap 取 15%~20%；检索先定 top_k 与混合检索，90% 的问题出在检索。英文：Chunking / Retrieval。

**Q4：三、Rerank 与混合检索：粗筛之后要精排 —— 怎么理解？**
A：像招聘先海选再面试：纯向量对专有名词差、BM25 对语义差，混合检索两者兼顾；Rerank 是"精排"，加上后 Precision 从 0.55 飙到 0.78。英文：Hybrid Retrieval / Rerank。

**Q5：四、多阶段网格搜索：把 360 种砍到 42 种 —— 怎么理解？**
A：像分楼层排查故障：每阶段只调 1~2 个参数、固定其余，四阶段锁定分块→检索→Rerank→温度，搜索空间从 360 降到 42，效率提升 8.5 倍。英文：Grid Search / Phased Tuning。
