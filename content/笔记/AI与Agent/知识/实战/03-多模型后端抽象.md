...
A：多模型后端抽象在上层代码和各家 SDK 间插一层，统一请求/响应/错误码，主挂自动切备用。

**Q2：一、为什么要多后端 —— 怎么理解？**
A：生产不能把鸡蛋放一个篮子：一家 API 宕机、涨价或限流，全线瘫痪。铁律是配 2~3 家后端，默认走主模型，挂了自动切备用。上层永远调 call_model(...)，不直接 OpenAI()，换厂商业务代码一行不改。

**Q3：二、统一请求与响应 —— 怎么理解？**
A：各家 OpenAI 兼容请求体一样（model/messages/temperature），只差 api_key/base_url/model 三参数。响应也一致，抽象层收拢成统一 ModelResponse（content/finish_reason/usage）。铁律：上层只看 content，永不直接解析原始 JSON。

**Q4：三、统一错误码 —— 怎么理解？**
A：各家错误码范围不同，但 HTTP 状态码对齐。抽象层映射成统一 BackendError 并标 retryable：400/401/402/422 是客户端错不重试；429 限流、500/503 服务端错才重试。4xx 不重试是铁律。

**Q5：四、重试/回退三板斧 —— 怎么理解？**
A：主模型失败（429/500/503）重试最多 3 次，指数退避（Exponential Backoff，第 n 次等 2ⁿ 秒）+ 抖动（jitter 防惊群）；3 次全挂切备用（如通义千问），备用也挂试备用 2，全失败才报错。陷阱：无 jitter 雪崩、流式难重试、超时别太大。

**Q6：核心速记主线有哪些？**
- 生产至少 2~3 家后端，主挂自动切备用
- 统一请求只差三参数，响应收口 ModelResponse
- 4xx 不重试，429/5xx 才指数退避加抖动
- 重试三板斧：重试主 → 切备用 → 全失败报错
