...
## 五、Tool Annotations（注解）：告诉主机"这工具危险吗"
...
A：MCP 规范对三者"谁说了算"有明确定位，这是理解它们的核心：
| 原语 | 谁控制（who controls） | 本质 | 有无副作用 | 类比 |
|-|-|-|-|-|
| **Tool** | **模型控制**（Model-controlled） | 执行动作 / 函数 | 可能有（改世界） | 菜谱步骤 /  |
...
A：Tool 是 Server 暴露给模型调用的"函数"。一个 Tool 的完整定义字段如下（以 2025-06-18 规范为准，版本差异见 §6）：
逐字段解释（全称 + 通俗）：
**name（名称）**：工具的**唯一标识符**，模型靠它路由调用（如  的 ）。规范建议长度 **1–128 字符、大小写敏感**。

**Q4：四、Resource 定义：一份只读数据长什么样 —— 怎么理解？**
A：Resource 是 Server 暴露的"只读上下文"，通过 **URI（Uniform Resource Identifier，统一资源标识符，类似网址但更通用）** 寻址。
逐字段：
**uri（资源标识）：唯一键**，格式如 （例：、）。Server 据此定位并返回内容。
**name / description**：同 Tool，给人/模型看的名字与说明。

**Q5：五、Tool Annotations（注解）：告诉主机"这工具危险吗" —— 怎么理解？**
A：这是 Tool 专属、也最容易被忽略的字段。四个 hint（提示）描述工具行为，主机（Host）据此决定"要不要弹确认框 / 能不能自动重试"：
| 注解字段 | 含义 | 主机可能的行为 |
|-|-|-|
|  | 只读，不改世界 | 可自动执行，无需确认 |
|  | 破坏性、不可撤销 | **要求用户明确确认** |
|  | 幂等，重复执行安全 | 可安全重试（呼应第 ⑥⑦⑪ 篇） |

**Q6：核心速记主线有哪些？**
A：抓住这几根：二、三种原语的控制权不同、三、Tool 定义：一个工具长什么样（逐字段拆解）、四、Resource 定义：一份只读数据长什么样、五、Tool Annotations（注解）：告诉主机"这工具危险吗"、六、注册模式：怎么把能力"挂"到 Server 上、七、版本差异（准确性红线，务必分清）。


## 相关链接

- 项目实践：[[项目实战/ai-resume笔记/01_技术研读/05_MCP协议#5 Tools 逐行走读|ai-resume: MCP协议]]
