...
A：上下文压缩用结构化摘要提纯信噪比，而非简单截断丢信息。

**Q2：一、为什么压、为什么不能乱压 —— 怎么理解？**
A：像脱水蔬菜而非扔菜：好压缩营养还在体积小，差压缩把关键指令丢了还不自知——压完必须校验，验证和压缩同等重要。英文：Signal-to-Noise Ratio。

**Q3：二、三种压缩形态：任务/文件/过程 —— 怎么理解？**
A：像做菜的三本笔记：任务摘要记"做到哪步了"、文件摘要记"这本食谱讲啥"、过程笔记记"下次别再炒糊"——对象不同、时机不同。英文：Task / File / Process Summary。

**Q4：三、任务与文件摘要：把轨迹和外部文件压成块 —— 怎么理解？**
A：像给 Agent 配"便利贴"：TaskSummary 存目标进度与踩坑，FileSummary 存用途与核心导出——读文件变成读摘要，窗口瞬间省一大截。英文：Structured Summary。

**Q5：四、质量校验与护栏：压坏了早发现 —— 怎么理解？**
A：像出货前质检三关：查关键实体在不在、查事实有无张冠李戴、查压缩比合不合理；否定句禁压、设最大压缩代数防累积误差。英文：Validation Guardrail。

**Q6：核心速记主线有哪些？**
- 三种形态：任务摘要（进度）/ 文件摘要（用途）/ 过程笔记（洞察）
- 核心目标：提高信噪比，不是单纯删字数
- 校验三关：实体完整、事实一致、压缩比合理
- 护栏：否定句禁压、最大压缩代数、数字入白名单
...
- [[笔记/AI与Agent/知识/实战/32-长期记忆：跨会话用户偏好、历史持久化|长期记忆：跨会话用户偏好、历史持久化]]
