...
- Agent 架构 → [[15-Agent架构与工具调用]]
- 项目实践：[[项目实战/ai-resume笔记/01_技术研读/02_RAG流水线#第 3 层：向量化与存储|ai-resume: RAG流水线]]

---

→ [[技术学习清单#Embedding 与语义搜索]]
...
A：[!warning] 版本锁死警告
Chroma **数据库迁移不可逆**——升级版本后无法降级。生产环境必须固定版本号。
Chroma 默认用 （384 维）做自动向量化，需要 Sentence-Transformers：
类比：Chroma 是个"快递站"——你扔进去包裹（文本），它自动贴标签（向量化），塞进货架（索引）。Sentence-Transformers 就是那个贴标签的机器。
...
| 模式 | 类 | 数据住哪 | 适合场景 |
|------|---|---------|---------|
| 内存模式 |  | 进程内存，退出没 | 快速试手 / 单元测试 |
| 持久化模式 |  | 磁盘 SQLite，重启还在 | 个人项目 / 生产 |
| 远程模式 |  | 远端 Chroma 服务器 | 分布式 / 多应用共享 |

**Q4：三、Collection——Chroma 的"表" —— 怎么理解？**
A：Collection 就是 SQL 里的"表"——所有操作都在 Collection 上做。
**Collection 内部长这样：**
| 字段 | 类型 | 说明 |
|------|------|------|
|  | str | 唯一标识，你给或 Chroma 自动生成 |
|  | float[] | 向量（默认 384 维） |
|  | str | 原始文本 |
...
A：[!tip] 批量>单条
逐条 add 约 1000 条/秒，批量 ≥ 100 条约 15000 条/秒——**差 15 倍**。这是被问得最多的性能考点。
你已经有向量了（比如用 OpenAI 的  算好了 1536 维向量），可以直接塞：
 vs ：add = INSERT（重复 ID 报错），upsert = REPLACE（重复 ID 覆盖）。常见陷阱。

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、安装、二、三种客户端模式、三、Collection——Chroma 的"表"、四、入库（写入数据）、五、自定义嵌入函数、六、检索。
