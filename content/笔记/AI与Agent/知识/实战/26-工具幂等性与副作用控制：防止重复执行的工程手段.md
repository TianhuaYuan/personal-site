...
# ============================================================
# 第二部分：模拟幂等去重存储（生产环境用 Redis/PostgreSQL）
# ============================================================
...
**Q1：一句话讲清「工具幂等性与副作用控制的 Python 实现」到底是什么？**
A：**幂等性（Idempotency）**：一个操作 f 满足 ，做了再做一次结果和只做一次一样。读操作天然幂等，写操作要**设计**才幂等。
**网络超时**：工具执行成功了但回复丢失，重试触发再执行一次
**框架默认重试**：LangGraph 默认 3 次指数退避重试
**ReAct 重新走**：模型推理绕回同一步，同一写工具被再调一次
