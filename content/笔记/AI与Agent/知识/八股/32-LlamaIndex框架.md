...
A：一句话：**LlamaIndex 是专门为 LLM 数据索引和检索设计的框架**。
如果说 LangChain 是"瑞士军刀"，LlamaIndex 就是"手术刀"——在数据检索领域更精准、更高效。它**原名 GPT Index**，2023 年更名为 LlamaIndex；
...
A：LlamaIndex 把"建索引"和"查索引"分成两个清晰阶段，这正是它比通用框架更顺手的原因：
索引只在数据变化时重建，查询阶段是无状态轻量推理，因此生产环境可把索引预构建好、查询侧弹性扩容。

**Q5：LlamaIndex 高级功能 —— 怎么理解？**
A：将复杂问题拆分为子问题，分别检索后综合：
根据问题类型自动路由到不同的索引/工具：
检索文档块时，如果相邻块都被检索到，则自动合并为更大的上下文：

**Q6：核心速记主线有哪些？**
A：抓住这几根：LlamaIndex 定位、核心组件、LlamaIndex RAG 数据流、LlamaIndex 高级功能、LlamaIndex vs LangChain 选型、快速问答。
