...
A：你的 Agent 第一天上线表现完美。第二天早上你醒来，发现它**同一个搜索函数调了 47 次**——同样的参数、同样的 URL、同样的空结果——然后才放弃（dev.to Alan West 的真实经历）。
这不是模型变蠢了，是你的 Agent 没有「刹车」。
类比：你让实习生小王去查一份资料。小王去翻文件柜，没找到。**然后又去翻一次**，还是没找到。

**Q2：一、场景：你不会想让你的 Agent 打出 47 次同一个 API 调用 —— 怎么理解？**
A：你的 Agent 第一天上线表现完美。第二天早上你醒来，发现它**同一个搜索函数调了 47 次**——同样的参数、同样的 URL、同样的空结果——然后才放弃（dev.to Alan West 的真实经历）。
这不是模型变蠢了，是你的 Agent 没有「刹车」。
类比：你让实习生小王去查一份资料。小王去翻文件柜，没找到。**然后又去翻一次**，还是没找到。

**Q3：二、核心名词与架构（先定义再展开） —— 怎么理解？**
A：先认几个关键概念，后面不重复解释：
**Agent（智能体）** = LLM + Tools（工具，如搜索、计算、查数据库）+ Orchestration（编排逻辑）。Agent 循环跑：思考→调工具→看结果→再思考。
**死循环（Infinite Loop / Tool-Call Loop）**：Agent 无法收敛到「任务完成」状态，不断重复调用工具而不推进。

**Q4：三、为什么 Agent 会死循环？（三大根源） —— 怎么理解？**
A：不是模型「想」循环，是三个结构性原因让它走不出来：
Agent 不知道「什么时候够了」。
大多数 Agent 循环的逻辑是：「不断调工具，直到模型说它做完了。」当任务边界不清晰、或模型不确定是否够信息时，「我再查一次确认一下」可以无限重复。
现实类比：让你写一份市场分析报告，但不告诉你「几页算完」。你可能会补充第 47 页数据「以防万一」。

**Q5：四、解决方案全景：四层防护，层层兜底 —— 怎么理解？**
A：死循环不是「加一个限制」就能解决的。需要四层：硬上限 → 精确去重 → 语义检测 → 分级降级。每一层解决不同维度的循环。
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、场景：你不会想让你的 Agent 打出 47 次同一个 API 调用、二、核心名词与架构（先定义再展开）、三、为什么 Agent 会死循环？（三大根源）、四、解决方案全景：四层防护，层层兜底、五、防护层一：硬步数上限（Max Iteration Limit）—— 兜底安全阀、六、防护层二：精确循环检测（Exact Loop Detection）—— 拦截「一模一样」的重复。
