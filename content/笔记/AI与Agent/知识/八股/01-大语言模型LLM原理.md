...
A：大语言模型（LLM）是基于 Transformer、在海量文本上预训练的神经网络：预训练让它懂语言，SFT 让它听指令，RLHF/DPO 让它合人意。

**Q2：生命周期：培养助理三阶段 —— 怎么理解？**

A：LLM 训练像培养读了海量书的助理：预训练（通读图书馆积累常识）→ SFT（岗前培训教按指令办事）→ RLHF/DPO（老员工带教让说话合人意），能力目标逐级收窄。

**Q3：Tokenization：BPE 子词 —— 怎么理解？**

A：LLM 先把文本拆成 Token（子词），BPE 从字符级反复合并最高频相邻对直到词表达标（GPT/LLaMA 都用）；子词平衡了词表大小和序列长度，避开词级 OOV 和字符级语义弱。

**Q4：Pre-training：训练目标与 Scaling —— 怎么理解？**

A：预训练学语言基本功：Causal LM（GPT 自回归预测下一 token）、Masked LM（BERT 掩码）、Prefix LM（T5）。Decoder-Only+Causal LM 因适配生成、Scaling Law、训练效率成主流；Chinchilla 主张参数量与数据量同步增长。

**Q5：SFT 与 RLHF/DPO 对齐 —— 怎么理解？**

A：SFT 用「指令-回复」对微调让模型听指令（数据质量>数量、Loss Masking 只在回复算 loss）；RLHF 三阶段（Reward Model→PPO+KL 约束）对齐偏好，DPO 免单独 Reward Model、更稳、效果接近。

**Q6：核心速记主线有哪些？**

- 生命周期：预训练→SFT→RLHF/DPO→部署

- 分词：BPE 子词，平衡词表与序列

- 预训练：Causal/Masked/Prefix LM，Decoder-Only 主流

- Scaling：Chinchilla 参数量与数据量同步增长

- 推理优化：Temperature/Top-p/投机解码/量化/KV Cache

**口诀**

A：预训练懂语言，SFT 听指令；

RLHF DPO 合人意，三级递收窄路径；

Decoder 成主流，Scaling 定律定输赢；
...
- [[笔记/AI与Agent/知识/八股/07-KV-Cache原理与推理优化|KV-Cache原理与推理优化]]
