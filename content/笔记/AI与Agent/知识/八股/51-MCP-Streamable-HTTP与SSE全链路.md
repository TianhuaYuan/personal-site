...
**Q3：HTTP + SSE 模式（传统） —— 怎么理解？**
A：**局限性：**
需要维护两个端点（POST + SSE）
SSE 连接管理复杂
不支持断线重连

**Q4：Streamable HTTP 模式（新） —— 怎么理解？**
A：**核心改进**：单端点处理所有请求，响应可以是普通 HTTP 或 SSE 流。
| 特性 | 说明 |
| ------ | ------ |
| 单端点 | 所有请求发往同一个 URL |
| 可升级响应 | 从普通 HTTP 升级为 SSE 流 |
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：传输层演进、HTTP + SSE 模式（传统）、Streamable HTTP 模式（新）、两种传输方式的选择、0. 先讲人话：传输层 = "信怎么送到"、1. 它是什么 / 为什么需要它。
