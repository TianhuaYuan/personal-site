...
**Q4：中间件链设计 —— 怎么理解？**
A：执行顺序：Logging → Permission → Auth → Handler

**Q5：敏感操作确认 —— 怎么理解？**
A：对于有副作用的工具调用（退款、发邮件等），需要人类确认：
| 策略 | 说明 |
| ------ | ------ |
| 工具级标记 | 在 tool definition 中标记  |
| 运行时拦截 | 中间件拦截标记为敏感的调用 |
| 用户确认 | 通过 Host UI 向用户展示确认对话框 |
| 审计日志 | 记录所有敏感操作的确认和执行 |
---
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：MCP 安全模型、认证架构、中间件链设计、敏感操作确认、0. 开篇：你家的专属电器，也得能插上标准口、1. FastMCP 是什么：把协议细节降到装饰器。
