...
**Q1：一句话讲清「Tokenization与词表」到底是什么？**

A：Tokenization 是把文本切成模型能读的数字编号序列，词表是这些编号的集合。

**Q2：一、核心概念 —— 怎么理解？**

A：像你发"你好"，AI 实际收到的是 [5831, 12084] 两个整数编号——每个编号就是一个 Token。模型不认识字只认识数字，Tokenization 负责把文字翻译成编号，词表(Vocabulary)就是允许的编号全集。

**Q3：二、为什么不用字符或词 —— 怎么理解？**

A：像乐高选块大小：字符级块太小(单字)，拼起来太费劲序列长；词级块太大(整词)，拼不出新词且易缺件(OOV)；子词(subword)刚好——常见词整体保留，罕见词拆成有意义片段，平衡效率与语义。

**Q4：三、主流分词算法 —— 怎么理解？**

A：像三种拼乐高策略：BPE 按频率合并最高频相邻对(GPT/LLaMA 用)；WordPiece 按互信息(PMI)合并、更重语义(BERT 用，## 标非词首)；Unigram 自顶向下用 EM 删最弱子词(SentencePiece)。SentencePiece 工具库语言无关、可逆。

**Q5：四、词表与中文代码挑战 —— 怎么理解？**

A：像选词典厚度：小(32K)Embedding 省但序列长，大(128K+)序列短但稀有 token 训练不足。中文无词边界，繁简/中英混合/单字被拆是难点；代码需专门代码词表+字节级 BPE。tiktoken 是 OpenAI 高效 BPE 分词器。

**Q6：核心速记主线有哪些？**

- 本质：文本→数字编号，模型只认编号不认字

- 三方案：字符级/词级/子词级，子词平衡最优

- 三算法：BPE(频率)/WordPiece(互信息)/Unigram(SentencePiece)

- 工程：词表大小权衡、中文代码挑战、tiktoken 计费
...
BPE 按频率，WordPiece 互信息；

Unigram 自顶落，SentencePiece 中日亲。
