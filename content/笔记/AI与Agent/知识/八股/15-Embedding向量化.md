...
A：**核心思想：** 将高维稀疏的文本表示（如 one-hot）映射到低维稠密的向量空间，使得**语义相似的文本在向量空间中距离相近**。

**Q2：什么是 Embedding？ —— 怎么理解？**
A：**核心思想：** 将高维稀疏的文本表示（如 one-hot）映射到低维稠密的向量空间，使得**语义相似的文本在向量空间中距离相近**。

**Q3：Embedding 模型分类 —— 怎么理解？**
A：| 类型 | 代表模型 | 特点 |
| ------ | --------- | ------ |
| Bi-Encoder | BERT, BGE, E5 | Q 和 D 独立编码，速度快，适合检索 |
| Cross-Encoder | BERT-cross-encoder | Q 和 D 联合编码，精度高，适合重排序 |
...
| 1024 | 高质量检索 | BGE-large 维度，推荐 |
| 3072 | 最高精度 | OpenAI large 维度，成本高 |
**经验法则：** 维度翻倍，精度提升有限但存储和计算成本翻倍。

**Q6：核心速记主线有哪些？**
A：抓住这几根：什么是 Embedding？、Embedding 模型分类、相似度度量、维度选择、Embedding Fine-tuning 微调、常见问题与最佳实践。
