...
**Q1：一句话讲清「Embedding向量化」到底是什么？**

A：Embedding 是把文本变成高维向量，让意思相近的文字在向量空间里挨着、意思无关的分得远。

**Q2：语义坐标卡类比 —— 怎么理解？**

A：Embedding 像给每句话发一张「语义坐标卡」——意思相近坐标近，八竿子打不着的坐标远。检索时只比坐标距离，不必真读懂每个字。好比把词看成地球上的点，"北京"和"上海"坐标近，"北京"和"巴黎"远；你搜"首都"找到的就是北京，哪怕没提"北京"二字。

**Q3：三步变向量像「拍照取景」 —— 怎么理解？**

A：① Tokenizer（分词器）把文字变数字 ID；② Transformer Encoder（编码器）用 Self-Attention 让每个词"看"上下文，叠 N 层融合全局；③ Pooling（池化）把多个词向量合并成一个，再 L2 Normalize 归一化成单位向量。Embedding 模型不做"理解"只做数学变换，和 LLM 同架构但去掉预测头换上池化层。

**Q4：余弦 vs 欧氏，买东西秒懂 —— 怎么理解？**

A：顾客 A 买 10 件衣服 0 本书、顾客 B 买 1 件衣服——余弦相似度=1.0（都是"衣服党"），欧氏距离=9（买的量差很多）。余弦只看"你是不是那类人"（方向），欧氏看"离我多远"（绝对值）。归一化后三者排序等价，但余弦值域固定[-1,1]最好设阈值。

**Q5：对称 vs 非对称，RAG 的坑 —— 怎么理解？**

A：对称场景查询≈文档（句子 vs 句子）；非对称如 RAG 检索，短查询"怎么退款"对长文档"退款流程…"，长度差几十倍。对称模型下所有长文档向量都"又大又长"彼此天然相似，淹没正负差距；非对称模型用 query/document 分离编码（如 BGE 加 instruction 前缀、阿里云 text_type）消除偏差。双塔（Bi-Encoder）是其物理实现。

**Q6：核心速记主线有哪些？**

- 本质：语义坐标卡，近义挨着反义远

- 推理三步：分词→Transformer 融合→池化归一

- 度量：余弦看方向、欧氏看距离，归一化后等价

- 对称非对称：RAG 用非对称，双塔分离编码

**口诀**

A：Embedding 坐标卡，近义挨着远义分；
