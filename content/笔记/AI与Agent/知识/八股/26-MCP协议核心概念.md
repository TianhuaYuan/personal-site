...
A：每个 AI 应用需要为每个工具/数据源编写专门的集成代码：
**一次开发，处处可用。** MCP Server 开发者只需实现标准协议，所有支持 MCP 的 AI 应用都能直接使用。
...
**Q4：MCP 三大核心能力 —— 怎么理解？**
A：类似 Function Calling，允许 LLM 调用外部函数。
提供对数据源的**只读访问**，类似 GET 请求。
| 维度 | Resources | Tools |
| ------ | ----------- | ------- |
| 操作 | 只读 | 读写都可 |
...
**Q5：MCP vs Function Calling —— 怎么理解？**
A：| 维度 | Function Calling | MCP |
| ------ | ----------------- | ----- |
| 标准化 | 各厂商 API 不同 | 统一开放协议 |
| 发现机制 | 静态定义 | 动态发现（运行时获取工具列表） |
| 传输方式 | HTTP API | stdio / HTTP + SSE |
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：为什么需要 MCP？、MCP 架构、MCP 三大核心能力、MCP vs Function Calling、实际 MCP 示例、0. 先别急着记定义，看一个生活场景。
