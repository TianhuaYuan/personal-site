...
**Q1：一句话讲清「MCP 协议核心概念」到底是什么？**

A：MCP 是 AI 工具接入的"标准插槽"，统一 LLM 与外部能力连接。

**Q2：一、为什么需要 MCP —— 怎么理解？ —— 怎么理解？**

A：像每家电器各搞一种充电口，N 个应用×M 个工具要写 N×M 套适配；MCP 把它统一成 USB-C，一次开发处处可用。英语：N×M integration problem（N×M 集成问题）。

**Q3：二、三角色：Host/Client/Server —— 怎么理解？ —— 怎么理解？**

A：像租房中介：Host 是门店（AI 应用），Client 是门店里对接你的业务员，Server 是握着真房源的房东；Host 经 Client 连 Server 取能力。英语：Host / Client / Server。

**Q4：三、三原语：Tools/Resources/Prompts —— 怎么理解？ —— 怎么理解？**

A：像智能厨房三件套：Tool 是"做"（手，有副作用、模型控），Resource 是"读"（眼，只读、应用控），Prompt 是"套话"（词典、用户控）。英语：Primitives（原语）。

**Q5：四、MCP 不取代 Function Calling —— 怎么理解？ —— 怎么理解？**

A：像蓝牙协议 vs 手机语音助手：Function Calling 是"这台机器怎么表达调工具"，MCP 是"不同机器怎么互相连"；Server 内部仍用 FC 思想，互补不替代。英语：Function Calling vs Protocol layer。

**Q6：核心速记主线有哪些？**

- MCP=AI 工具接入的"标准插槽"（USB-C）

- 三角色：Host / Client / Server

- 三原语：Tool(做)/Resource(读)/Prompt(模板)

- 与 Function Calling 互补，不取代

**口诀**

A：MCP 统一插槽，开发一次处处用；

Host 门店 Client 连，Server 露真功。

Tool 出手 Resource 看，Prompt 套路供；
...
- [[笔记/AI与Agent/知识/八股/50-MCP认证中间件与安全模型|MCP认证中间件与安全模型]]
