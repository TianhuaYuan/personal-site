...
A：Lost in the Middle：长上下文里答案放中间时模型最答不好，首尾最强呈 U 型曲线。

**Q2：一、现象与来源 —— 怎么理解？**
A：背单词表首尾记得牢、中间模糊——LLM 同款：答案放中间正确率掉 20~30%，论文 Liu 2023 命名（U-shaped Curve）。

**Q3：二、为什么会这样 —— 怎么理解？**
A：因果注意力掩码让开头被全程看到、RoPE 距离衰减让中间掉"死亡地带"，训练数据又把重要信息放首尾（Causal Mask + RoPE）。

**Q4：三、新模型治好了吗 —— 怎么理解？**
A：Gemini/Claude/GPT 长上下文 U 型更平，但真实多事实任务里中部仍低 5~20%——工程结论没变（Flatter not gone）。

**Q5：四、怎么缓解 —— 怎么理解？**
A：rerank 后把最相关文档放首尾（U 型摆放）、Cross-Encoder 精排留 3~5 篇、Query 放最前、长文压缩（U-shaped Reorder）。

**Q6：核心速记主线有哪些？**
- 现象：首尾强、中间弱，U 型曲线
- 成因：因果掩码 + RoPE 衰减 + 位置先验
- 新模型：更平但真实任务仍在
- 缓解：U 型重排首尾放重要 + 精排 + 压缩
...
- [[笔记/AI与Agent/场景系统设计/八股/01-RAG系统设计|RAG系统设计]]
- [[笔记/AI与Agent/场景系统设计/八股/08-RAG知识库系统设计|RAG知识库系统设计]]
