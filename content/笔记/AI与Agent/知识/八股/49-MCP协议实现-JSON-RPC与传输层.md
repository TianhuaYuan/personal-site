...
A：MCP 用 JSON-RPC 2.0 通信，支持 stdio 与 HTTP+SSE 两种传输。

**Q2：MCP 握手与生命周期 —— 怎么理解？**

A：像拨通电话先报身份：Client 先发 initialize（协议版本+能力声明），Server 回能力清单，Client 再发 initialized 通知，握手完成才进入就绪、才能 tools/list 与 tools/call。

**Q3：传输方式 —— 怎么理解？**

A：像本地内线 vs 长途网络：stdio（标准输入输出，本地进程通信、安全不需联网）；HTTP+SSE（HTTP POST 发请求、Server-Sent Events 收响应流，适合远程多客户端）。

**Q4：JSON-RPC 2.0 规范 —— 怎么理解？**

A：像统一格式的明信片：每条消息含 jsonrpc、method（要干啥）、params（参数）、id（唯一编号，回信原样带回好配对）。无 id 的是通知（notification），Server 不回。

**Q5：MCP 开发入门 —— 怎么理解？**

A：像照着模板开插座：Server 端用 @list_tools 暴露工具、@call_tool 执行；Client 端在 Claude Desktop 配置 mcpServers 指定 command/args/env，连上即可动态发现并调用工具。

**Q6：核心速记主线有哪些？**

- MCP 通信：JSON-RPC 2.0 消息格式

- 传输：stdio 本地 / HTTP+SSE 远程

- 握手：initialize→能力交换→initialized

- 开发：Server 暴露工具，Client 配置连接

**口诀**

A：MCP 打电话，先握手报身份；

JSON-RPC 写明文，id 配对不丢信；

stdio 本地稳，HTTP 远程行；

Server 露工具，Client 配连通。
