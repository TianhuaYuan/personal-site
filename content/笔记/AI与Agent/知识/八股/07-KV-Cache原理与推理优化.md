...
A：KV Cache 把自回归生成中已算的 K/V 缓存起来避免重复计算，是 LLM 推理提速与省显存的关键。

**Q2：KV Cache 原理 —— 怎么理解？**

A：像记笔记不重算：每生成一个 token 本要重算所有位置的 K/V（O(n²d)）；缓存后每步只算新 token 的 Q/K/V（O(nd)），重复活不干第二遍。

**Q3：FlashAttention 原理 —— 怎么理解？**

A：像把活儿在桌上算完再归档：标准 Attention 要把完整 N×N 矩阵写进慢速 HBM；FlashAttention 用分块（Tiling）+ kernel 融合把中间结果留 SRAM，提速 2–4x、显存降到 O(N)。

**Q4：其他推理优化技术 —— 怎么理解？**

A：像给推理装不同省电模式：量化（FP16→INT8/INT4）减显存；稀疏/线性注意力降计算量；投机解码用小模型草拟大模型验证提速；Continuous Batching 提吞吐。

**Q5：快速问答 —— 怎么理解？**

A：像考前速记卡：KV Cache 显存 = 2×层数×头数×序列长×d_head×字节（LLaMA-2 7B 约 2GB）；减显存靠 MQA/GQA、量化、动态淘汰。复杂度从 O(n²d) 降到 O(nd)。

**Q6：核心速记主线有哪些？**

- KV Cache 缓存已算 K/V，计算量从 O(n²d) 降到 O(nd)

- KV Cache 显存大（长序列数 GB），成推理瓶颈

- FlashAttention 用 IO 感知分块计算，提速 2–4x、显存 O(N)

- 其他优化：量化、稀疏/线性注意力、投机解码、Continuous Batching

**口诀**

A：KV 缓存不重算，O(n²) 降 O(n)；

显存吃紧数 GB，长序成瓶颈；

Flash 分块留 SRAM，提速四倍显存轻；
...
- [[笔记/AI与Agent/知识/八股/60-LoRA微调|LoRA微调]]
