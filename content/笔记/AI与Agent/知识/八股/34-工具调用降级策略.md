...
**Q1：一句话讲清「一、先讲大白话：什么是「降级」，和重试/熔断有何不同？」到底是什么？**
A：| 触发条件 | 说明 | 典型阈值 |
|-|-|-|
| **显式失败** | 工具调用抛出异常 / 返回错误码 | 任何异常 |
| **超时** | 响应超过预期时间 | P95 延迟 + 2σ（如 >3s） |
...
**Q3：6.1 现状盘点（真实代码） —— 怎么理解？**
A：| 能力 | 项目真实实现 | 评价 |
|-|-|-|
| per-feature 降级 |  多查询并行时部分失败不阻断； rerank 失败降级为原始顺序； 改写失败回退直接搜索； 反思失败兜底 JSON | ✅ 做得很扎实 |
| 披露（Disclosure） |  检索失败时注入降级说明；
...
**Q5：核心速记主线有哪些？**
A：抓住这几根：3.1 降级触发条件（什么时候该降？）、6.1 现状盘点（真实代码）、。
