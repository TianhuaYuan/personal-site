...
**Q1：一句话讲清「工具调用降级策略」到底是什么？**
A：降级是某环节失败时切到次优但可用的方案先把结果交付，并明确告知用户这是「80 分版」。

**Q2：餐厅招牌菜卖光了 —— 怎么理解？**
A：重试=让后厨再翻冰柜（治偶发抖动）；熔断=今天供货断了别做了（治持续宕机）；降级=换上红烧肉次优替代（治功能缺失但能交付）；硬失败=直接说不卖（但当有安全隐患必须硬失败）。降级（Degradation，又名 Fallback 退路）核心：给不出满分就给 80 分，但让用户知道这是 80 分版。

**Q3：per-feature 降级，一个坏不影响其他 —— 怎么理解？**
A：Agent 每个环节都可能单独出问题——检索挂了 LLM 还能靠自身知识答、Rerank 超时还能按原始顺序返回、主模型宕机还能换备用模型。核心原则 per-feature 降级：每个功能有自己的 fallback，Search 挂了不该让整个 Agent 瘫掉。这是 Agent 韧性设计第一原则。

**Q4：多级 fallback 链像「一层层兜底」 —— 怎么理解？**
A：生产级降级是多级链条而非二分：模型级（主 DeepSeek/Claude→后备通义/GPT-4o mini→本地 Ollama）；检索级（向量库稠密→BM25 稀疏→最近缓存→查询重写）；生成级（完整流程→关 Rerank→关 Reflexion→静态话术含 fallback_reason）。每层只往更保底走，恢复反向回升不跳级。

**Q5：披露比技术更重要 —— 怎么理解？**
A：很多团队写满降级代码却忘了说一声，这就是静默降级（Silent Degradation）——检索挂了模型硬答无来源，用户拿到看似完整实则是幻觉的报告，信任崩塌。正确做法 Disclosure：明确告知"检索不可用，本次基于模型自身知识"。支付/PII/权限类必须硬失败不能降级（hard-fail allowlist）。
