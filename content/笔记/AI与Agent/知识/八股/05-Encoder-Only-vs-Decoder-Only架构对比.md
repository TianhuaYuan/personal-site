...
A：| 特性 | Encoder-Only (BERT) | Decoder-Only (GPT) | Encoder-Decoder (T5) |
| ------ | -------------------- | -------------------- | ---------------------- |
...
**Q3：注意力流向对比 —— 怎么理解？**
A：实线=可关注，虚线=被掩码禁止。这正是 BERT 擅长「理解」、GPT 擅长「生成」的结构根因。

**Q4：Decoder-Only 架构详解 —— 怎么理解？**
A：**简洁统一**：一个架构解决所有生成任务
**Scaling 友好**：decoder-only 架构在参数量增大时性能提升更稳定
**自回归天然适配**：语言建模本质就是预测下一个 token
因果注意力掩码确保在预测第 t 个 token 时，模型只能看到前 t-1 个 token：
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：架构类型总览、注意力流向对比、Decoder-Only 架构详解、GPT 系列演进、BERT 与 Encoder 架构、架构选择指南。
