...
A：分布式训练把模型拆到多 GPU，解决单卡显存与算力不够。

**Q2：一、为什么要多卡 —— 怎么理解？ —— 怎么理解？**

A：像一个人搬不动大沙发：70B 模型训练要 1.1TB 显存，单卡 80GB 根本装不下，只能叫上多卡分工。英语：memory bottleneck（显存瓶颈）。

**Q3：二、数据并行 —— 怎么理解？ —— 怎么理解？**

A：像几个人抄同一本书各做一章题：每卡持完整模型、分不同数据算梯度，再 AllReduce 求平均同步。通信量是痛点。英语：Data Parallelism / AllReduce。

**Q4：三、模型并行（张量+流水线） —— 怎么理解？ —— 怎么理解？**

A：像大件家具拆着扛：张量并行把一层算符切到多卡（Megatron 列/行切），流水线并行把不同层分给不同卡，但会有"气泡"空转。英语：Tensor / Pipeline Parallelism。

**Q5：四、ZeRO 与 FSDP 省显存 —— 怎么理解？ —— 怎么理解？**

A：像合租分摊房租：ZeRO 把参数/梯度/优化器切片到各卡（3 阶段逐步省 N 倍），FSDP 是 PyTorch 原生 ZeRO-3 实现。英语：ZeRO / FSDP（Fully Sharded Data Parallel）。

**Q6：核心速记主线有哪些？**

- 动机：单卡显存算力不够，需多卡

- 数据并行：每卡全模型+AllReduce 同步

- 模型并行：张量切层+流水线分层（有气泡）

- ZeRO/FSDP 切片状态，省 N 倍显存
