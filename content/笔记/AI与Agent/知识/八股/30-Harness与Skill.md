...
A：**定义：** Harness 是包裹在 LLM 外层的"控制框架"，包含 System Prompt、工具定义、约束规则和安全护栏。

**Q2：什么是 Agent Harness？ —— 怎么理解？**
A：**定义：** Harness 是包裹在 LLM 外层的"控制框架"，包含 System Prompt、工具定义、约束规则和安全护栏。

**Q3：Harness 的四大组件 —— 怎么理解？**
A：System Prompt 是 Harness 的核心，决定了 Agent 的"人格"和行为模式。
**组成要素：**
| 要素 | 说明 | 示例 |
| ------ | ------ | ------ |
| 角色定义 | Agent 是谁 | "你是一位资深后端工程师" |
...
**Q4：什么是 Skill？ —— 怎么理解？**
A：**定义：** Skill 是对 Agent 可执行能力的抽象封装，包含输入/输出定义、实现逻辑和评估标准。
| 维度 | Tool | Skill |
| ------ | ------ | ------- |
| 复杂度 | 简单函数调用 | 可能包含多步骤流程 |
| 组合性 | 原子操作 | 可组合多个 Tool |
...
**Q5：Prompt Engineering vs Harness 设计 —— 怎么理解？**
A：| 维度 | Prompt Engineering | Harness 设计 |
| ------ | ------------------- | ------------- |
| 关注点 | 单次输入输出优化 | 整体系统架构 |
| 复杂度 | 低 | 高 |
| 涉及组件 | 仅 Prompt | Prompt + Tools + Memory + Constraints |
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：什么是 Agent Harness？、Harness 的四大组件、什么是 Skill？、Prompt Engineering vs Harness 设计、安全护栏 (Safety Guardrails)、评估循环 (Evaluation Loop)。
