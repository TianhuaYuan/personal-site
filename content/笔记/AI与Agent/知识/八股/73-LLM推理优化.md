...
A：LLM 推理优化的核心挑战是：**模型巨大（数十亿到万亿参数）、内存占用高、生成速度慢、成本高昂**。
优化目标：在保持模型质量的前提下，提升吞吐量、降低延迟、减少显存占用。
...
**Q3：推理性能瓶颈分析 —— 怎么理解？**
A：LLM 推理分为两个阶段，各有不同的性能瓶颈：
| 阶段 | 特点 | 瓶颈 |
| ------ | ------ | ------ |
| Prefill（预填充） | 处理整个输入 prompt，计算所有 token 的 KV | **计算密集型**（Compute-bound） |
| Decode（解码） | 逐 token 生成，每步只处理一个新 token | **内存密集型**（Memory-bound） |

**Q4：KV Cache（关键值缓存） —— 怎么理解？**
A：在 Transformer 的自回归生成中，每生成一个新 token，都需要与之前所有 token 计算注意力。但之前 token 的 Key 和 Value 不会改变，因此可以缓存起来避免重复计算。
Decode 阶段显存带宽成为瓶颈：每生成一个 token 都要从显存搬回整个 KV Cache，而计算量极小。因此 KV Cache 的容量与访问效率直接决定长上下文推理的可行性与速度。

**Q5：模型量化（Quantization） —— 怎么理解？**
A：将模型参数从高精度（FP32/FP16）转换为低精度（INT8/INT4），减少内存占用和计算量。
| 精度 | 每参数字节数 | 相对 FP16 | 适用场景 |
| ------ | ------------ | ---------- | --------- |
| FP32 | 4 bytes | 1x | 训练 |
| FP16/BF16 | 2 bytes | 0.
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：核心概念、推理性能瓶颈分析、KV Cache（关键值缓存）、模型量化（Quantization）、推测解码（Speculative Decoding）、Continuous Batching（连续批处理）。
