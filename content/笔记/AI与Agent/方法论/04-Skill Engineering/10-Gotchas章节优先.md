...
A：**在 Skill 文档里，Gotchas（已验证的失败模式）应作为一等章节、放在靠前位置**——经验证实的避坑知识，权重高于理论指导。
讲「Gotchas 为什么值钱」，这章讲「把它摆到 C 位」。就像工厂把安全须知贴在机器正对面、而不是夹在操作手册第 20 页——Agent 一打开 Skill 先看到坑在哪，比读完一长串「应该怎么做」再凭记忆躲坑靠谱得多。
---
...
A：Gotchas 章节优先 = 在 Skill 结构里，把「失败处理」提升为靠前的一等章节，且按场景 / 优先级组织，而非塞在文末的附录。
Agent 是概率性的：开头的强提醒比结尾的附录更容易被「带进上下文」并执行；
经验 > 理论：验证过的坑比抽象教条更防错（见 ）。
**按场景**：输入相关（空 diff）、工具相关（mypy 未装）、输出相关（报告脱敏）。
**按优先级**：🔴 高频必放最前，🟢 低频可后置甚至移入 。

**Q4：Gotchas —— 怎么理解？**
A：diff 为空：通常因忘记 git add，先提醒而非返空报告
文件 >100 个：分批处理
mypy 未装：跳过类型检查
ruff 未装：跳过风格检查
报告过长：分页
...
A：**上下文是预算（一手）**：Anthropic 在《Claude Code Best Practices》强调「上下文窗口填满后性能下降」，把高频、确定性提醒前置，正是对有限预算的优化（）。
**让错误「结构性不可能」（一手）**：HumanLayer 在 Harness Engineering 中给出核心原则——「每次 Agent 犯错，就把它工程化成永不重犯的结构」。Gotchas 章节优先正是这一原则的 Skill 层落地（）。
