...
A：**Skill 默认「睡着了」：只在被触发时才加载进上下文，而不是启动时全量灌入。** 两种触发方式—— 自动匹配 vs 用户显式  调用。
办公室里的专家平时不在会上刷存在感，你一提到相关问题他才进来；你也可以直接点名叫他。Skill 就是这位专家：启动时只有它的「简历摘要」（name+description）在系统提示里挂着，正文和深度资料都睡在硬盘上，触发才醒。
...
A：延迟加载（Lazy Loading）= Skill 仅在触发时读取  乃至 ，平时只占极小的元数据额度。
**官方额度**：Level 1 元数据约 **100 token/个**；Level 2 正文 **<5k token**；Level 3 资源实质无限（）。

**Q4：三、最新研究与企业数据（2025–2026） —— 怎么理解？**
A：**三层 token 成本（一手）**：Level 1 元数据约 100 token/个 Skill；Level 2 正文 <5k token；Level 3 资源无限（）。这解释了「为什么装 50 个 Skill 也不爆上下文」。
