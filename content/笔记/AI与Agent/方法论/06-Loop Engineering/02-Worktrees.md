...
A：**Worktrees（工作树隔离） = 给每个 Agent 发一张独立工位桌**：多 Agent 并行跑，谁也碰不到谁的文件，最后再把成果合并。
没有它，三个 Agent 同时改 ，就像三个人在同一张草稿纸上抢着写字——最后那页纸没人认得。
---
...
**Q3：二、定义与原理 —— 怎么理解？**
A：**Worktree** 借用 Git 的能力，让同一个仓库同时检出多个分支、各自放在独立目录。每个 Agent 领一个 worktree，等于拥有「隔离的工作副本」：
**隔离工作目录**：A 改 ，B 改 ，互不踩脚。
**并行不阻塞**：A 跑飞了，不影响 B 的交付。
**可丢弃**：实验失败，直接 ，主仓库干干净净。

**Q4：三、实践与示例 —— 怎么理解？**
A：Git worktree 本身就是 Git 原生能力，零依赖：
工程要点（来自 Harness 实践）：worktree 必须配合**权限边界**才安全——Agent 在 worktree 内可读可写，worktree 外只读（见 ）。HumanLayer 在 Harness Engineering 中把「隔离执行环境」列为可靠 Agent 的底座之一（）。
---

**Q5：四、优势与局限 —— 怎么理解？**
A：✅ **并行提速**：N 个 Agent 同时开干，吞吐近似线性增长。
✅ **故障隔离**：一个 worktree 炸了，主仓库和其它 Agent 不受影响。
✅ **干净回滚**：去掉 worktree 即撤销实验，不留尾巴。
❌ **合并冲突仍可能发生**：worktree 防的是「运行中互踩」，不防「合并时冲突」——交叉改同一文件迟早要解冲突。
