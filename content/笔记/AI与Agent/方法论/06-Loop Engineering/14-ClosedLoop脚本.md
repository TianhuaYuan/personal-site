...
**Q1：一句话讲清「Closed Loop 脚本」到底是什么？**
A：Closed Loop 脚本是 Loop 最简闭环：触发→执行→验证→停止四步成环，有头有尾才是闭环而非开口乱跑，把验证/预算/爆炸半径真正串起来。

**Q2：核心观点：做菜四步 —— 怎么理解？**
A：闭环=触发（开火）→执行（炒）→验证（尝咸淡）→停止（关火装盘），缺一不可——开口 Loop 只触发不验证不停止，就是 Loop 高风险的温床。

**Q3：定义与原理：四阶段 —— 怎么理解？**
A：触发（事件/定时/手动）、执行（Agent 调工具产出）、验证（用确定性 verifier 如测试/lint/typecheck 判通过）、停止（通过交付、未过带反馈重试、预算耗尽则停）——四阶段环环相扣。

**Q4：实践：带预算护栏的示例 —— 怎么理解？**
A：最小闭环里 `budget.check()` 放循环最前（先刹车后油门），再 generate/execute，verify 通过就 return 收敛；注意验证用代码 verify 而非「Agent 说好了」，呼应终止条件机器可检查。

**Q5：优势、局限与最新研究 —— 怎么理解？**
A：优势是最小可控、可组合（每阶段可替换）；局限是只是骨架（生产还要叠 worktree/子 Agent/断路器/看门狗）、验证器是瓶颈。Anthropic 提醒多数「触发→执行→验证」用确定性 workflow 即可，不必请 LLM。

**Q6：核心速记主线有哪些？**
- 核心：触发→执行→验证→停止四步成环
- 关键：验证必须机器可检查，预算检查放最前
- 定位：最小可控骨架，生产还要叠 worktree/断路器/看门狗
- 瓶颈：验证器差，闭环只会高效做错事
- 研究：Anthropic 多数三步可被确定性 workflow 替代
