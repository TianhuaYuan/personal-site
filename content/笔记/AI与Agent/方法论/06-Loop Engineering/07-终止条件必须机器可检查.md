...
A：Loop 的终止条件必须是代码层面的确定性属性，绝不能让 LLM 自己判断「干完了吗」。

**Q2：一、核心观点 —— 怎么理解？**
A：让醉汉自己决定「我还清醒吗」，答案永远是「清醒，再来一杯」。把「该不该停」交给 LLM，等于把刹车片交给了想继续开车的醉汉。终止条件的铁律：pass 信号只能来自代码。

**Q3：二、四个退出机制 —— 怎么理解？**
A：Loop 有四种独立的退出机制（Exit Mechanism，确定性的停止条件），全都不问模型：①verifier 绿（验证器测试/lint 通过）；②步数到顶 MAX_STEPS；③连续无进展 no-progress；④预算耗尽 budget。其中无进展检测是兜底闸。

**Q4：三、为什么问模型必翻车 —— 怎么理解？**
A：自评不可靠：让 Agent 改自己的卷，它永远给自己打及格。会「伪收敛」——第 3 步就卡住，却每步自信说「快好了」。无进展检测（no-progress，连续 N 步状态不变强制停）是最后一道不问模型的闸。

**Q5：四、血的教训 —— 怎么理解？**
A：Kusireddy 的多 Agent 系统两个 Agent 互相要澄清，跑 11 天烧掉 $47,000（美元），根因第一条就是「没有终止条件」。团队有日志有监控却没硬上限——观测是目击者，不是断路器（Circuit Breaker，自动跳闸的保护）。

**Q6：核心速记主线有哪些？**
- 终止条件必须是 Loop 状态的确定性属性，绝不让 LLM 自判
- 四机制：verifier 绿 / 步数到顶 / 无进展 / 预算耗尽，全不问模型
- 问模型 = 回到 vibes（凭感觉），必然伪收敛
- $47K 事故根因：无终止条件，观测≠断路器
