...
**Q1：一句话讲清「存活检查」到底是什么？**
A：存活检查 = 让 Loop 定时报平安，长时间没心跳就判定已死，该重启或告警。

**Q2：一、核心观点 —— 怎么理解？**
A：像 ICU 监护仪：没波浪线不是病人乖，是没气了。心跳由 Loop 自己写，但判死必须由 Loop 之外的进程做——否则卡死时连心跳都不写，自己查自己永远"活着"。

**Q3：二、定义与原理 —— 怎么理解？**
A：存活检查是检测 Loop 是否还"活着"的机制，和断路器互补：断路器管"是不是在兜圈子"，存活检查管"是不是还动得了"。三动作：写心跳、外部 watchdog 检心跳、恢复。

**Q4：三、实践与示例 —— 怎么理解？**
A：心跳就是写/读带时间戳的 json：write_heartbeat 写 ts+status，is_alive 比当前时间与上次差是否 ≤ timeout。像外卖员每步打卡，超时不打卡平台就派别人。

**Q5：四、优势与局限 —— 怎么理解？**
A：优势：静默失败可见、可自治恢复；局限：假活（还在写心跳但已无进展，需叠无进展检测）、timeout 难定（太短误判、太长发现太晚）。像打卡机只能证明人到，不能证明在干活。
...
- 同系列：[[12-断路器|断路器]] · [[15-看门狗熔断|看门狗熔断]] · [[14-ClosedLoop脚本|Closed Loop 脚本]]
