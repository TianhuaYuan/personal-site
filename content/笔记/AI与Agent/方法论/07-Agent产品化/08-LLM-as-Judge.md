...
A：**LLM-as-Judge（用大模型当裁判）**：拿一个强 LLM 去给另一个模型的输出打分、挑刺、给反馈，是规模化评估 Agent 的利器；但它有**位置偏见、冗长偏见、自我提升偏见**三大 known issues，必须配缓解手段，不能盲信。
---
...
**Q3：二、定义与原理 —— 怎么理解？**
A：**LLM-as-Judge**：用一个 LLM 充当评估者（judge），对候选输出做**成对比较（pairwise）**或**单答打分（single-answer grading）**
**核心动作**：给分数、找漏洞、给理由
**作用**：把"人工评测"变成可批量、可复现的自动化流程
**类比——请名师阅卷**：你出了一堆作业（Agent 输出），自己逐份看太慢，就请一位水平更高的老师（judge LLM）来批。

**Q4：三、实践：优势与局限 —— 怎么理解？**
A：**自动化 & 可扩展**：批量、并行，成本远低于人力
**一致性**：同一 prompt 下标准稳定、可复现
**可解释**：要求 judge 输出理由，便于追溯
| 偏见 | 表现 | 缓解 |
|------|------|------|
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一、核心观点、二、定义与原理、三、实践：优势与局限、四、示例、五、优劣势、六、核心要点。
