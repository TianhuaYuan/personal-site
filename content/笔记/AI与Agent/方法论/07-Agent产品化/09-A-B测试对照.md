...
A：A/B 测试对照控制变量只改一个因素，两组跑同批任务对比，验证新方案是否真的更好。

**Q2：一、核心观点 —— 怎么理解？**

A：像同菜换种盐盲品：只换盐不动厨子火候，才分得清是盐的功还是厨子的功（Control Variable，控制变量）。

**Q3：二、定义与原理 —— 怎么理解？**

A：像娇气的考试：模型有随机性（temperature>0），单次会抖，每组要跑 N 次取均值加置信（Statistical Significance，统计显著性）。

**Q4：三、实践：三类典型对照 —— 怎么理解？**

A：像三种擂台赛：加载 Skill 与否、旧对对新 Skill、不同模型，看成功率与成本（A/B Test，A/B 测试）。

**Q5：四、示例 —— 怎么理解？**

A：像置信区间不重叠：不加载 88% 加减 3 对加载 93% 加减 2，可较有把握说"加载真有用"（Confidence Interval，置信区间）。

**Q6：核心速记主线有哪些？**

- 核心观点：控制变量，两组跑同批任务

- Agent 要跑多次：单次随机性会骗人，取均值加置信区间

- 三类对照：加载 Skill 与否 / 旧对新 / 不同模型

- 指标来自上下游：成功率、judge 分当因变量
