...
> 一句话：要"可换模型的开源轻量助手"，用 OpenCode；要"派活给能跑全流程的同事"，看 [[01-Claude Code特性|Claude Code]]；要"下工单给并行团队"，看 [[03-Codex特性|Codex]]。
...
- **进阶**：[[07-MCP协议|MCP 协议]]——OpenCode 的工具层走 MCP；[[01-Claude Code特性|Claude Code 特性]]——对比闭源旗舰
...
- 同主题：[[01-Claude Code特性|Claude Code 特性]] · [[07-MCP协议|MCP 协议]]
...
**Q3：二、定义与原理 —— 怎么理解？**
A：**OpenCode**：由 Anomaly（即原来的 SST 团队）维护的开源终端 AI 编程 Agent，MIT 许可
**运行方式**：终端 TUI（Bubble Tea 风格）、桌面 App，也可通过其他界面驱动
**核心特点**：**模型无关**——通过 models.

**Q4：三、实践：中文与轻量场景怎么用 —— 怎么理解？**
A：**Build 模式**：默认全权限开发
**Plan 模式**：只读、默认拒绝改文件，先探索/规划再动手——适合接手陌生仓库
**轻量**：终端优先、零配置启动、安装即用
**中文友好**不是玄学，而是**模型可换**带来的：中文注释 / 中文文档生成效果取决于你选的模型——接 Qwen / DeepSeek 时中文体验直接拉满
**AGENTS.md**：项目规则写进 AGENTS.
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一、核心观点、二、定义与原理、三、实践：中文与轻量场景怎么用、四、典型场景、五、优劣势、六、最新研究与企业数据（2024–2026）。
