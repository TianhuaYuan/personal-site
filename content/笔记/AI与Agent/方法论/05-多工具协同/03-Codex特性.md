...
> 一句话：要"下工单给并行团队"，用 Codex；要"派活给能跑全流程的同事"，看 [[01-Claude Code特性|Claude Code]]；要"轻量可换模型"，看 [[02-OpenCode特性|OpenCode]]。
...
- 同主题：[[01-Claude Code特性|Claude Code 特性]] · [[02-OpenCode特性|OpenCode 特性]]

## 速记卡（面试闪卡）

**Q1：一句话讲清「Codex：下工单给"在云端并行的外包团队"」到底是什么？**
A：**Codex 是 OpenAI 的云端异步 Agent，强在"云沙箱 + 并行批处理"——你一次下发多个任务，它在隔离沙箱里并行跑，干完把 PR 还给你。**
如果 Claude Code 是"坐你旁边的全流程同事"，Codex 更像**下工单给一个在云端并行干活的外包团队**：你不用盯着每一步，等 PR 回来验收就行。
---
...
**Q3：二、定义与原理 —— 怎么理解？**
A：**Codex**：OpenAI 的 AI 编程 Agent，包含 **Codex CLI**（本地终端）、**Codex 云环境**（云端沙箱）、**IDE 扩展**、**Web App（codex.app）**、**Chrome 扩展**
**运行方式**：云端沙箱（默认）／ 本地终端 ／ Web ／ IDE
**核心模型**：（基于 o3、在真实 PR 上 RL 训练）、（基于 o4-mini、CLI 默认）；

**Q4：三、实践：怎么把 Codex 用出"快" —— 怎么理解？**
A：默认在**隔离云沙箱**里操作你仓库的副本，本地照常写代码，它并行干
PR 在任务完成时回来，不用阻塞等待
codex cloud exec --env ENV_ID --attempts 3 "..." 可一次要多个候选方案（best-of-N）
适合"做这三处迁移"这类彼此独立、本地串行会卡住的模式
**Code review agent**：开 PR 前先自己审一遍（基于 GPT-5-Codex，专为抓关键缺陷训练）
...
| 模式 | 异步并行、下工单式 | 需要实时盯着每一步交互 |
| 角色 | 需要"同时推进很多独立任务" | 只想写两行并立刻看结果 |
一句话：要"下工单给并行团队"，用 Codex；

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、核心观点、二、定义与原理、三、实践：怎么把 Codex 用出"快"、四、典型场景、五、优劣势、六、最新研究与企业数据（2024–2026）。
