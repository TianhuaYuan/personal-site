...
- **进阶**：[[01-Claude Code特性|Claude Code]] · [[02-OpenCode特性|OpenCode]] · [[03-Codex特性|Codex]] · [[11-工具切换成本|工具切换成本]]
...
A：**一个项目里混用多种 Agent，本质是"按长处分工"——让每个工具干它最擅长的那段，整体效率最高。**
就像装修不会让水电工去刷墙：Claude Code 做复杂重构、Codex 跑批量并行、OpenCode 做中文/可换模型场景。混用取长补短，但也要付"协调成本"。
---
...
**Q3：二、为什么混用 —— 怎么理解？**
A：**Claude Code**：Loop Engineering 最强（自校正、Agent Teams）
**Codex**：云端并行、批处理最快
**OpenCode**：模型可换、中文/本地隐私友好
用 Claude Code 跑"需要边干边验证"的复杂重构
用 Codex 把"彼此独立的批量任务"并行扔进云沙箱
...
**Q5：四、混用的挑战 —— 怎么理解？**
A：**上下文切换**：不同工具上下文不同，接力时可能丢信息 → 用 AGENTS.md / CLAUDE.md 固化约定
**配置管理**：不同工具配置不同 → 用 Plugins / 版本控制统一
**学习成本**：团队要学多个工具 → 用 ACP Registry 在一个 IDE 内切换，降低门槛
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、核心观点、二、为什么混用、三、混用策略（Mermaid）、四、混用的挑战、五、优劣势小结、六、最新研究与企业数据（2024–2026）。
