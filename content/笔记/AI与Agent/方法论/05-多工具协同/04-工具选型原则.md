...
- **进阶**：[[01-ClaudeCode特性|Claude Code]] · [[02-OpenCode特性|OpenCode]] · [[03-Codex特性|Codex]] · [[10-混用Agent策略|混用 Agent 策略]]
...
A：工具选型原则：别看谁最强，看“协作姿势”——Claude Code 派活、Codex 下工单、OpenCode 换钻头。

**Q2：一、核心观点 —— 怎么理解？**

A：选型不是“哪个最强”，而是“哪个协作姿势最贴任务”。三种委派姿势：Claude Code=派活给跑全流程的同事、Codex=下工单给并行团队、OpenCode=可换钻头的乐高扳手。选错姿势比不用更费时（delegation posture）。

**Q3：二、按复杂度与团队选型 —— 怎么理解？**

A：简单用 OpenCode（轻量）、中等用 Claude Code（自校正）、复杂用 Claude+Codex 混用。个人 Claude Code、小团队加 AGENTS.md、大团队 AGENTS.md+多工具统一管理。工具形状要匹配任务形状（fit-for-purpose）。

**Q4：三、决策树与铁律 —— 怎么理解？**

A：决策树：实时闭环→Claude Code；批量异步→Codex；换模型/中文/隐私→OpenCode。铁律：默认从最简方案起步（Anthropic），别为“听说更强”频繁换工具。简单方法失败才上 Agent（start simple）。

**Q5：四、优劣势与坑 —— 怎么理解？**

A：✅ 按姿势选型上手快返工少，混用取长补短。❌ 坑：为“听说更强”切换（成本>收益）、用 Codex 干日常业务（脱离 IDE）、所有任务都上最重 Claude Code（轻任务背重 harness）。

**Q6：核心速记主线有哪些？**

- 三姿势：Claude Code 派活 / Codex 下工单 / OpenCode 换钻头

- 复杂度：简单 OpenCode、中等 Claude、复杂混用

- 团队：个人 Claude、小团队加 AGENTS.md、大团队统一

- 铁律：从最简方案起步，别为传闻频繁换工具

- 坑：切换成本>收益、Codex 干日常、轻任务背重 harness
