...
- **Google《Towards a Science of Scaling Agent Systems》(2025/2026)**：独立并行 Agent 若没有验证瓶颈，会把单个错误**放大 17.2 倍**；中心化架构靠协调者验证也只能压到 4.4 倍——这从反面说明「独立审查」为什么必要（见 [[12-MakervsChecker自检|Maker vs Checker 自检]]）。
...
**Q1：一句话讲清「分离规划与执行（Maker & Checker）」到底是什么？**

A：Maker 负责生成、Checker 独立审查，禁止同一 Agent 自己审自己。

**Q2：一、核心观点 —— 怎么理解？**

A：像银行四眼原则：开票和盖章不能同一人，一笔交易要两人之手，防的就是"自己给自己打分"。Maker 生成、Checker 审查，二者分开。四眼原则叫 Four-Eyes Principle。

**Q3：二、定义与原理（类比先行） —— 怎么理解？**

A：像工厂的"写稿人"和"校对员"：Maker 出初稿/代码/方案，Checker 找漏洞；源自银行四眼原则，搬到 AI 就是 Creator Agent 写实现、Checker Agent 独立审。

**Q4：三、为什么需要分离？ —— 怎么理解？**

A：像不让考生自己改卷：同一 Agent 自检有相同盲区、易陷确认偏误、还因谄媚倾向（sycophancy）放水；独立 Checker 视角不同，质量更稳。谄媚叫 Sycophancy。

**Q5：四、示例对比 —— 怎么理解？**

A：像"自审"vs"互审"：同一模型又写又审容易放水；拆成 Maker 出方案、Checker 按三条标准验证（正确？新 bug？符合标准？），质量立刻上一个台阶。

**Q6：核心速记主线有哪些？**

- 核心：生成（Maker）与审查（Checker）分离

- 铁律：绝不让同一 Agent 自检（盲区+谄媚）
...
同 Agent 自检盲区，谄媚放水质量低；

Maker 写稿 Checker 审，独立视角更稳密；
