...
A：**Reflexion（带自我反思）**：适合需要多轮迭代、质量敏感的场景。
像写论文被拒稿后写「审稿意见回复信」——不是重写，而是针对具体意见改，改完再投，直到达标。
---
...
**Q4：三、优缺点 —— 怎么理解？**
A：✅ 持续改进；多次迭代后质量高；有反思过程可解释
❌ 慢、贵（每轮都调 LLM）；必须有明确评估函数与最大迭代次数
---

**Q5：四、生产化注意事项 —— 怎么理解？**
A：❌ 错误做法：执行 → 自己评估 → 自己反思 → 重试（自己审自己，漏盲区）
✅ 正确做法：执行 → **独立 verifier** 评估 → 结构化反思 → 重试
要求：verifier 独立于 actor；反思有界、结构化；**设最大迭代次数**（如 3 轮），防死循环。这正好衔接  的「独立审查」原则。
---

**Q6：核心速记主线有哪些？**
A：抓住这几根：一、核心观点、二、定义与原理（类比先行）、三、优缺点、四、生产化注意事项、五、适用场景、六、最新研究与企业数据（2024–2026）。
