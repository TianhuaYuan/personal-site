...
**Q1：一句话讲清「Maker-Checker 实战落地」到底是什么？**

A：把「Checker」从 Prompt 提醒升级成确定性护栏——用 post-edit hook 自动跑 lint+typecheck，不通过就卡住。

**Q2：一、核心观点：靠 Hook 不靠提醒 —— 怎么理解？**

A：Hook 像收银台扫码枪：不管店员记不记得，钱都得过这关。Checker 落地也一样——把验证嵌进工具链，而非指望模型自觉。

**Q3：二、定义：Hook 是什么 —— 怎么理解？**

A：Hook 是特定事件点自动执行的脚本：PreToolUse 像防火墙拦一道，PostToolUse 在编辑后自动检查，Notification 发通知——三个点把验证焊进流程。

**Q4：三、实战：post-edit hook 配置 —— 怎么理解？**

A：在 settings.json 给 PostToolUse 配 matcher=Edit|Write，命令 ruff+mypy。像给每篇作文自动过两道校对，退出码 0 放行、2 阻断，漏网难。

**Q5：四、其他落地：pre-commit/CI/IDE —— 怎么理解？**

A：不止 hook 一种：Git pre-commit 跑全套检查，CI 流水线统一把关，IDE 插件实时标红。本质都是「把 Checker 确定化」，别赌模型自觉。
