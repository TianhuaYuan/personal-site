...
**不能是同一个 Agent 自检**——否则「自己给自己打分」，漏过盲区（详见 [[12-MakervsChecker自检|Maker vs Checker 自检]]）。
...
- **Google Research (2025/2026)**：无验证瓶颈的独立并行 Agent 会把错误**放大 17.2 倍**，中心化也只压到 4.4 倍——反证「独立审查」的必要（见 [[12-MakervsChecker自检|Maker vs Checker 自检]]）。
...
A：Maker-Checker 是让 Creator Agent 写实现、Checker Agent 独立审查的双重校验，杜绝同一模型自检放水。

**Q2：为什么不能自己当裁判 —— 怎么理解？**

A：就像运动员不能兼裁判，自己给自己打分永远判不出犯规。这源自银行业的 Four-Eyes Principle（四眼原则：一笔交易必须两人经手）；搬到 AI 上，独立审查的 Checker 能戳破 Maker 的盲区与谄媚倾向。

**Q3：职责怎么切分 —— 怎么理解？**

A：像医院分诊台：Maker 出初稿、写代码、做方案；Checker 审输出、对标准、找漏洞。但判对错这种硬活（pytest / ruff / mypy）交给确定性测试，别让模型拍板。

**Q4：有数据支撑吗 —— 怎么理解？**

A：像两名质检员分开看货更可靠。Anthropic 发现把「干活的」和「挑刺的」拆成独立上下文窗口是提质的 key lever（关键杠杆）；Google 更测出无验证的并行 Agent 会把错误放大 17.2 倍。

**Q5：怎么落地最稳 —— 怎么理解？**

A：像夫妻分工记账，但账本用机器对账最靠谱。工程上用 Claude Code hook 自动跑硬门禁做验证；模型只负责「写」和「挑」，最终对错由确定性测试一锤定音。

**Q6：核心速记主线有哪些？**

- Maker 写、Checker 审，双重校验绝不自检

- 源自四眼原则：两双眼睛才保险

- 独立上下文是质量关键杠杆（Anthropic）

- 硬门禁用确定性测试，模型不拍板

**口诀**

A：Maker 写 Checker 审，自己打分必走神
...
- 同主题：[[03-分离规划与执行|分离规划与执行]] · [[12-MakervsChecker自检|Maker vs Checker 自检]]
