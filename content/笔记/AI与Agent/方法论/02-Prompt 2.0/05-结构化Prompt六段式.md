...
A：**Prompt 分层的标准结构**：角色 → 能力边界 → 决策准则 → 输出格式 → 安全规则 → 错误处理。
Agent 不是答题的学生，是替你干活的同事——你不会只甩一句「去干活」，而是得说清「你是谁、能碰啥、优先啥、交啥样、红线在哪、出事咋办」。
---
...
A：把六段式想成给新同事的入职 JD（Job Description）：
**角色** = 你是谁、对啥负责
**能力边界** = 你会啥、不碰啥
**决策准则** = 遇到矛盾先保哪个
**输出格式** = 交付物长啥样
...
**Q4：三、为什么是这六段？ —— 怎么理解？**
A：**角色 + 能力边界**：防止 Agent 越权、乱作为。
**决策准则**：把「优先级」写死，避免它在两难时随机选。
**输出格式**：下游好解析、好接 Checker（见 ）。
**安全规则 + 错误处理**：生产环境的「安全带」与「降落伞」。
---
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一、核心观点、二、定义与原理（类比先行）、三、为什么是这六段？、四、实践示例（代码审查）、五、优劣势、六、最新研究与企业数据（2024–2026）。
