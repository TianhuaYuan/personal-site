...
A：**四层是嵌套关系（外层包含内层），不是替代关系**：新层出来，旧层变成它的子集，不是作废。即「外层包含内层」——Loop 包含 Harness 包含 Context 包含 Prompt，详见下方嵌套关系图。
---
...
**Q3：二、嵌套关系图（Mermaid） —— 怎么理解？**
A：---

**Q4：三、「包含」与「不是替代」的含义 —— 怎么理解？**
A：**Prompt 在 Context 中**：Prompt 是模型看到的上下文的一部分
**Context 在 Harness 中**：Context 运行在 Harness 提供的环境里
**Harness 在 Loop 中**：Harness 是 Loop 的一个执行单元
**都不会过时**：每次调用仍要 Prompt；
...
**Q6：核心速记主线有哪些？**
A：抓住这几根：一、核心观点、二、嵌套关系图（Mermaid）、三、「包含」与「不是替代」的含义、四、每层的定位、五、实践中的应用、六、学习资源。
