...
**Q1：一句话讲清「2026 中 Loop Engineering 爆发：从手动调 Prompt 到设计自主循环」到底是什么？**
A：Loop Engineering 爆发：你不再逐次手写 Prompt，而是造一套"会自己 prompt Agent 的循环"。

**Q2：一、核心观点 —— 怎么理解？**
A：从"聊天框里那个人"变成"造聊天框机器的人"——你设计、调优、测试 loop，而不只是敲 prompt（Loop as Object）。

**Q3：二、四大退出机制 —— 怎么理解？**
A：loop 绝不能"问 LLM 该不该停"——用确定性验证器、步数上限、无进展检测、成本上限四道闸（Exit Conditions）。

**Q4：三、六大构件 —— 怎么理解？**
A：Automations 定时扫活、Worktrees 隔离目录、Skills 复用知识、Connectors 接外系统、Sub-agents 分工、State 落盘（Loop Primitives）。

**Q5：四、Maker-Checker —— 怎么理解？**
A：生成器绝不给自己的活打分——独立 Checker 像考官挑刺，消除自评"点头循环"，实证 6 倍提升（Independent Verifier）。

**Q6：核心速记主线有哪些？**
- 本质：loop 成工程首要对象，可设计、调优、测试
- 退出：四机制确定性，绝不问 LLM 该停否
- 构件：自动化/工作树/Skills/连接器/子 Agent/状态
- 铁律：Maker 写、Checker 独立验，跨模型互审

**口诀**
A：Loop 工程火二六，造机不敲 prompt 令；
退出四闸确定硬，莫让模型自己停；
六大构件搭骨架，Skills 复用状态定；
Maker 写来 Checker 验，独立考官才清醒。

## 相关链接
- 系列清单：[[学习路线图/Agent方法论与产品思维学习路线图|Agent 方法论与产品思维学习路线图]]
- 上一层级：[[00-Agent方法论与产品思维|认知升级：四层模型 · 索引]]
- 同主题：[[03-2026初HarnessEngineering|Harness Engineering]] · [[08-BorisCherny语录|Boris Cherny 语录]]
