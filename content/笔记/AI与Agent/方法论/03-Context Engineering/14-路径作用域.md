...
A：**路径作用域（Path-scoping）**：子目录的 CLAUDE.md 只在该目录下生效，按文件路径加载对应规则——把全局上下文「分田到户」，谁的地盘谁的规则，互不污染。
就像公司制度：总部发一本《员工手册》管全员，但研发部、财务部、客服部各自还有部门细则，你进哪个部门就只看哪个部门的规定，不用把全公司制度都背身上。Agent 也是——在  里就加载 ，在  里就加载 ，全局只留最核心的。
...
A：路径作用域 = 根据当前文件路径，只加载该路径及其祖先目录下的规则文件。它是  的「空间版」：不在当前工作目录的规则，根本不进上下文，从根上省下 。Claude Code 原生支持目录级 CLAUDE.md。
工具实现上，从文件路径向上逐级查找 CLAUDE.md，从全局到局部拼接—— 目录最终加载「全局 100 行 + src 50 行」，而非全部 500 行。
**根文件只放永真约束**：跨目录都适用的架构红线、硬性禁止项。
...
**Q5：最新研究与企业数据 —— 怎么理解？**
A：**原生能力（一手）**：Claude Code 支持在子目录放置 CLAUDE.md，按当前路径加载对应规则；官方建议保持文件精简、长文档外置（来源：Anthropic Claude Code Best Practices / CLAUDE.md 文档）。
