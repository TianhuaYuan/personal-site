...
文件大小与注意力衰减描述的是：**常驻规则文件的体积，和模型对其中规则的遵循率呈负相关**。它同时踩中两个坑——[[02-AttentionBudget|Attention Budget]]（总 token 多→预算被摊薄）和 [[01-LostintheMiddle|Lost in the Middle]]（长文件里中间条款最易被忽略）。
...
**Q1：一句话讲清「文件大小与注意力衰减：CLAUDE.md 越长，模型越「懒得看」」到底是什么？**

A：CLAUDE.md / AGENTS.md 这种"每次会话自动加载"的常驻规则文件越大，模型对它的遵循率越低——不是看不懂，是看不过来，关键条款容易埋进"注意力洼地"被默默无视。

**Q2：核心观点 —— 怎么理解？**

A：类比便利店的《员工守则》：两页纸大家能背，600 页没人翻。模型也一样，根文件越长，关键条款越容易掉进注意力洼地。所以 CLAUDE.md 不是写得越全越好，而是"够用 + 好找"才好用。

**Q3：定义 / 原理 / 实践 / 示例 / 优劣势 —— 怎么理解？**

A：原理上它同时踩两个坑：Attention Budget（总 token 多→预算被摊薄）和 Lost in the Middle（长文件中间条款最易被忽略）。实践上：根文件控在 ~200 行内，超了就拆；根文件当目录，只放核心约束 + 用 @path 导入 docs/ 详情；子目录各自用路径作用域管；关键永真规则写进开头（窗口顶部衰减最慢）。优点是常驻文件几乎零成本持久记忆，放开头的不衰减；缺点是过长反而变噪声源。

**Q4：核心要点 —— 怎么理解？**

A：速记：文件大小衰减 = 常驻规则文件越长遵循率越低；根文件建议控在 ~200 行内超了就拆；根文件用法是当目录 + @path 导入详情；它同时踩中 Attention Budget + Lost in the Middle 双坑。

**Q5：最新研究与企业数据 —— 怎么理解？**

A：一手原则：Anthropic《Claude Code Best Practices》明确"上下文窗口填得越快性能越差"，上下文是"最需要管理的资源"，建议 CLAUDE.md 保持精简、长文档外置。具体行数—遵循率曲线（如 1-100 行 ~95%、600+ 行 ~45%）来自社区复盘，未见一手实验，建议自测。实践共识是 CLAUDE.md 控制在 100-200 行。

**Q6：核心速记主线有哪些？**

A：抓住这几根：核心观点（越长越懒得看）、定义/原理（Attention Budget + Lost in the Middle 双坑）、工程实践（200 行内 + @path + 路径作用域 + 开头放永真）、核心要点、最新研究（Anthropic 一手原则，曲线待核实）。
