...
渐进式加载 = 把信息按「需要的概率」分层，高频常驻、低频外置、用时再取。它直接对抗 [[02-AttentionBudget|Attention Budget]]：少占常驻额度，把预算留给当下任务。Anthropic（2025-09）把「按需策展 token」列为 context engineering 的核心纪律。
...
- **子智能体隔离**：重型调研交给 Sub-agent，主会话只收结论（见 [[02-AttentionBudget|Attention Budget]]）。
...
- **进阶**：[[07-RecencyEffect|Recency Effect]] · [[08-摘要压缩|摘要压缩]] · [[14-路径作用域|路径作用域]]
...
- 四招：根目录 @path / Skills 触发 / RAG top-k / Sub-agent

- 代价：前期分层归档有成本，链路太多变复杂
...
**下一篇**：[[07-RecencyEffect|Recency Effect]]——渐进式加载讲完，下篇讲「模型对末尾指令更买账，hard gate 放最后」。
