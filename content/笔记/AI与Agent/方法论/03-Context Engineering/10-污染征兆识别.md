...
Context Pollution = 上下文中存在不准确 / 不完整 / 不相关信号，稀释了高信号 token 的权重。它和 [[02-AttentionBudget|Attention Budget]] 同源：噪声每多一个，真信号就被分走一份注意力。Anthropic 称之为 context rot（上下文衰减）的表现之一。
...
- **历史对话污染**：早期规则滚到中间被忽略（见 [[01-LostintheMiddle|Lost in the Middle]]）。
...
- 核心：上下文发霉，脏信息带偏决策非模型笨

- 四征兆：跑偏/忘令/自语/质量降

- 四来源：历史/检索/工具/过时

- 诊断三看：看 context + 日志 + 用户反馈
