...
A：大规模知识库拼的不是"能不能检"，而是摄入可运维、分块可实验、检索可监控。

**Q2：规模带来的新挑战 —— 怎么理解？**
A：像超市从小店扩成连锁万店：几百件商品随手找，百万件必须上系统盘库存、盯效期、查损耗。文档过百万，问题从"能不能检索"变成"质量稳不稳、运维累不累、成本控不控"。

**Q3：数据摄入 Pipeline（可运维） —— 怎么理解？**
A：像物流收货流水线：变更监听→解析清洗→分块打标→向量化→入库，还带增量同步把旧货下架、用 doc_id+chunk_hash 去重保幂等、失败进死信可重跑，不丢文档。

**Q4：分块策略调优（实验驱动） —— 怎么理解？**
A：像裁缝量体裁衣：结构化文档用递归/语义剪，纯文本用固定窗口。搭一套固定评测集扫 chunk_size/overlap，用元数据标签（来源/时间/权限）预过滤缩候选，选最优不靠拍脑袋。

**Q5：检索质量监控（可量化） —— 怎么理解？**
A：像给生产线装仪表盘：在线埋点看命中率、Top-K 相关度、生成 Faithfulness；离线定期跑全库看指标漂移；低分坏 Case 回标注反推分块/嵌入/检索参数，形成闭环。

**Q6：核心速记主线有哪些？**
- 三重点：可运维摄入、可实验分块、可量化监控
- 摄入：增量同步 + 幂等去重 + 失败重试
- 分块：按文档类型分策略 + 实验选优 + 元数据过滤
- 监控：在线埋点 + 离线回归 + 坏 Case 回流
