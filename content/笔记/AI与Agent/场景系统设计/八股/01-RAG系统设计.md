...
A：RAG 系统设计把「检索→生成」从 demo 拼凑升级成可观测、可回归、可扩展的工程系统。

**Q2：为什么需要"设计"而非"拼链路" —— 怎么理解？**

A：朴素 RAG 上线就翻车：检索不稳、长尾答不出、效果看不见。像开餐馆不能只把食材扔锅里，得有供应链、品控和出餐 SOP。

**Q3：双路径架构：摄入与查询解耦 —— 怎么理解？**

A：离线摄入和在线查询走不同节奏，只在向量库交汇。像仓库备货（离线）和前台点单（在线）互不堵，一方垮了另一半还能撑。

**Q4：模块一：数据摄入路径 —— 怎么理解？**

A：PDF/Word/HTML/数据库解析质量天差地别，决定检索上限。像不同食材处理难度不同，切配不到位，炒出来必夹生。

**Q5：模块二：查询与评估闭环 —— 怎么理解？**

A：用指标驱动分块、检索、重排调参，靠缓存异步降本。像给菜品装称重和评分表，持续改进而非凭手感。
