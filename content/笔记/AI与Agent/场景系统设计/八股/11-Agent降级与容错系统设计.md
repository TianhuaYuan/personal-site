...
A：生产级 Agent 的底线是"**默认不崩溃**"：即使部分工具/模型不可用，也要给出"尽可能好"的结果，而不是整条链路 500。

**Q2：设计原则 —— 怎么理解？**
A：生产级 Agent 的底线是"**默认不崩溃**"：即使部分工具/模型不可用，也要给出"尽可能好"的结果，而不是整条链路 500。

**Q3：① 重试策略 —— 怎么理解？**
A：**指数退避 + 抖动**：避免重试风暴打爆依赖。
**只重试安全错误**：网络超时、429 可重试；**编程错误（4xx 参数错）不重试**，否则无意义。
**幂等**：重试前确认工具调用可重复（如查询幂等，写操作需幂等键），见 。

**Q4：② 熔断机制 —— 怎么理解？**
A：错误率超阈值 → 熔断器"打开"，短时间内直接拒绝，不再打依赖（给它喘息）。
一段时间后"半开"探测，成功则恢复"关闭"。
与分布式熔断思路一致，见 。

**Q5：核心速记主线有哪些？**
A：抓住这几根：设计原则、四机制架构、① 重试策略、② 熔断机制、③ Fallback 兜底、④ 降级路径。
