...
A：当 Agent 从"一个脚本"变成"公司多个业务都在用"时，会出现重复造轮子、工具各自为政、安全无统一防线、无法统一观测等问题。平台化要解决四件事：**能力复用（Skill）、工具统一治理、安全隔离、可观测**。

**Q2：为什么需要 Agent 平台 —— 怎么理解？**
A：当 Agent 从"一个脚本"变成"公司多个业务都在用"时，会出现重复造轮子、工具各自为政、安全无统一防线、无法统一观测等问题。平台化要解决四件事：**能力复用（Skill）、工具统一治理、安全隔离、可观测**。

**Q3：模块一：Harness（编排核心） —— 怎么理解？**
A：负责 Agent 的"思考-行动-观察"主循环、最大步数限制、循环检测、错误处理与降级。它是平台的稳定内核，业务方只写"技能"和"工具"，不碰编排。详见 。

**Q4：模块二：Skill 加载（插件化能力） —— 怎么理解？**
A：每个 Skill = 一段可复用的提示词 + 工具组合 + 输入输出契约。
平台按需动态加载，避免把所有能力塞进一个超长 System Prompt。
版本化管理 Skill，支持灰度与回滚。

**Q5：核心速记主线有哪些？**
A：抓住这几根：为什么需要 Agent 平台、平台总体架构、模块一：Harness（编排核心）、模块二：Skill 加载（插件化能力）、模块三：工具注册中心（统一治理）、模块四：安全隔离。
