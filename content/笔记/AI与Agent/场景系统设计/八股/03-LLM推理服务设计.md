...
A：LLM 推理服务设计：把模型部署成高吞吐、低延迟、可弹性伸缩的在线服务。

**Q2：一、设计目标 —— 怎么理解？**
A：在线推理像开高峰期餐厅：贵（显存）、慢（逐 token）、客流忽大忽小——要在翻台率、上菜速度、成本间权衡（Throughput/Latency/Cost）。

**Q3：二、推理网关与模型放置 —— 怎么理解？**
A：网关像餐厅迎宾：鉴权、按难易派单（简单问题给小模型）、长请求单列队；热门模型常驻显存、长尾按需加载（Gateway & Placement）。

**Q4：三、Prefill/Decode 分离 —— 怎么理解？**
A：备菜（算整段输入）和炒菜（逐 token）瓶颈不同，分开用不同 GPU 各自优化，靠 KV Cache 衔接（Decoupled Prefill/Decode）。

**Q5：四、KV Cache 管理 —— 怎么理解？**
A：KV Cache 像冰箱：热料留 GPU 显存、冷料换 CPU/SSD；多轮对话前缀可复用，量化省显存（KV Cache Tiering）。
...
A：推理服务三大难，贵慢波动要权衡；
网关路由分得清，简单任务小模型；
Prefill Decode 分开跑，各用 GPU 各显能；
KV Cache 分冷热，前缀复用显存轻。
