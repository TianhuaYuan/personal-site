...
**Q1：一句话讲清「---- 配置日志格式，方便观察权限检查过程 ----」到底是什么？**
A：普通程序的用户能做什么是开发者**预先写死**的——按钮"查看订单"只能查，"删除"按钮才有删的权限。但 Agent 不同：它根据自然语言指令**自己决定**调用哪个工具，执行路径不可预枚举。如果你给 Agent 一把"万能钥匙"，攻击者通过 Prompt Injection 就可能让它删库。
