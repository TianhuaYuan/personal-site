...
# ============================================================
# 4. LLM 二次判断 —— 对可疑输入做深度检测
# ============================================================
# 注意：这个函数需要 LLM 实例。以下为接口定义和调用方式，
...
**Q1：一句话讲清「============================================================」到底是什么？**
A：普通程序和 LLM 的核心区别：
**普通程序**：代码和数据物理隔离。你在搜索框输入 ，它只会被当成搜索关键词，不会真的删表——因为有参数化查询把数据和 SQL 指令分开了。
**LLM**：指令和数据都是**同一串 token**。它读到的所有文字——System Prompt、用户输入、工具输出——在模型眼里没有天然边界。
...
- 上一篇：[[3-MCP Gateway鉴权：Bearer Token + JWT中间件实现]]
- 下一篇：[[5-OWASP Top 10 for Agentic AI概念了解]]
