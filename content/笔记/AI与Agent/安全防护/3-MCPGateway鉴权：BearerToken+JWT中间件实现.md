...
tags:
  ...
  - agent安全
---
# MCP Gateway鉴权：Bearer Token + JWT中间件实现
...
**Q1：一句话讲清「MCP Gateway鉴权：Bearer Token + JWT中间件实现」到底是什么？**
A：MCP Gateway 在入口集中做双层鉴权，Bearer 验应用身份、JWT 验用户身份并贯穿请求链路。

**Q2：基本原理 —— 怎么理解？**
A：像小区门禁加身份证双证：Bearer Token 是门禁卡验"哪个应用在调"，JWT 是身份证验"哪个用户在用"（Bearer Token，持有者令牌）。

**Q3：代码实现 —— 怎么理解？**
A：像工厂流水线：JWTIssuer 签发 Access/Refresh，恒定时间比较防时序攻击，Refresh 一次一换（JSON Web Token，JWT）。

**Q4：中间件洋葱模型与顺序 —— 怎么理解？**
A：像洋葱剥层：Bearer 在最外先挡无效请求，省 CPU 解析 JWT 防 DoS；JWT 在内层验用户（Onion Model，洋葱模型）。

**Q5：contextvars 贯穿请求链路 —— 怎么理解？**
A：像每单的专属便签：请求级用户身份写进 contextvars，工具随用随取、结束自动清理不串味（Context Variables，上下文变量）。

**Q6：核心速记主线有哪些？**
- 基本原理：Gateway 单点认证加集中审计，Bearer 验应用、JWT 验用户
- 代码实现：JWTIssuer 签发双 Token，恒定时间比较防时序攻击
- 洋葱模型：Bearer 最外层先挡，避免无效请求耗 CPU
- contextvars：用户身份随请求流转，工具无需显式传参
...
- 下一篇：[[4-PromptInjection防御：输入过滤-上下文隔离-前置规则]]
