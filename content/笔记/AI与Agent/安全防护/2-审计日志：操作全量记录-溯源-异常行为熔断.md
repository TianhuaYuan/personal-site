...
**Q1：一句话讲清「审计日志：操作全量记录-溯源-异常行为熔断」到底是什么？**

A：审计日志是 Agent 的"监控录像"：全量记录工具调用，可溯源并据异常自动熔断。

**Q2：基本原理 —— 怎么理解？**

A：像小区监控：权限分级管"事前"（能不能做），审计日志管"事后"（做了什么），缺一不可。三条硬约束——结构化（记全 who/when/what/result/permission）、不可篡改（append-only）、隔离存储（Agent 够不到）。

**Q3：代码实现 —— 怎么理解？**

A：像三层保险：AuditEvent 记全字段；AuditLogger 只追加写入，并用哈希链（每条指向前一条哈希）防篡改；AnomalyDetector 扫日志找深夜高频、突发删除潮等异常模式。

**Q4：审计事件与哈希链 —— 怎么理解？**

A：像装订的账本：每条 AuditEvent 含身份链/时间链/操作链/结果链/权限链，外加 `previous_event_hash` 串成哈希链。改中间任一条，下一条对不上就断裂——`verify_chain_integrity` 一验便知是否被篡改。

**Q5：异常检测与熔断器 —— 怎么理解？**

A：像保安+断路器：AnomalyDetector 按规则（深夜高频/突发 destructive/单用户暴涨）告警；CircuitBreaker 状态机 CLOSED→OPEN→HALF_OPEN，连触告警就熔断暂停 Agent，冷却后少量探测恢复。

**Q6：核心速记主线有哪些？**

- 原理：权限管事前、审计管事后，结构化+不可篡改+隔离

- 事件：AuditEvent 全字段，哈希链串接防篡改

- 检测：深夜高频/突发删除/用户暴涨三类异常规则

- 熔断：状态机连续告警即 OPEN，冷却后 HALF_OPEN 探测
...
- 下一篇：[[3-MCPGateway鉴权：BearerToken+JWT中间件实现]]
